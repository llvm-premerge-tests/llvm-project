...
public:
  ...
  const ConceptReference *getConceptReference() const { return ConceptRef; }
  ...
  SourceLocation getBeginLoc() const LLVM_READONLY {
    if (auto QualifierLoc = ConceptRef->getNestedNameSpecifierLoc())
      return QualifierLoc.getBeginLoc();
    return ConceptRef->getConceptNameInfo().getBeginLoc();
  }
  ...
  SourceLocation getEndLoc() const LLVM_READONLY {
    ...
    return ConceptRef->hasExplicitTemplateArgs() &&
                   ConceptRef->getTemplateArgsAsWritten()
                       ->getRAngleLoc()
                       .isValid()
               ? ConceptRef->getTemplateArgsAsWritten()->getRAngleLoc()
               : ConceptRef->getConceptNameInfo().getEndLoc();
  }
