...
static bool SemaBuiltinOverflow(Sema &S, CallExpr *TheCall,
  ...
  bool CkdOperation = llvm::any_of(Builtins, [&](const std::pair<unsigned, const char *> &P) {
    return BuiltinID == P.first && Lexer::getImmediateMacroName(
      TheCall->getExprLoc().isMacroID() && Lexer::getImmediateMacroName(
        TheCall->getExprLoc(), S.getSourceManager(), S.getLangOpts()) == P.second
    );
  });
