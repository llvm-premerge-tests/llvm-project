...
Value *EmitAMDGPUWorkGroupSize(CodeGenFunction &CGF, unsigned Index) {
  ...
  if (Cov == clang::TargetOptions::COV_None) {
    ...
    OffsetOld = llvm::ConstantInt::get(CGF.Int32Ty, 4 + Index * 2);
    DP1 = EmitAMDGPUDispatchPtr(CGF);
    auto *GEPOld = CGF.Builder.CreateGEP(CGF.Int8Ty, DP1, OffsetOld);

    auto Result = CGF.Builder.CreateSelect(Iscov5, GEPNew, GEPOld);
