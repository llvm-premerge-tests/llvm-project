...
void CodeGenFunction::EmitIfStmt(const IfStmt &S) {
  ...
  if (const Stmt *Else = S.getElse()) {
    ...
    // When single byte coverage is enabled, add a counter to else block.
    if (llvm::EnableSingleByteCoverage)
  ...
  // When single byte coverage is enabled, add a counter to continuation block.
  if (llvm::EnableSingleByteCoverage)
    incrementProfileCounter(&S);
}
...
void CodeGenFunction::EmitWhileStmt(const WhileStmt &S,
  ...
  // When single byte coverage is enabled, add a counter to loop condition.
  if (llvm::EnableSingleByteCoverage)
  ...
  // because it might be a singleton DeclStmt.
  {
    ...
    EmitBlock(LoopBody);
    // When single byte coverage is enabled, add a counter to the body.
    if (llvm::EnableSingleByteCoverage)
