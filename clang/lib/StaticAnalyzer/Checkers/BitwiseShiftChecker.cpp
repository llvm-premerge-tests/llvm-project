...
class BitwiseShiftValidator {
  ...
  static constexpr unsigned int NoUpperBound = 999;
  // This member stores either the size of an integer type (expressed in bits)
  // or the sentinel NoUpperBound which is larger than all "normal" values:
  unsigned UpperBoundBitCount = NoUpperBound;
...
void BitwiseShiftValidator::run() {
  ...
  // left operand:
  if (BugReportPtr Bug = checkOvershift()) {
    Ctx.emitReport(std::move(Bug));
    return;
  }

  // Report a bug if the right operand is negative:
  if (BugReportPtr Bug = checkOperandNegative(OperandSide::Right)) {
    Ctx.emitReport(std::move(Bug));
    return;
  }
  ...
  if (shouldPerformPedanticChecks()) {
    ...
    if (BugReportPtr Bug = checkOperandNegative(OperandSide::Left)) {
      Ctx.emitReport(std::move(Bug));
      return;
    }

    // Report a bug when left shift of a concrete signed value overflows:
    if (BugReportPtr Bug = checkLeftShiftOverflow()) {
      Ctx.emitReport(std::move(Bug));
      return;
    }
  }
...
bool BitwiseShiftValidator::assumeRequirement(OperandSide Side,
  ...
  auto ResultVal = SVB.evalBinOp(State, Comparison, OperandVal, LimitVal,
...
const NoteTag *BitwiseShiftValidator::createNoteTag() const {
  if (!NonNegOperands && UpperBoundBitCount == NoUpperBound)
    return nullptr;
  ...
  Out << Templ.SignInfo;
  if (UpperBoundBitCount != NoUpperBound)
    Out << Templ.UpperBoundIntro << UpperBoundBitCount;
