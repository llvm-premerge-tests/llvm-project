...
// Get a name Fragment by finding the matching template parameter for an arg
DeclarationFragments
DeclarationFragmentsBuilder::getFragmentsForTemplateArgument(
    const ArrayRef<NamedDecl *> TemplateParameters, std::string TypeParameter) {
  DeclarationFragments Fragments;
  // The arg is a generic parameter from a partial spec, e.g. T in
  // template<typename T> Foo<T, int>. Those names appear as
  // "type-parameter-<index>-<depth>", so we must find its name from the
...
DeclarationFragmentsBuilder::getFragmentsForTemplateArguments(
  ...
  for (unsigned i = 0, end = TemplateArguments.size(); i != end; ++i) {
    ...
    if (Type.substr(0, 14).compare("type-parameter") == 0) {
      Fragments.append(
          getFragmentsForTemplateArgument(*TemplateParameters, Type));
    } else
      // A non-generic arg
      Fragments.append(getFragmentsForType(TemplateArguments[i].getAsType(),
                                           Context, After));
