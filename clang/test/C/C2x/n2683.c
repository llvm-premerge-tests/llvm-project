...
void test_invalid_input() {
  ...
  bool flag_no_bitint = ckd_add(&result, a33, a); /* expected-error {{operand argument to checked integer operation must be an integer type '_BitInt(33)' invalid}} */
  bool flag_no_bool = ckd_sub(&result, bool_var, b); /* expected-error {{operand argument to checked integer operation must be an integer type 'bool' invalid}} */
  bool flag_no_char = ckd_mul(&result, char_var, a); /* expected-error {{operand argument to checked integer operation must be an integer type 'char' invalid}} */
  bool flag_no_enum = ckd_mul(&result, day, b); /* expected-error {{operand argument to checked integer operation must be an integer type 'enum week' invalid}} */
  ...
  bool flag_noshort = ckd_mul(&short_result, a, b); /* expected-error {{'short' may not be suitable to hold the result of operating two 'int's}} */
}
