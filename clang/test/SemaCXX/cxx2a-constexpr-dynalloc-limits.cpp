...
void ohno() {
  ...
  int foo[stack_array<1025>()]; // expected-warning {{variable length arrays are a Clang extension}} \
                                // expected-note {{in call to 'stack_array<1025>()'}}

  constexpr int foo[stack_array<1025>()]; // expected-warning {{variable length arrays are a Clang extension}} \
