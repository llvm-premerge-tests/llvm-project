...
void foo ()
{ 
  int b = 1, a[b]; // expected-warning {{variable length arrays are a Clang extension}} \
...
int foo(int b) { // expected-note {{declared here}}
  int varr[b][(b+=8)]; // expected-warning 2{{variable length arrays are a Clang extension}} \
