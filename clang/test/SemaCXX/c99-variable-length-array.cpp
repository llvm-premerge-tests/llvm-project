...
void vla(int N) {
  int array1[N]; // expected-warning{{variable length arrays are a Clang extension}}
  POD array2[N]; // expected-warning{{variable length arrays are a Clang extension}}
  NonPOD array3[N]; // expected-warning{{variable length arrays are a Clang extension}}
  NonPOD2 array4[N][3]; // expected-warning{{variable length arrays are a Clang extension}}
}
...
void vla_in_template(T t) {
  int array2[T::value]; // expected-warning{{variable length arrays are a Clang extension}}
}
...
void inst_with_vla(int N) {
  int array[N]; // expected-warning{{variable length arrays are a Clang extension}}
...
namespace PR7206 {
  void f(int x) {
    ...
    struct edge_info edgeInfo[x]; // expected-warning{{variable length arrays are a Clang extension}}
  }
}
...
namespace rdar8020206 {
  ...
  void f(int i) {
    ...
    int array[value * i]; // expected-warning 2{{variable length arrays are a Clang extension}} expected-note 2{{initializer of 'value' is not a constant}}
  }
...
namespace rdar8021385 {
  ...
  struct B {
    ...
    void f0() {
      int M = 4;
      my_int a[M]; // expected-warning{{variable length arrays are a Clang extension}}
    }
  };
  B<A> a;
}
...
namespace PR8209 {
  void f(int n) {
    typedef int vla_type[n]; // expected-warning{{variable length arrays are a Clang extension}}
...
namespace PR11744 {
  template<typename T> int f(int n) {
    T arr[3][n]; // expected-warning 3 {{variable length arrays are a Clang extension}}
    return 3;
  }
...
namespace pr18633 {
  ...
  template<typename T>
  void func () {
    int arr[A1::sz]; // expected-warning{{variable length arrays are a Clang extension}} expected-note {{initializer of 'sz' is unknown}}
  }
  template<typename T>
  void func2 () {
    int arr[A1::sz2];
