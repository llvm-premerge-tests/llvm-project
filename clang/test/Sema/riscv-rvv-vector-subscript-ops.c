...
void subscript(vint8m8_t i8, vint16m4_t i16, vint32m2_t i32, vint64m1_t i64,
               ...
               vbool1_t b) {
  (void)b[0];    // expected-error{{subscripted value is not an array, pointer, or vector}}
  (void)b[0.f];  // expected-error{{subscripted value is not an array, pointer, or vector}}
  (void)b[0.];   // expected-error{{subscripted value is not an array, pointer, or vector}}
