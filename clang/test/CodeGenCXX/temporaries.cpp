...
namespace Vector {
  ...
  // CHECK: @_ZGRN6Vector1rE_ = internal global i32 0,
  // CHECK: @_ZN6Vector1rE ={{.*}} constant ptr @_ZGRN6Vector1rE_,
  int &&r = S().v[1];

  // CHECK: @_ZGRN6Vector1sE_ = internal global i32 0,
  // CHECK: @_ZN6Vector1sE ={{.*}} constant ptr @_ZGRN6Vector1sE_,
  int &&s = S().w[1];
