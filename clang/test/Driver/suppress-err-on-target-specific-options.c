...
// RUN: touch %t.c
// RUN: not %clang --target=x86_64-unknown-linux-gnu --verbose -mcpu=corei7 %t.c -S 2>&1 | FileCheck %s --check-prefix=ERROR-X86
// RUN: not %clang --target=x86_64-unknown-linux-gnu -### -mcpu=corei7 %t.c -S 2>&1 | FileCheck %s --check-prefix=ERROR-X86

// RUN: %clang --target=aarch64-none-gnu --verbose -mcpu=neoverse-v1 %t.c -S 2>&1 | FileCheck %s --check-prefix=ERROR-AARCH64
// RUN: %clang --target=aarch64-none-gnu -### -mcpu=neoverse-v1 %t.c 2>&1 -S | FileCheck %s --check-prefix=ERROR-AARCH64
