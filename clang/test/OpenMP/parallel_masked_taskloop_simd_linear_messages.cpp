...
int main(int argc, char **argv) {
  ...
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp parallel masked taskloop simd linear(i: val, step(2 // omp52-error {{expected ')' or ',' after 'step expression'}} omp52-error {{expected ')' after 'step expression'}} omp52-error {{expected ')'}}  omp52-note {{to match this '('}}
  for (int k = 0; k < argc; ++k) ++k;
#endif
