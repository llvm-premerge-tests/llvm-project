...
#ifdef OMP52
#pragma omp target simd linear(j: step() //omp52-error{{expected expression}} omp52-error{{expected ')'}} omp52-note{{to match this '('}}
#else
#pragma omp target simd linear(j)
#endif
  for (int k = 0; k < argc; ++k)
    ++k;
#ifdef OMP52
#pragma omp target simd linear(i: step(1),) // omp52-error{{expected linear modifier ('val', 'uval', or 'ref')}}
#else  
#pragma omp target simd linear(i)
#endif
  for (int k = 0; k < argc; ++k)
