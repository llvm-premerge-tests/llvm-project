...
#pragma omp distribute parallel for simd linear(h, C::x) // expected-error 2 {{threadprivate or thread local variable cannot be linear}}
  ...
  #pragma omp parallel
  {
    ...
    #pragma omp target
    #pragma omp teams
    #pragma omp distribute parallel for simd linear(i: pval, step(1)) // omp52-error {{use of undeclared identifier 'pval'}} omp52-warning {{extra tokens at the end of '#pragma omp linear' are ignored}} 
      for (i = 0; i < argc; ++i) ++i;
#endif
  }
