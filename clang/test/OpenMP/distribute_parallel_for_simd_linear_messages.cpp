...
#pragma omp distribute parallel for simd linear(h, C::x) // expected-error 2 {{threadprivate or thread local variable cannot be linear}}
  ...
  #pragma omp parallel
  {
    ...
    #pragma omp target
    #pragma omp teams
    #pragma omp distribute parallel for simd linear(i: step(1), step(2)) // omp52-warning {{multiple 'step size' found, ignoring the previous one}}
      for (i = 0; i < argc; ++i) ++i;
    #pragma omp target
    #pragma omp teams
    #pragma omp distribute parallel for simd linear(i: val, val) // omp52-warning {{multiple 'linear modifier' found, ignoring the previous one}}
      for (i = 0; i < argc; ++i) ++i;
    #pragma omp target
    #pragma omp teams
    ...
    #pragma omp target
    #pragma omp teams
    #pragma omp distribute parallel for simd linear(i: val, step(2 // omp52-error {{expected ')' or ',' after 'step expression'}} omp52-error {{expected ')' after 'step expression'}} omp52-error {{expected ')'}}  omp52-note {{to match this '('}}
      for (i = 0; i < argc; ++i) ++i;
#endif
  }
