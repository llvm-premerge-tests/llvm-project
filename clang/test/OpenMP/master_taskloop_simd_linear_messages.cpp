...
int main(int argc, char **argv) {
  ...
  for (int k = 0; k < argc; ++k) ++k;
#ifdef OMP52
  #pragma omp master taskloop simd linear(i: step(1),) // omp52-error{{expected linear modifier ('val', 'uval', or 'ref')}}
#else  
  #pragma omp master taskloop simd linear(i)
#endif
  for (int k = 0; k < argc; ++k) ++k;
#ifdef OMP52
  #pragma omp master taskloop simd linear(j: step()) // omp52-error{{expected expression}}
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp master taskloop simd linear(j: step(1), pval) // omp52-error{{expected one of 'ref', val' or 'uval' modifiers}}
  for (int k = 0; k < argc; ++k) ++k;
#endif
