...
#pragma omp distribute simd linear(h, C::x) // expected-error 2 {{threadprivate or thread local variable cannot be linear}}
  ...
  #pragma omp parallel
  {
    ...
    #pragma omp target
    #pragma omp teams
    #pragma omp distribute simd linear(k: pval, step(1)) // omp52-error {{use of undeclared identifier 'pval'}} omp52-warning {{extra tokens at the end of 'simple step modifier' are ignored}}
      for (k = 0; k < argc; ++k) ++k;
#endif
  }
