...
int main(int argc, char **argv) {
...
#ifdef OMP52
  #pragma omp for linear(j: step() //omp52-error{{expected expression}} omp52-error{{expected ')'}} omp52-note{{to match this '('}}
#else
  #pragma omp for linear(j)
#endif
  for (int k = 0; k < argc; ++k) ++k;
#ifdef OMP52
  #pragma omp for linear(i: step(1),) // omp52-error{{expected linear modifier ('val', 'uval', or 'ref')}}
#else
  #pragma omp for linear(i)
#endif
  for (int k = 0; k < argc; ++k) ++k;
  ...
  for (int k = 0; k < argc; ++k) ++k;
#ifdef OMP52
  #pragma omp for linear(j: step()) // omp52-error{{expected expression}}
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp for linear(j: step(1), pval) // omp52-error{{expected one of 'ref', val' or 'uval' modifiers}}
  for (int k = 0; k < argc; ++k) ++k;
#endif
