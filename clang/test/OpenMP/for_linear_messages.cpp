...
int main(int argc, char **argv) {
  ...
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp for linear(j: step(1), step(2)) // omp52-warning {{multiple 'step size' found, ignoring the previous one}}
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp for linear(j: val, val) // omp52-warning {{multiple 'linear modifier' found, ignoring the previous one}}
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp for linear(j: pval) // omp52-error{{use of undeclared identifier 'pval'}}
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp for linear(i: val, step(2 // omp52-error {{expected ')' or ',' after 'step expression'}} omp52-error {{expected ')' after 'step expression'}} omp52-error {{expected ')'}}  omp52-note {{to match this '('}}
  for (int k = 0; k < argc; ++k) ++k;
#endif
