...
int main(int argc, char **argv) {
  ...
  for (int k = 0; k < argc; ++k) ++k;
  #pragma omp taskloop simd linear(j: pval, step(1)) // omp52-error {{use of undeclared identifier 'pval'}} omp52-warning {{extra tokens at the end of 'simple step modifier' are ignored}}
  for (int k = 0; k < argc; ++k) ++k;
#endif
