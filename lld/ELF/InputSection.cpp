...
void InputSection::copyRelocations(uint8_t *buf,
  ...
  for (const Relocation &rel : rels) {
    ...
    } else if (config->emachine == EM_RISCV && !config->relocatable &&
               type == R_RISCV_ALIGN) {
      // Don't emit R_RISCV_ALIGN since it will be invalid once the padding nops
      // are removed.
      p->setType(R_RISCV_NONE, /*IsMips64EL*/ false);
      p->r_addend = 0;
    }
  }
}
