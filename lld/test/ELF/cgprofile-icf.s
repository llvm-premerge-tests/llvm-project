...
# RUN: ld.lld -e A %t --call-graph-ordering-file %t.call_graph --no-use-cdsort -o %t.out -icf=all
# RUN: llvm-readobj --symbols %t.out | FileCheck %s
# RUN: ld.lld -e A %t --call-graph-ordering-file %t.call_graph --no-use-cdsort -o %t2.out
