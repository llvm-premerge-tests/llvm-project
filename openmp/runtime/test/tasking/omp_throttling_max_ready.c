...
// KMP_TASK_MAXIMUM_READY=0      %libomp-run RUN: %libomp-compile && env
// OMP_NUM_THREADS=2 KMP_ENABLE_TASK_THROTTLING=1 KMP_TASK_MAXIMUM_READY=1
// %libomp-run RUN: %libomp-compile && env OMP_NUM_THREADS=2
// KMP_ENABLE_TASK_THROTTLING=1 KMP_TASK_MAXIMUM_READY=256    %libomp-run RUN:
// %libomp-compile && env OMP_NUM_THREADS=2 KMP_ENABLE_TASK_THROTTLING=1
// KMP_TASK_MAXIMUM_READY=65536  %libomp-run RUN: %libomp-compile && env
// OMP_NUM_THREADS=2 KMP_ENABLE_TASK_THROTTLING=1 KMP_TASK_MAXIMUM_READY=100000
// %libomp-run
...
int main(void) {
  ...
  char *max_tasks_ready_str = getenv("KMP_TASK_MAXIMUM_READY");
  int max_tasks_ready =
