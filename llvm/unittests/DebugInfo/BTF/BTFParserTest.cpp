...
TEST(BTFParserTest, enumRelocs) {
  ...
  SYMBOLIZE(SA({16, 1}),
            "<enumval_value> [1] '1:0' <unexpected enumval relo spec size>");
  SYMBOLIZE(SA({24, 1}),
            "<enumval_value> [2] '1' <unexpected type kind for enum relo: 1>");
...
TEST(BTFParserTest, miscBadRelos) {
  ...
  SYMBOLIZE(SA({0, 1}), "<relo kind #777> [0] '' <unknown relo kind: 777>");
...
TEST(BTFParserTest, parserResetReloAndTypes) {
  ...
  // First time: two types, two relos
  D.addType({D.addString("foo"), mkInfo(BTF::BTF_KIND_STRUCT), {0}});
...
TEST(BTFParserTest, selectiveLoad) {
  ...
  Opts = {};
  Opts.LoadLines = true;
  Error E1 = BTF1.parse(Obj1, Opts);
  ASSERT_FALSE(E1);

  Opts = {};
  Opts.LoadTypes = true;
  Error E2 = BTF2.parse(Obj1, Opts);
  ASSERT_FALSE(E2);

  Opts = {};
  Opts.LoadRelocs = true;
  Error E3 = BTF3.parse(Obj1, Opts);
  ASSERT_FALSE(E3);

  ASSERT_TRUE(BTF1.findLineInfo({0, 1}));
  ASSERT_FALSE(BTF2.findLineInfo({0, 1}));
  ASSERT_FALSE(BTF3.findLineInfo({0, 1}));

  ASSERT_FALSE(BTF1.findType(1));
  ASSERT_TRUE(BTF2.findType(1));
  ASSERT_FALSE(BTF3.findType(1));

  ASSERT_FALSE(BTF1.findFieldReloc({0, 1}));
  ASSERT_FALSE(BTF2.findFieldReloc({0, 1}));
  ASSERT_TRUE(BTF3.findFieldReloc({0, 1}));
}
