...
TEST_F(JITLinkRedirectionManagerTest, BasicRedirectionOperation) {
  ...
  EXPECT_THAT_ERROR(
      (*RM)->createRedirectableSymbols({{RedirectableSymbol, InitialTarget}},
  ...
  EXPECT_THAT_ERROR((*RM)->redirect({{RedirectableSymbol, MiddleTarget}}),
                    Succeeded());
  Result = RTPtr();
  EXPECT_EQ(Result, 13) << "Failed to call middle redirected target";

  EXPECT_THAT_ERROR((*RM)->redirect({{RedirectableSymbol, FinalTarget}}),
                    Succeeded());
  Result = RTPtr();
