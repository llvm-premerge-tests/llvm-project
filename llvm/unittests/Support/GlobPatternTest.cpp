...
TEST_F(GlobPatternTest, Escape) {
  ...
  Expected<GlobPattern> Pat3 = GlobPattern::create("\\{");
  EXPECT_TRUE((bool)Pat3);
  EXPECT_TRUE(Pat3->match("{"));
  EXPECT_FALSE(Pat3->match("\\{"));
  EXPECT_FALSE(Pat3->match(""));

  Expected<GlobPattern> Pat4 = GlobPattern::create("\\a");
...
TEST_F(GlobPatternTest, BraceExpansion) {
  ...
  auto Pat2 = GlobPattern::create(",}{foo,\\,\\},z*}");
  EXPECT_TRUE((bool)Pat2);
  ...
  auto Pat3 = GlobPattern::create("{a,ab}b");
  EXPECT_TRUE((bool)Pat3);
...
TEST_F(GlobPatternTest, BraceExpansionCharacterClass) {
  ...
  auto Pat = GlobPattern::create("_Z{N,NK,}S[tabsiod]*");
  EXPECT_TRUE((bool)Pat);
