...
TEST_F(InstrProfTest, test_irpgo_read_deprecated_names) {
  ...
  Writer.addRecord({getIRPGOFuncName(*FooF), 0x1234, {1}}, Err);
  // Write a record with a deprecated name
  Writer.addRecord({getPGOFuncName(*BarF), 0x5678, {2}}, Err);

  auto Profile = Writer.writeBuffer();
  readProfile(std::move(Profile));

  EXPECT_THAT_EXPECTED(Reader->getInstrProfRecord(getIRPGOFuncName(*FooF),
                                                  0x1234,
                                                  getPGOFuncName(*FooF)),
                       Succeeded());
  // Ensure we can still read this old record name
  EXPECT_THAT_EXPECTED(Reader->getInstrProfRecord(getIRPGOFuncName(*BarF),
                                                  0x5678,
                                                  getPGOFuncName(*BarF)),
                       Succeeded());
}
