...
Error BTFParser::parseTypesInfo(ParseContext &Ctx, uint64_t TypesInfoStart,
  ...
  using support::endian::system_endianness;
...
static ReloKindGroup reloKindGroup(const BTF::BPFFieldReloc *Reloc) {
...
// Such relocations are generated by BPF backend, specifically by code
...
void BTFParser::symbolize(const BTF::BPFFieldReloc *Reloc,
  ...
  if (Group == RKG_ENUMVAL) {
    ...
    if (RawSpec.size() != 1)
      return Fail("unexpected enumval relo spec size");
    ...
    if (auto *T = dyn_cast<BTF::EnumType>(Type)) {
      ...
      return Fail(
          format("unexpected type kind for enum relo: %d", Type->getKind()));
    }
  ...
  return Fail(format("unknown relo kind: %d", Reloc->RelocKind));
}
