...
public:
  SingleThreadedState(DWARFContext &DC, std::string &DWP) :
...
class ThreadSafeState : public SingleThreadedState {
  std::recursive_mutex Mutex;

public:
  ThreadSafeState(DWARFContext &DC, std::string &DWP) :
      SingleThreadedState(DC, DWP) {}

  DWARFUnitVector &getNormalUnits() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getNormalUnits();
  }
  DWARFUnitVector &getDWOUnits(bool Lazy) override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDWOUnits(Lazy);
  }
  const DWARFUnitIndex &getCUIndex() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getCUIndex();
  }
  const DWARFDebugAbbrev *getDebugAbbrevDWO() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugAbbrevDWO();
  }

  const DWARFUnitIndex &getTUIndex() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getTUIndex();
  }
  DWARFGdbIndex &getGdbIndex() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getGdbIndex();
  }
  const DWARFDebugAbbrev *getDebugAbbrev() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugAbbrev();
  }
  const DWARFDebugLoc *getDebugLoc() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugLoc();
  }
  const DWARFDebugAranges *getDebugAranges() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugAranges();
  }
  Expected<const DWARFDebugLine::LineTable *>
  getLineTableForUnit(DWARFUnit *U, function_ref<void(Error)> RecoverableErrorHandler) override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getLineTableForUnit(U, RecoverableErrorHandler);
  }
  void clearLineTableForUnit(DWARFUnit *U) override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::clearLineTableForUnit(U);
  }
  Expected<const DWARFDebugFrame *> getDebugFrame() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugFrame();
  }
  Expected<const DWARFDebugFrame *> getEHFrame() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getEHFrame();
  }
  const DWARFDebugMacro *getDebugMacinfo() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugMacinfo();
  }
  const DWARFDebugMacro *getDebugMacinfoDWO() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugMacinfoDWO();
  }
  const DWARFDebugMacro *getDebugMacro() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugMacro();
  }
  const DWARFDebugMacro *getDebugMacroDWO() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugMacroDWO();
  }
  const DWARFDebugNames &getDebugNames() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDebugNames();
  }
  const AppleAcceleratorTable &getAppleNames() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getAppleNames();
  }
  const AppleAcceleratorTable &getAppleTypes() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getAppleTypes();
  }
  const AppleAcceleratorTable &getAppleNamespaces() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getAppleNamespaces();
  }
  const AppleAcceleratorTable &getAppleObjC() override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getAppleObjC();
  }
  std::shared_ptr<DWARFContext>
  getDWOContext(StringRef AbsolutePath) override {
    std::unique_lock<std::recursive_mutex> LockGuard(Mutex);
    return SingleThreadedState::getDWOContext(AbsolutePath);
  }
};
    ...
    : DIContext(CK_DWARF),
      ...
      WarningHandler(WarningHandler), DObj(std::move(DObj)) {
        if (EnableMultiThreading)
          State.reset(new SingleThreadedState(*this, DWPName));
        else
...
DWARFContext::create(const object::ObjectFile &Obj, bool EnableMultiThreading,
  ...
  return std::make_unique<DWARFContext>(std::move(DObj), EnableMultiThreading,
                                        std::move(DWPName),
                                        RecoverableErrorHandler,
                                        WarningHandler);
}
...
DWARFContext::create(const StringMap<std::unique_ptr<MemoryBuffer>> &Sections,
  ...
  return std::make_unique<DWARFContext>(
      std::move(DObj), EnableMultiThreading, "", RecoverableErrorHandler,
      WarningHandler);
}
