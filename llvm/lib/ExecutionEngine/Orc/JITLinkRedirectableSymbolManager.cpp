...
Error JITLinkRedirectableSymbolManager::createRedirectableSymbols(
    const SymbolAddrMap &InitialDests, ResourceTrackerSP RT) {
  ...
  if (auto Err = JD.define(absoluteSymbols(NewSymbolDefs), RT))
    return Err;
...
Error JITLinkRedirectableSymbolManager::handleRemoveResources(JITDylib &JD,
                                                              ResourceKey K) {
  if (&JD != &this->JD)
    return Error::success();
  ...
  for (auto &Symbol : TrackedResources[K]) {
    if (!SymbolToStubs.count(Symbol))
      return make_error<StringError>(
          "Tried to remove non-existent redirectalbe symbol",
          inconvertibleErrorCode());
    AvailableStubs.push_back(SymbolToStubs.at(Symbol));
    SymbolToStubs.erase(Symbol);
  }
