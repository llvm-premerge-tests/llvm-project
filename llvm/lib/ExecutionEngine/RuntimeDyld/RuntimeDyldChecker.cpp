...
private:
  ...
  std::pair<EvalResult, StringRef> evalDecodeOperand(StringRef Expr) const {
    ...
    if (OpIdx >= Inst.getNumOperands()) {
      ...
      ErrMsgStream << "Invalid operand index '" << format("%i", OpIdx)
                   ...
                   << " operands.\nInstruction is:\n  ";
      auto InstPrinter = Checker.getInstPrinter(Symbol);
  ...
  bool decodeInst(StringRef Symbol, MCInst &Inst, uint64_t &Size,
    ...
    auto Dis = Checker.getDisassembler(Symbol);
...
RuntimeDyldCheckerImpl::getDisassembler(StringRef Symbol) const {
  Triple TheTriple = getTripleFromTargetFlag(getTargetFlag(Symbol));
  auto TripleName = TheTriple.str();
...
RuntimeDyldCheckerImpl::getInstPrinter(StringRef Symbol) const {
  Triple TheTriple = getTripleFromTargetFlag(getTargetFlag(Symbol));
  auto TripleName = TheTriple.str();
  std::string ErrorStr;
  const Target *TheTarget = TargetRegistry::lookupTarget(TripleName, ErrorStr);
...
RuntimeDyldCheckerImpl::getTripleFromTargetFlag(TargetFlagsType Flag) const {
  ...
  case Triple::ArchType::thumb:
    TheTriple.setArchName((Flag & 0x1
                               ...
                               : (Twine("arm") + TT.getArchName().substr(5)))
                              .str());
    break;
