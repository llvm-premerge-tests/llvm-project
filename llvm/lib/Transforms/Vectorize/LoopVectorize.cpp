    ...
    for (unsigned Part = 0; Part < State.UF; ++Part) {
      ...
      if (CreateGatherScatter) {
        ...
        if (Value *EVLPart = State.EVL ? State.get(State.EVL, Part) : nullptr) {
          ...
          // intrinsic.
          auto *Call = Builder.CreateIntrinsic(
              Type::getVoidTy(EVLPart->getContext()), Intrinsic::vp_scatter,
              {StoredVal, VectorGep, MaskValue(Part), EVLPart});
          Call->addParamAttr(
        ...
        if (Value *EVLPart = State.EVL ? State.get(State.EVL, Part) : nullptr) {
          ...
          VBuilder.setEVL(EVLPart).setMask(MaskValue(Part));
  ...
  for (unsigned Part = 0; Part < State.UF; ++Part) {
    ...
    if (CreateGatherScatter) {
      ...
      if (Value *EVLPart = State.EVL ? State.get(State.EVL, Part) : nullptr) {
        // If EVLPart is not null, we can vectorize using predicated
        // intrinsic.
        auto *Call = Builder.CreateIntrinsic(DataTy, Intrinsic::vp_gather,
                                             {VectorGep, MaskValue(Part), EVLPart},
