...
VPBasicBlock *PlainCFGBuilder::getOrCreateVPBB(BasicBlock *BB) {
  ...
  if (RegionExists) {
    assert((!isHeaderBB(BB, LoopOfBB) || LoopOfBB == TheLoop) &&
           "a region to use only exists for non-header blocks or the top-level "
           "loop");
    ...
    assert(isHeaderBB(BB, LoopOfBB) &&
           "new regions must be created for header blocks only");
    auto *RegionOfVPBB =
