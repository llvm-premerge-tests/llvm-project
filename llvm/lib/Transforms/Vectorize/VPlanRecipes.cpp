...
void VPExplicitVectorLengthExitPHIRecipe::execute(VPTransformState &State) {
  BasicBlock *VectorPH = State.CFG.getPreheaderBBFor(this);
  assert(State.UF == 1 && "Expected unroll factor 1 for VP vectorization.");
  Value *Start = State.get(getOperand(0), VPIteration(0, 0));
  PHINode *EntryPart = State.Builder.CreatePHI(Start->getType(), 2,
                                               "explicit.vector.length.exit");
  EntryPart->addIncoming(Start, VectorPH);
  EntryPart->setDebugLoc(getDebugLoc());
  State.set(this, EntryPart, 0);
}
...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
void VPExplicitVectorLengthExitPHIRecipe::print(
    raw_ostream &O, const Twine &Indent, VPSlotTracker &SlotTracker) const {
  O << Indent << "EXPLICIT-VECTOR-LANE-EXIT-PHI ";
