...
void VPlanTransforms::addActiveLaneMask(
  ...
  // active-lane-mask.
  replacePredicateWithIdiom(Plan, *LaneMask->getVPSingleValue());
}
...
// vector.body:
//   ...
//   %P = EXPLICIT-VECTOR-LANE-PHI [ %StartV, %vector.ph ], [ %NextEVL,
//   %vector.body ]
...
void VPlanTransforms::addVectorPredication(VPlan &Plan) {
  ...
  replacePredicateWithIdiom(Plan, *VPTrueMask);
  // Now create the ExplicitVectorLengthPhi recipe in the main loop.
  auto *EVLPhi = new VPExplicitVectorLengthPHIRecipe(StartV, DebugLoc());
