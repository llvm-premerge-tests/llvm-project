...
Instruction *InstCombinerImpl::foldICmpWithMinMaxImpl(
  ...
  if (ICmpInst::isSigned(Pred) && (MinMaxIntrinsic == Intrinsic::umin ||
    ...
    return nullptr;
  if (ICmpInst::isUnsigned(Pred) && (MinMaxIntrinsic == Intrinsic::smin ||
                                     MinMaxIntrinsic == Intrinsic::smax))
    return nullptr;
  ...
  case ICmpInst::ICMP_NE: {
    ...
    if ((Pred == ICmpInst::ICMP_EQ) == *CmpXZ) {
      ICmpInst::Predicate NewPred = ICmpInst::getNonStrictPredicate(
          MinMaxIntrinsic::getPredicate(MinMaxIntrinsic));
  ...
  case ICmpInst::ICMP_UGE: {
    ...
    bool IsSame = MinMaxIntrinsic::getPredicate(MinMaxIntrinsic) ==
                  ICmpInst::getStrictPredicate(Pred);
    if (*CmpXZ) {
      if (IsSame)
        //      Expr        Fact    Result
        // min(X, Y) < Z    X < Z   true
...
Instruction *InstCombinerImpl::foldICmpWithMinMax(ICmpInst &Cmp) {
  ...
  if (MinMaxIntrinsic *MinMax = dyn_cast<MinMaxIntrinsic>(Lhs)) {
    if (Instruction *Res =
            foldICmpWithMinMaxImpl(Cmp, MinMax->getLHS(), MinMax->getRHS(), Rhs,
                                   MinMax->getIntrinsicID(), Pred))
      return Res;
  }

  if (MinMaxIntrinsic *MinMax = dyn_cast<MinMaxIntrinsic>(Rhs)) {
    if (Instruction *Res = foldICmpWithMinMaxImpl(
            Cmp, MinMax->getLHS(), MinMax->getRHS(), Lhs,
            MinMax->getIntrinsicID(), ICmpInst::getSwappedPredicate(Pred)))
      return Res;
  }
