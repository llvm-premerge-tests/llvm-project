...
bool SkipUpdateDueToValidation(
  ...
  for (auto Type : Types) {
    if (auto *TypeID = dyn_cast<MDString>(Type->getOperand(1).get())) {
      ...
      if (IsVisibleToRegularObj(typeInfo))
        return true;
    }
  }
...
void getVisibleToRegularObjVtableGUIDs(
  ...
  for (const auto &typeID : Index.typeIdCompatibleVtableMap()) {
    ...
    // non-externally visible types which cannot interact with
    // external native files. See CodeGenModule::CreateMetadataIdentifierImpl.
    if (!StringRef(typeID.first).starts_with("_ZTS"))
      continue;

    // TypeID is keyed off the type name symbol (_ZTS). However, the native
    // object may not contain this symbol if it does not contain a key function
    // for the base type and thus only contains a reference to the type info
    // (_ZTI). To catch this case we query using the type info symbol
    // corresponding to the TypeID.
    std::string typeInfo = "_ZTI" + typeID.first.substr(strlen("_ZTS"));
