...
struct AA::PointerInfo::State : public AbstractState {
  ...
  OffsetBinsTy getOffsetBins() const { return OffsetBins; }
...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    const auto *OffsetBins = PI->getOffsetBinsAddr();

    int64_t BinSize = OffsetBins->size();

    // TODO: Handle for more than one Bin
    if (BinSize > 1)
      return indicatePessimisticFixpoint();

    const auto &It = OffsetBins->begin();
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto *IntValue = llvm::ConstantInt::get(
        AI->getContext(), llvm::APInt(32, NumBytesToAllocate));
