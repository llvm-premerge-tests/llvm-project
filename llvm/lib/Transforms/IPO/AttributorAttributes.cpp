...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto *AI = dyn_cast<AllocaInst>(getIRPosition().getCtxI());
    ...
    int64_t BinSize = AAPointerInfoState.numOffsetBins();

    // TODO: Handle for more than one Bin
    if (BinSize > 1)
      return indicatePessimisticFixpoint();

    const auto &It = AAPointerInfoState.begin();
    ...
    uint64_t OffsetEnd = It->getFirst().Offset + It->getFirst().Size;
    const DataLayout &DL = A.getDataLayout();
    const auto &AllocationSize = DL.getTypeAllocSize(AI->getAllocatedType());

    // If struct has only one element and does not need additional padding
    // If it needs additional padding then we can potentially reduce its size
    // and should not call PessimisticFixPoint.
    // TODO: Confirm if isFirstClassType() is correct condition for the
    // Contained Type. FirstClassTypes includes the types below
    // (https://nondot.org/sabre/LLVMNotes/FirstClassAggregates.txt)
    // 1. Integer
    // 2. Floating point
    // 3. Pointer
    // 4. Vector  <- Except Vector
    // 5. Opaque (which is assumed to eventually resolve to 1-4)

    if (AI->getAllocatedType()->getNumContainedTypes() == 1 &&
        AI->getAllocatedType()->isStructTy() &&
        AI->getAllocatedType()->getContainedType(0)->isFirstClassType() &&
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    Type *CharType = Type::getInt8Ty(AI->getContext());
    Type *CharArrayOfAllocationSize =
        llvm::ArrayType::get(CharType, NumBytesToAllocate);
    AI->setAllocatedType(CharArrayOfAllocationSize);
