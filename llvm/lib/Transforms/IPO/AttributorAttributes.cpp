...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    // At the IR level, ptr doesn't have an allocated size.
    // The size returned is 0 so we just indicate a fixpoint.
    // We do not change the allocation size of the pointer.
    if (AI->getAllocatedType()->isPointerTy())
      return indicatePessimisticFixpoint();

    // If struct has only one element which is a pointer element
    // Then we can just leave it's allocated size as it is.
    if (AI->getAllocatedType()->getNumContainedTypes() == 1 &&
        AI->getAllocatedType()->getContainedType(0)->isPointerTy())
      return indicatePessimisticFixpoint();
