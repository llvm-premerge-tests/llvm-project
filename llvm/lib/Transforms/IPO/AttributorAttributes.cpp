...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    const AAPointerInfo *PI = nullptr;
    // Only create AAPointer Info for Alloca and Malloc Instructions
    switch (I->getOpcode()) {
    case Instruction::Alloca: {
      PI = A.getOrCreateAAFor<AAPointerInfo>(IRP, *this, DepClassTy::REQUIRED);
      break;
    }
    case Instruction::Call: {
      CallInst *Call = dyn_cast<CallInst>(getIRPosition().getCtxI());
    ...
    case Instruction::Call: {
      ...
      // TODO: Handle if this is a variable.
      // If it can be constant folded at compile time then we can reduce the
      // allocation size.
      Value *ValueOperand = Call->getArgOperand(0);
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto *IntValue = llvm::ConstantInt::get(
    ...
    case Instruction::Alloca: {
      ...
      AllocaInst *NewAllocaInst =
          new AllocaInst(CharType, AI->getAddressSpace(), IntValue,
    ...
    case Instruction::Call: {
      ...
      Instruction *NewMallocInst =
          CallInst::CreateMalloc(Call->getNextNode(), IntPtrTy, CharType,
                                 IntValue, nullptr, nullptr, Call->getName());

      if (A.changeAfterManifest(IRPosition::callsite_returned(*Call),
                                *NewMallocInst)) {
        A.deleteAfterManifest(*Call);
        return ChangeStatus::CHANGED;
      }
