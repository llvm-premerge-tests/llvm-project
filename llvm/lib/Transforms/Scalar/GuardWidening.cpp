...
class GuardWideningImpl {
  ...
  bool mergeChecks(SmallVectorImpl<Value *> &ChecksToHoist,
                   SmallVectorImpl<Value *> &ChecksToWiden,
  ...
  bool isWideningCondProfitable(SmallVectorImpl<Value *> &ChecksToHoist,
                                SmallVectorImpl<Value *> &ChecksToWiden) {
    Value *ResultUnused;
    return mergeChecks(ChecksToHoist, ChecksToWiden,
...
bool GuardWideningImpl::eliminateInstrViaWidening(
    ...
    const DenseMap<BasicBlock *, SmallVector<Instruction *, 8>>
        &GuardsInBlock) {
  SmallVector<Value *, 4> ChecksToHoist;
  ...
  for (unsigned i = 0, e = DFSI.getPathLength(); i != e; ++i) {
    ...
    for (auto *Candidate : make_range(I, E)) {
      ...
      if (!WideningPoint)
        continue;
      SmallVector<Value *, 4> CandidateChecks;
...
bool GuardWideningImpl::mergeChecks(SmallVectorImpl<Value *> &ChecksToHoist,
                                    SmallVectorImpl<Value *> &ChecksToWiden,
                                    Instruction *InsertPt, Value *&Result) {
