...
static bool
CanMergeEmptyBBToSucc(BasicBlock *BB, BasicBlock *Succ,
                      const SmallPtrSetImpl<BasicBlock *> &BBPreds) {
  // Check to see if merging these blocks would cause conflicts for any of the
  // phi nodes in BB or Succ. If not, we can safely merge.
  if (!CanPropagatePredecessorsForPHIs(BB, Succ, BBPreds))
    return false;

  // Check to see if merging these blocks would cause conflicts for any of
...
CanRedirectPredsOfEmptyBBToSucc(BasicBlock *BB, BasicBlock *Succ,
  ...
  // Gather common predecessors of both BB and Succ
...
bool llvm::TryToSimplifyUncondBranchFromEmptyBlock(BasicBlock *BB,
  ...
  bool BBKillable = CanMergeEmptyBBToSucc(BB, Succ, BBPreds);
