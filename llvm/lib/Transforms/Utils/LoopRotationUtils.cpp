...
static void updateBranchWeights(BranchInst &PreHeaderBI, BranchInst &LoopBI,
  ...
  if (!ConditionalPreHeader) {
    ...
    if (OrigLoopBackedgeWeight > OrigLoopExitWeight) {
      ...
      ExitWeight0 = 1;
      // Scale up counts if necessary so we can have the
      // `ExitWeight0`:`ExitWeight1` (aka `x0`:`x1` ratio`) ratio match be an
      // unlikely branch.
      const uint32_t LikelyWeight = 127;
      while (OrigLoopExitWeight <= LikelyWeight + 1) {
        // ... but don't overflow.
        uint32_t const high_bit = uint32_t{1} << (sizeof(uint32_t) * 8 - 1);
        if (OrigLoopBackedgeWeight & high_bit != 0 ||
            OrigLoopExitWeight & high_bit != 0)
          break;
