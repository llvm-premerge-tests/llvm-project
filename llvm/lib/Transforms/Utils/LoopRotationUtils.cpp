...
static void updateBranchWeights(BranchInst &PreHeaderBI, BranchInst &LoopBI,
  ...
  if (!ConditionalPreHeader) {
    ...
    if (OrigLoopBackedgeWeight > OrigLoopExitWeight) {
      ...
      ExitCount0 = OrigLoopExitWeight - OrigLoopBackedgeWeight;
    }
  }
  uint32_t ExitCount1 = OrigLoopExitWeight - ExitCount0;        // aka x1
  uint32_t EnterCount = ExitCount1;                             // aka y0
  uint32_t LoopBackCount = OrigLoopBackedgeWeight - EnterCount; // aka y1
