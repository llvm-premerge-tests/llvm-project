...
bool SimplifyCFGOpt::hoistCommonCodeFromSuccessors(BasicBlock *BB,
  ...
  // This flag affects all successors.
  unsigned SkipFlagsBB = 0;
  SmallVector<SuccIterPair, 8> SuccIters;
  for (auto *Succ : successors(BB)) {
    BasicBlock::iterator SuccItr = Succ->begin();
    if (isa<PHINode>(*SuccItr))
      return false;
    SuccIters.push_back(SuccIterPair(SuccItr, 0));
  }
  ...
  if (EqTermsOnly) {
    ...
    for (auto &SuccIter : SuccIters) {
      auto *INonDbg = &*skipDebugIntrinsics(SuccIter.first);
  ...
  for (;;) {
    // If we find an unreachable instruction, we can still hoist the instruction
    // that doesn't has any side effect.
    if (SuccIters.size() > 2)
      for (auto *Iter = SuccIters.begin(); Iter != SuccIters.end();) {
        if (isa<UnreachableInst>(*Iter->first)) {
          SkipFlagsBB |= SkipSideEffect;
    ...
    auto *SuccIterBegin = SuccIters.begin();
    auto &BB1Itr = *SuccIterBegin++;

    Instruction *I1 = &*BB1Itr.first;
    auto *BB1 = I1->getParent();

    // Skip debug info if it is not identical.
    for (auto &SuccIter : iterator_range(SuccIterBegin, SuccIters.end())) {
      Instruction *I2 = &*SuccIter.first;
    ...
    if (HasTerminator) {
      ...
      if (NumSkipped || SuccSize != SuccIters.size() || !AllInstsAreIdentical)
        return Changed;
    ...
    if (AllInstsAreIdentical) {
      unsigned SkipFlagsBB1 = BB1Itr.second | SkipFlagsBB;
      AllInstsAreIdentical = isSafeToHoistInstr(I1, SkipFlagsBB1);
      for (auto &SuccIter : iterator_range(SuccIterBegin, SuccIters.end())) {
        Instruction *I2 = &*SuccIter.first;
        unsigned SkipFlagsBB2 = SuccIter.second | SkipFlagsBB;
        // Even if the instructions are identical, it may not be safe to
        // hoist them if we have skipped over instructions with side effects
        // or their operands weren't hoisted.
        if (!isSafeToHoistInstr(I2, SkipFlagsBB2) ||
    ...
    if (AllInstsAreIdentical) {
      ...
      if (isa<DbgInfoIntrinsic>(I1)) {
        ...
        for (auto &SuccIter : iterator_range(SuccIterBegin, SuccIters.end())) {
          auto *I2 = &*SuccIter.first++;
          assert(isa<DbgInfoIntrinsic>(I2));
          BB->splice(TI->getIterator(), I2->getParent(), I2->getIterator());
        }
      } else {
      ...
      NumHoistCommonInstrs += SuccIters.size();
    } else {
      ...
      // across them.
      for (auto &SuccIter : SuccIters) {
        Instruction *I = &*SuccIter.first++;
        SuccIter.second |= skippedInstrFlags(I);
      }
      ++NumSkipped;
    }
  }
