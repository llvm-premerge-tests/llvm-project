...
bool SimplifyCFGOpt::hoistCommonCodeFromSuccessors(BasicBlock *BB,
  ...
  if (SuccIters.size() > 2) {
    auto Iter = remove_if(SuccIters, [](const auto &Pair) {
      return isa<UnreachableInst>(Pair.first);
    });
    if (Iter != SuccIters.end()) {
      SuccIters.erase(Iter, SuccIters.end());
    }
    if (SuccIters.size() < 2)
      return Changed;
  }
  ...
  for (;;) {
    ...
    if (HasTerminator) {
      ...
      for (auto &SuccIter : iterator_range(SuccIterBegin, SuccIters.end()))
        Insts.push_back(&*SuccIter.first);
    ...
    if (AllInstsAreIdentical) {
      ...
      AllInstsAreIdentical =
          ...
          llvm::all_of(iterator_range(SuccIterBegin, SuccIters.end()),
                       [=](const auto &Pair) {
                         Instruction *I2 = &*Pair.first;
