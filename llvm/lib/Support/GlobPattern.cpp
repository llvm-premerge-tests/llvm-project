...
parseBraceExpansions(StringRef S, StringRef Original) {
    ...
    if (S[I] == '[') {
      ...
      if (CurrentBE)
        return make_error<StringError>(
            "nested brace expansions are not supported: " + Original,
            errc::invalid_argument);
  ...
  if (CurrentBE)
    return make_error<StringError>("incomplete brace expansion: " + Original,
                                   errc::invalid_argument);
...
Expected<GlobPattern> GlobPattern::create(StringRef S) {
  ...
  if (auto Err = parseBraceExpansions(S, Original).moveInto(SubPats))
    return Err;
  for (StringRef SubPat : SubPats) {
    auto SubGlobOrErr = SubGlobPattern::create(SubPat, Original);
    if (!SubGlobOrErr)
...
GlobPattern::SubGlobPattern::create(StringRef S, StringRef Original) {
  SubGlobPattern Pat;
