...
static bool
PerformCloningAndPathLayouts(MachineFunction &MF,
  ...
  for (const BBProfile<ProfileBBID> &P : RawProfile.RawBBProfiles) {
    unsigned FinalBBID = P.BasicBlockID.CloneID == 0
                             ? P.BasicBlockID.BBID
                             : ProfileBBIDToBBID.at(P.BasicBlockID);
    Out.push_back(
        BBProfile<unsigned>{FinalBBID, P.ClusterID, P.PositionInCluster});
  }
...
bool BasicBlockSections::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (const auto &P : FunctionProfile)
    BBProfilesByBBID[P.BasicBlockID] = P;
