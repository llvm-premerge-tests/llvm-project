...
static bool PerformCloningAndPathLayouts(MachineFunction &MF,
  ...
  for (auto &ClonePath : RawProfile.ClonePaths) {
    ...
    for (unsigned I = 0; I < ClonePath.size(); ++I) {
      ...
      if (PrevBB && !PrevBB->isSuccessor(PathBB)) {
        WithColor::warning() << "Block " << BlockID << " is not a successor of "
      ...
      if (I != ClonePath.size() - 1 && !PathBB->empty() &&
        ...
        WithColor::warning()
            << "Non-final block " << BlockID << " has indirect branch in "
            << MF.getName() << "\n";
        return false;
      }
      PrevBB = PathBB;
  ...
  for (auto &ClonePath : RawProfile.ClonePaths) {
    ...
    for (auto BlockID : ClonePath) {
      ...
      ProfileIDToBlockID[ProfileBBID({BlockID, CloneNumber})] =
  ...
  for (const RawBBProfile &P : RawProfile.RawBBProfiles) {
    unsigned FinalID =
        P.BBID.CloneID == 0 ? P.BBID.BlockID : ProfileIDToBlockID.at(P.BBID);
    Out.push_back(BBClusterInfo{FinalID, P.ClusterID, P.PositionInCluster});
  }
