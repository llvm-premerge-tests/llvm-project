...
bool hasUnconditionalBranch(const MachineBasicBlock &MBB) {
  if (MBB.terminators().empty()) return false;
...
updateBranches(MachineFunction &MF,
  ...
  for (auto &MBB : MF) {
    ...
    // branch to that block if either these two conditions hold and the block doesn't 
    // currently have an unconditional branch.
    ...
    //        order.
    if (FTMBB && (MBB.isEndSection() || &*NextMBBI != FTMBB) && !hasUnconditionalBranch(MBB))
