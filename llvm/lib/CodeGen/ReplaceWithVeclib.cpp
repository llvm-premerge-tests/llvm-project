...
static bool replaceWithTLIFunction(Instruction &I, const StringRef TLIName,
                                   bool Masked = false) {
  ...
  // otherwise insert it.
  auto *TLIFunc = Mod->getFunction(TLIName);
  std::string OldName;
  ...
  if (!TLIFunc) {
    if (!CInst) {
      ...
      OldName = I.getOpcodeName();
      auto *RetTy = I.getType();
      NumElements = (dyn_cast<ScalableVectorType>(RetTy))->getElementCount();
      ...
      TLIFunc = Function::Create(FunctionType::get(RetTy, Tys, false),
                                 Function::ExternalLinkage, TLIName, *Mod);
    } else {
      // Intrinsics handling.
      auto *OldFunc = CInst->getCalledFunction();
      auto *OldFuncTy = OldFunc->getFunctionType();
      OldName = OldFunc->getName();
      TLIFunc =
          Function::Create(OldFuncTy, Function::ExternalLinkage, TLIName, *Mod);
    ...
    appendToCompilerUsed(*Mod, {TLIFunc});
  ...
  if (!CInst) {
    ...
    CInst->getOperandBundlesAsDefs(OpBundles);
