...
// Reads the version 1 basic block sections profile. Profile for each function is encoded as follows:
// m <module_name>
// f <function_name_1> <function_name_2> ...
// c <bb_id_1> <bb_id_2> <bb_id_3>
// c <bb_id_4> <bb_id_5>
// ...
// Module name specifier (starging with 'm') is optional and allows distinguishing profile for internal-linkage functions with the same name.
...
// Basic block clusters are specified by 'c' and specify the cluster of basic blocks, and the internal order in which they must be placed in the same section.
Error BasicBlockSectionsProfileReader::ReadV1Profile() {
  ...
  // Debug-info-based module filename for the current function. Empty string means no filename.
  ...
  for (; !LineIt.is_at_eof(); ++LineIt) {
    ...
    case 'm':
      if (Values.size() != 1) {
        return createProfileParseError(Twine("invalid module name value: ") + S);
      }
    ...
    case 'f': {
      ...
      CurrentCluster = 0;
      FuncBBIDs.clear();
      // We won't need DIFilename anymore. Clean it up to avoid its application on the next function.
      DIFilename = "";
      continue;
    }
    case 'c':
      ...
      continue;
    default:
      return createProfileParseError(Twine("invalid specifer: ") +
                                     Twine(Specifier));
    }
  }
