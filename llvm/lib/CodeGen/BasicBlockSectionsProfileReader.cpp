...
Error BasicBlockSectionsProfileReader::ReadProfile() {
  ...
  for (; !LineIt.is_at_eof(); ++LineIt) {
    ...
    for (int I = 0; I < ClonePath.size(); ++I) {
      auto BBIDStr = ClonePath[I];
      unsigned long long BBID = 0;
      if (getAsUnsignedInteger(BBIDStr, 10, BBID))
        return createProfileParseError(Twine("unsigned integer expected: '") +
                                       BBIDStr + "'");
      if (I != 0 && !BBsInPath.insert(BBID))
        return createProfileParseError(Twine("duplicate cloned block in path: '" + BBID + ''");
