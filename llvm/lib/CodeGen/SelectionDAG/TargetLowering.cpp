...
bool TargetLowering::SimplifyDemandedBits(
  ...
  case ISD::SHL: {
    ...
    if (const APInt *SA =
      ...
      if ((BitWidth % 2) == 0 && !VT.isVector() && ShAmt < (BitWidth / 2)) {
        APInt HiBits = APInt::getHighBitsSet(BitWidth, ShAmt + (BitWidth / 2));
        ...
        if (isNarrowingProfitable(VT, HalfVT) &&
          ...
          SDNodeFlags Flags;
          Flags.setNoSignedWrap(NumSignBits > ShAmt);
          Flags.setNoUnsignedWrap(NewKnownBits.countMinLeadingZeros() >= ShAmt);
          SDValue NewShift =
              TLO.DAG.getNode(ISD::SHL, dl, HalfVT, NewOp, NewShiftAmt, Flags);
          return TLO.CombineTo(
