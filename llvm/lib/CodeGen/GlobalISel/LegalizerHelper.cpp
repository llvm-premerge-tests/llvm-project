...
LegalizerHelper::LegalizeResult LegalizerHelper::lowerEXT(MachineInstr &MI) {
  ...
  if (SrcTyScalarSize * 2 < DstTyScalarSize) {
    ...
    // zext x -> zext(merge(zext(unmerge), zext(unmerge)))
      ...
      auto UnmergeSrc = MIRBuilder.buildUnmerge(EltTy, MidRegister);
      ...
      MIRBuilder.buildInstr(MI.getOpcode(), {ZExtRes1}, {UnmergeSrc.getReg(0)});
      MIRBuilder.buildInstr(MI.getOpcode(), {ZExtRes2}, {UnmergeSrc.getReg(1)});
