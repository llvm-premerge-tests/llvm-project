...
bool GCEmptyBasicBlocks::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineFunction::iterator MBB = MF.begin(), E = MF.end(), NextMBB;
       MBB != E; MBB = NextMBB) {
    ...
    if (MBB->isEHPad() || MBB->isMachineBlockAddressTaken())
      continue;
    // Capture blocks without real code.
    ...
    // If this is the last block of the function, we cannot remove it if it has
