...
uint16_t getAuxMaxAlignment(uint16_t AuxHeaderSize, AuxiliaryHeader *AuxHeader,
  ...
  // MaxAlignOfText field, it is not a loadable shared object file,
  // so align at the minimum value. The 'ModuleType' member is located
  // right after 'MaxAlignOfData' in the AuxiliaryHeader.
  if (AuxHeaderSize < offsetof(AuxiliaryHeader, ModuleType))
    return MinBigArchiveMemDataAlign;

  // If the XCOFF object file does not have a loader section, it
  // is not loadable, so align at the minimum value.
  ...
  // both fields. If the desired alignment is > PAGESIZE, 32-bit members are
  // aligned on a word boundary, while 64-bit members are aligned on a
  // PAGESIZE(2^12=4096) boundary.
...
// 32-bit members. This ensures that when these members are loaded they
  ...
  for (auto M = NewMembers.begin(); M < NewMembers.end(); ++M) {
    ...
    if (isAIXBigArchive(Kind)) {
      ...
      if ((M + 1) != NewMembers.end()) {
        Expected<std::unique_ptr<SymbolicFile>> SymFileOrErr =
            getSymbolicFile((M + 1)->Buf->getMemBufferRef(), Context);
        if (!SymFileOrErr)
...
static Error writeArchiveToStream(raw_ostream &Out,
  ...
  if (WriteSymtab != SymtabWritingMode::NoSymtab && !is64BitKind(Kind)) {
  ...
  if (!isAIXBigArchive(Kind)) {
    ...
    if (WriteSymtab != SymtabWritingMode::NoSymtab && NumSyms)
    ...
    uint64_t GlobalSymbolOffset =
        (WriteSymtab != SymtabWritingMode::NoSymtab &&
         (WriteSymtab != SymtabWritingMode::Bit64Only) && NumSyms32 > 0)
    ...
    if (WriteSymtab != SymtabWritingMode::NoSymtab &&
        (WriteSymtab != SymtabWritingMode::Bit32Only) && NumSyms64 > 0) {
