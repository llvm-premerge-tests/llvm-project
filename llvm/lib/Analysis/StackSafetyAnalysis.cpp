  ...
  while (!WorkList.empty()) {
    ...
    for (const Use &UI : V->uses()) {
      ...
      auto RecordWrite = [&](unsigned ValOp) {
        ...
        auto TypeSize = DL.getTypeStoreSize(I->getOperand(ValOp)->getType());
      ...
      case Instruction::Store:
        RecordWrite(StoreInst::getValueOperandIndex());
        break;
      case Instruction::AtomicCmpXchg:
        RecordWrite(AtomicCmpXchgInst::getNewValOperandIndex());
        break;
      case Instruction::AtomicRMW:
        RecordWrite(AtomicRMWInst::getValOperandIndex());
        break;
