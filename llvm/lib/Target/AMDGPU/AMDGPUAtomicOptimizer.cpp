...
Value *AMDGPUAtomicOptimizerImpl::buildReduction(
  ...
  V = buildNonAtomicBinOp(B, Op, B.CreateBitCast(V, AtomicTy), Permlanex16Call);
  if (ST->isWave32()) {
    return V;
  }
...
AMDGPUAtomicOptimizerImpl::buildScan(IRBuilder<> &B, AtomicRMWInst::BinOp Op,
  ...
  if (ST->hasDPPBroadcasts()) {
    ...
    PermX = B.CreateBitCast(PermX, Int32Ty);
    Value *UpdateDPPCall = B.CreateCall(
...
std::pair<Value *, Value *> AMDGPUAtomicOptimizerImpl::buildScanIteratively(
  ...
  if (NeedResult) {
    OldValue = B.CreateIntrinsic(
        ...
        {isAtomicFloatingPointTy ? B.CreateBitCast(Accumulator, Int32Ty)
                                 : Accumulator,
         LaneIdxInt,
         isAtomicFloatingPointTy ? B.CreateBitCast(OldValuePhi, Int32Ty)
                                 : OldValuePhi});
...
void AMDGPUAtomicOptimizerImpl::optimizeAtomic(Instruction &I,
  ...
  Value *Identity = getIdentityValueForAtomicOp(C, Ty, Op, TyBitWidth);
  ...
  if (ValDivergent) {
    ...
    if (ScanImpl == ScanOptions::DPP) {
      ...
      if (!NeedResult && ST->hasPermLaneX16()) {
        ...
        NewV = B.CreateBitCast(NewV, Int32Ty);
