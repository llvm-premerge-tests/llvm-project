...
public:
  ...
  unsigned allocPreloadSGPRs(unsigned AllocSize, uint64_t ArgOffset,
...
static bool lowerKernelArguments(Function &F, const TargetMachine &TM) {
  ...
  for (Argument &Arg : F.args()) {
    ...
    if (Arg.hasInRegAttr() && InPreloadSequence && ST.hasKernargPreload() &&
      ...
      if (unsigned PreloadSGPRs = PreloadInfo.allocPreloadSGPRs(
              AllocSize, EltOffset, LastExplicitArgOffset)) {
        // Preload this argument.
        HasPreloadArgs = true;
        MDBuilder MDB(Ctx);
        auto *MDIndex =
            MDB.createConstant(ConstantInt::get(Builder.getInt32Ty(), Idx));
