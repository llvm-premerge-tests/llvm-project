...
static std::optional<bool> checkSignedness(const SDValue &N,
  ...
  // In cases where we are accumulating into an i8 (for v_dot4), the ByteProvider will
  // not have signedness info since the MSBs are dont-cares. In this case, we simply use 
  // the TopLevelSignedness of the instruction.
