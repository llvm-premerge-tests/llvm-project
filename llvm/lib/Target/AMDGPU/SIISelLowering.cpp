...
static unsigned getMulOpcode(const SDValue Op) {
  ...
  if (Opcode == ISD::INTRINSIC_WO_CHAIN) {
    ...
    if (IID == Intrinsic::amdgcn_mul_u24)
      return AMDGPUISD::MUL_U24;
    return ISD::DELETED_NODE;
  }

  if (Opcode == ISD::MUL || Opcode == AMDGPUISD::MUL_U24 ||
...
SDValue SITargetLowering::performAddCombine(SDNode *N,
  ...
  if ((getMulOpcode(LHS) != ISD::DELETED_NODE ||
       getMulOpcode(RHS) != ISD::DELETED_NODE) &&
      Subtarget->hasDot7Insts() &&
      (Subtarget->hasDot1Insts() || Subtarget->hasDot8Insts())) {
    SDValue TempNode(N, 0);
    auto MulIdx = getMulOpcode(LHS) != ISD::DELETED_NODE ? 0 : 1;

    auto MulOpcode = getMulOpcode(TempNode.getOperand(MulIdx));
    bool IsSigned =
    ...
    int ChainLength = 0;
    for (int I = 0; I < 4; I++) {
      auto MulIdx = getMulOpcode(LHS) != ISD::DELETED_NODE   ? 0
                    : getMulOpcode(RHS) != ISD::DELETED_NODE ? 1
                                                             : -1;
      if (MulIdx == -1)
        break;
      auto IterIsSigned =
      ...
      unsigned Src0Op =
          TempNode->getOperand(MulIdx).getOpcode() == ISD::INTRINSIC_WO_CHAIN
              ? 1
              : 0;
      unsigned Src1Op =
          TempNode->getOperand(MulIdx).getOpcode() == ISD::INTRINSIC_WO_CHAIN
              ...
              : 1;
      auto Src0 =
          handleMulOperand(TempNode->getOperand(MulIdx)->getOperand(Src0Op));
      if (!Src0)
        break;
      auto Src1 =
          handleMulOperand(TempNode->getOperand(MulIdx)->getOperand(Src1Op));
      if (!Src1)
        break;
      ...
      // add (mul24, mul24)
      if (I == 2 &&
          (getMulOpcode(TempNode->getOperand(AddIdx)) != ISD::DELETED_NODE)) {
        Src2s.push_back(TempNode->getOperand(AddIdx));
        auto Src0 =
            handleMulOperand(TempNode->getOperand(AddIdx)->getOperand(Src0Op));
        if (!Src0)
          break;
        auto Src1 =
            handleMulOperand(TempNode->getOperand(AddIdx)->getOperand(Src1Op));
        if (!Src1)
          break;
