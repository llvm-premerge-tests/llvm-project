...
void llvm::GCNBalancedSchedStrategy::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
    ...
    if (Instr) {
      ...
      unsigned Latency = SM.computeInstrLatency(Instr);
      unsigned Distance = SU->TopReadyCycle + Latency;
      if (Distance > TheLongestTopDownSuccEdge) {
        TheLongestTopDownSuccEdge = Distance;
        TheFurthestPred = SU;
      }
    }
  } else {
...
bool llvm::GCNBalancedSchedStrategy::computeScheduleMetric(
  ...
  bool Result = false;
  unsigned TopToBotDistance = 0;
  if (TheLongestTopDownSuccEdge > Top.getCurrCycle()) {
    unsigned TopDistance = TheLongestTopDownSuccEdge - Top.getCurrCycle();
    unsigned TheHighestInBot = 0;
    for (auto &Succ : TheFurthestPred->Succs) {
      if (Succ.isAssignedRegDep()) {
        unsigned BotReadyCycle = Succ.getSUnit()->BotReadyCycle;
        if (BotReadyCycle > TheHighestInBot)
          TheHighestInBot = BotReadyCycle;
      }
    }
    assert(Bot.getCurrCycle() > TheHighestInBot);
    unsigned BotDistance = Bot.getCurrCycle() - TheHighestInBot;
    TopToBotDistance = TopDistance - BotDistance;
  }
  unsigned ScheduleLength =
      Top.getCurrCycle() + Bot.getCurrCycle() + TopToBotDistance;
  if (ScheduleLength) {
#ifndef NDEBUG
