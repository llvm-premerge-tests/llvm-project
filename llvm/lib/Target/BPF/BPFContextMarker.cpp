...
//===----------------------------------------------------------------------===//
//
// TLDR: replaces llvm.context.marker.bpf + GEP + load / store
//           with llvm.bpf.getelementptr.and.load / store
//
// This file implements BPFRewriteContextAccessPass transformation.
...
//   llvm.context.marker.bpf are inserted.
// - BPFRewriteContextAccessPass is executed as early as possible
//   with AllowPatial set to true, this handles marked GEP chains
//   with constant offsets.
// - BPFRewriteContextAccessPass is executed at ScalarOptimizerLateEPCallback
...
//
// struct context { int x[4]; } __attribute__((btf_decl_tag("ctx")));
//
//   struct context *ctx = ...;
...
//     foo(ctx->x[i]);
//
// The early BPFRewriteContextAccessPass run is necessary to allow
...
// BPFRewriteContextAccessPass is done at (3) but not done at (1)
...
static bool isContextMarkerCall(Value *I) {
  return isIntrinsicCall(I, Intrinsic::context_marker_bpf);
}
...
BPFRewriteContextAccessPass::reconstructLoad(CallInst *Call) {
...
BPFRewriteContextAccessPass::reconstructStore(CallInst *Call) {
  ...
  auto Msg = DiagnosticInfoUnsupported(
      *Insn->getFunction(),
      Twine("Non-constant offset in access to a field of a type marked "
            "with btf_decl_tag(\"ctx\") might be rejected by BPF verifier")
...
static void rewriteUses(Instruction *Insn,
  ...
  for (User *U : Insn->users()) {
    ...
    if (UI && (isPointerOperand(Insn, UI) || isContextMarkerCall(UI) ||
               isInlineableCall(UI)))
      rewriteAccessChain(UI, GEPs, Visited, AllowPatial, StillUsed);
    else
      LLVM_DEBUG({
        llvm::dbgs() << "unsupported usage in BPFContextMarker:\n";
        llvm::dbgs() << "  Insn: " << *Insn << "\n";
        llvm::dbgs() << "  User: " << *U << "\n";
      });
  }
...
// llvm.context.marker.bpf calls. Recursion stops at any other
...
// If nested calls to `llvm.context.marker.bpf` are encountered these
...
// - StillUsed - set to true if one of the GEP chains could not be folded,
//   makes sense when AllowPartial is false, means that root marker is still
//   in use and should remain until the next run of this pass.
static void rewriteAccessChain(Instruction *Insn,
                               SmallVector<GetElementPtrInst *> &GEPs,
                               SmallVector<Instruction *> &Visited,
                               bool AllowPatial, bool &StillUsed) {
  ...
  auto TryToReplace = [&](Instruction *LoadOrStore) {
    // Do nothing for (marker (load/store ..)) or for GEPs with zero indices.
    // Such constructs lead to zero offset and are simplified by other passes.
  ...
  if (isa<LoadInst>(Insn) || isa<StoreInst>(Insn)) {
    ...
    auto [GEP, Load] =
        BPFRewriteContextAccessPass::reconstructLoad(cast<CallInst>(Insn));
    GEPs.push_back(GEP);
    TryToReplace(Load);
    GEPs.pop_back();
    delete Load;
    ...
    auto [GEP, Store] =
        BPFRewriteContextAccessPass::reconstructStore(cast<CallInst>(Insn));
    GEPs.push_back(GEP);
    TryToReplace(Store);
    GEPs.pop_back();
    delete Store;
    ...
    // Preserve marker for parameters of functions that might be inlined.
    // These would be removed on a second pass after inlining.
    // Might happen when a pointer to a context structure is passed as parameter
    // of a function that would be inlined inside a loop that would be unrolled.
    if (AllowPatial)
      StillUsed = true;
  } else {
    SmallString<128> Buf;
...
static void removePAICalls(Instruction *Marker) {
  auto IsPointerOperand = [](Value *Op, User *U) {
    ...
    if (isContextMarkerCall(U) || isPreserveArrayIndex(U) ||
...
// Look for sequences:
// - llvm.context.marker.bpf -> getelementptr... -> load
// - llvm.context.marker.bpf -> getelementptr... -> store
// And replace those with calls to intrinsics:
// - llvm.bpf.getelementptr.and.load
// - llvm.bpf.getelementptr.and.store
static bool rewriteContextAccess(Function &F, bool AllowPartial) {
  LLVM_DEBUG(dbgs() << "********** Rewrite Context Access (AllowPartial="
                    << AllowPartial << ") ************\n");

  auto ContextMarkers = collectContextMarkers(F);
  SmallPtrSet<Instruction *, 16> RemovedMarkers;

  LLVM_DEBUG(dbgs() << "There are " << ContextMarkers.size()
                    << " context markers\n");
  ...
  for (auto *Marker : ContextMarkers)
    removePAICalls(Marker);

  for (auto *Marker : ContextMarkers) {
    if (RemovedMarkers.contains(Marker))
      continue;
    bool StillUsed = rewriteAccessChain(Marker, AllowPartial, RemovedMarkers);
...
PreservedAnalyses
llvm::BPFRewriteContextAccessPass::run(Function &F,
                                       FunctionAnalysisManager &AM) {
  return rewriteContextAccess(F, AllowPartial) ? PreservedAnalyses::none()
