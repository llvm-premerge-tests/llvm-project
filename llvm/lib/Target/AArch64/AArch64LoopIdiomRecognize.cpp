...
bool AArch64LoopIdiomRecognize::recognizeByteCompare() {
  ...
  LLVM_DEBUG(dbgs() << "FOUND IDIOM IN LOOP: \n"
                    << *(EndBB->getParent()) << "\n\n");
  transformByteCompare(GEPA, GEPB, MaxLen, Index, StartIdx, IncIdx, FoundBB,
                       EndBB);
  return true;
}
...
Value *AArch64LoopIdiomRecognize::expandFindMismatch(IRBuilder<> &Builder,
  ...
  // TODO: Query the target for the minimum architectural page size? I believe
  // for most architectures this is 4096, with the exception of Ultra SPARC
  // which is 8192. Are there are any architectures with page size < 4096?!
  const uint64_t MinPageSize = 4096;
