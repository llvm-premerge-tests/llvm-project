...
static SDValue lowerBuildVectorOfConstants(SDValue Op, SelectionDAG &DAG,
  ...
  auto AddLowering = [&CheapestLowering, CurCost = -1u](
                         unsigned Cost,
  ...
  if (auto SimpleVID = isSimpleVIDSequence(Op)) {
    ...
    if (Addend || Negate) {
      ...
      if (!isInt<5>(Addend))
        Cost += RISCVMatInt::getIntMatCost(APInt(64, Addend), EltBitSize,
                                           Subtarget.getFeatureBits());
    }
    if (StepOpcode == ISD::MUL && SplatStepVal != 1) {
      Cost++;
      // There's no vmul.vi so always include the materialization cost.
      Cost += RISCVMatInt::getIntMatCost(APInt(64, SplatStepVal), EltBitSize,
                                         Subtarget.getFeatureBits());
    }
  ...
  if (VT.getSizeInBits() <= 32 &&
    ...
    Cost += RISCVMatInt::getIntMatCost(APInt(64, SplatValue), EltBitSize,
                                       Subtarget.getFeatureBits());
