...
struct {
  bool operator()(const GlobalVariable *LHS, const GlobalVariable *RHS) const {
    ...
    // Smaller offsets are easier to materialize becuause materializing a large
...
private:
  ...
  void insertGEPBeforeUser(SmallVector<User *> &UsersList,
                           GlobalVariable *GToReplace, GlobalVariable *GPool,
...
static bool hasValidUsers(GlobalVariable &GV) {
  for (User *CurrentUser : GV.users()) {
    ...
    // uses.
    if (dyn_cast<GlobalValue>(CurrentUser))
      return false;
...
void PPCMergeStringPool::collectCandidateConstants(Module &M) {
  for (GlobalVariable &Global : M.globals()) {
    ...
    ConstsToMerge.push_back(&Global);
    ...
    if (ConstsToMerge.size() >= MaxStringsPooled)
      break;
  }
}
...
bool PPCMergeStringPool::mergeModuleStringPool(Module &M) {
  ...
  // If we only have one (or fewer) constants in the module that are merge
  // candidates we can skip doing the merging.
  if (ConstsToMerge.size() <= 1)
    return false;

  // Sort the global constants to make access more efficient.
  std::sort(ConstsToMerge.begin(), ConstsToMerge.end(), CompareConstants);
...
void PPCMergeStringPool::insertGEPBeforeUser(SmallVector<User *> &UsersList,
  ...
  for (User *CurrentUser : UsersList) {
    Instruction *UseInstruction = dyn_cast<Instruction>(CurrentUser);

    // The user is a valid instruction.
    if (UseInstruction) {
      ...
      if (PHINode *UsePHI = dyn_cast<PHINode>(UseInstruction)) {
        ...
        UsePHI->replaceUsesOfWith(GToReplace, ConstGEP);
        continue;
      }

      GetElementPtrInst *GEPInst =
          GetElementPtrInst::Create(PooledStructType, GPool, Indices);
      GEPInst->insertBefore(UseInstruction);

      LLVM_DEBUG(dbgs() << "Inserting GEP before:\n");
      LLVM_DEBUG(UseInstruction->dump());

      LLVM_DEBUG(dbgs() << "Replacing this global:\n");
      LLVM_DEBUG(GToReplace->dump());
    ...
    UserConstant->handleOperandChange(GToReplace, ConstGEP);
  }
}
