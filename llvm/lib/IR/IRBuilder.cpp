...
// TODO(kwk): Change return type to CallInst* once Name is properly used.
//            And then update the failing tests.
Instruction *IRBuilderBase::CreateMalloc(Type *IntPtrTy, Type *AllocTy,
  ...
  // TODO(kwk): To use Name argument properly, replace "malloccall" with Name.
  MCall = CreateCall(MallocFunc, AllocSize, OpB, "malloccall");
  Result = MCall;

  // TODO(kwk): Remove bitcast once name argument is used above.
  PointerType *AllocPtrType = PointerType::getUnqual(AllocTy);
  if (MCall->getType() != AllocPtrType)
    // Bitcast with Name
  ...
  // TODO(kwk): Return MCall once using Name argument properly.
  return Result;
}
