...
SampleProfileReaderBinary::readSampleContextFromTable() {
  ...
  hash_code Hash = support::endian::read64le(MD5SampleContextStart + Idx);
...
std::error_code SampleProfileReaderBinary::readNameTable() {
  ...
  for (size_t I = 0; I < *Size; ++I) {
    ...
    if (std::error_code EC = Name.getError())
      return EC;
    if (UseMD5) {
      hash_code FID = hashFuncName(*Name);
      if (!ProfileIsCS)
...
SampleProfileReaderExtBinaryBase::readNameTableSec(bool IsMD5,
  ...
  if (IsMD5) {
    ...
    if (!ProfileIsCS)
      MD5SampleContextTable.resize(*Size);
    for (size_t I = 0; I < *Size; ++I) {
      auto Value = readNumber<uint64_t>();
      if (std::error_code EC = Value.getError())
        return EC;
      hash_code FID = *Value;
      if (!ProfileIsCS)
