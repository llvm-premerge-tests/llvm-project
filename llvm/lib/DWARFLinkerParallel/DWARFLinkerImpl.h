...
public:
  DWARFLinkerImpl(MessageHandlerTy ErrorHandler,
      ...
      : UniqueUnitID(0), DebugStrStrings(GlobalData),
        DebugLineStrStrings(GlobalData) {
...
protected:
  ...
  /// Increment counter for specified \p File.
  void addBinHolderNameRef(DWARFFile &File);

  /// Decrement counter for specified \p File and unload file if counter
  /// becomes zero.
  void unloadObjectFile(DWARFFile &File);
  ...
  struct LinkContext : public OutputSections {
    ...
    LinkContext(LinkingGlobalData &GlobalData, DWARFFile &File,
                ...
                std::atomic<size_t> &UniqueUnitID,
                std::optional<Triple> TargetTriple)
        : GlobalData(GlobalData), InputDWARFFile(File),
  ...
  /// BinaryHolderRefCounter counts whether references to the binary
  /// files exist.
  DenseMap<StringRef, uint32_t> BinaryHolderRefCounter;
  std::mutex BinaryHolderRefMutex;
