...
void CompileUnit::updateDieRefPatchesWithClonedOffsets() {
  ...
  DebugInfoSection.ListDebugDieRefPatch.forEach([](DebugDieRefPatch &Patch) {
    Patch.RefDieIdxOrClonedOffset = Patch.RefCU.getPointer()->getDieOutOffset(
        Patch.RefDieIdxOrClonedOffset);
  });

  DebugInfoSection.ListDebugULEB128DieRefPatch.forEach(
  ...
  SectionDescriptor &DebugLocSection =
      getSectionDescriptor(DebugSectionKind::DebugLoc);

  DebugLocSection.ListDebugULEB128DieRefPatch.forEach(
      [](DebugULEB128DieRefPatch &Patch) {
        Patch.RefDieIdxOrClonedOffset =
            Patch.RefCU.getPointer()->getDieOutOffset(
                Patch.RefDieIdxOrClonedOffset);
  ...
  SectionDescriptor &DebugLocListsSection =
      getSectionDescriptor(DebugSectionKind::DebugLocLists);

  DebugLocListsSection.ListDebugULEB128DieRefPatch.forEach(
      [](DebugULEB128DieRefPatch &Patch) {
        Patch.RefDieIdxOrClonedOffset =
            Patch.RefCU.getPointer()->getDieOutOffset(
                Patch.RefDieIdxOrClonedOffset);
...
Error CompileUnit::cloneAndEmitDebugLocations() {
  ...
  if (getOrigUnit().getVersion() < 5) {
    emitLocations(getSectionDescriptor(DebugSectionKind::DebugInfo),
                  getSectionDescriptor(DebugSectionKind::DebugLoc));
    return Error::success();
  }
...
void CompileUnit::emitLocations(SectionDescriptor &DebugInfoSection,
  ...
  for (const LinkedLocationExpressionsWithOffsetPatches &LocExpression :
    ...
    if (LocExpression.Expression.Range) {
      ...
      if (!BaseAddress) {
        ...
        encodeULEB128(getAddrIndex(*BaseAddress), OutLocationSection.OS);
      }
...
Error CompileUnit::emitDebugAddrSection() {
  ...
  SectionDescriptor &OutAddrSection =
      getSectionDescriptor(DebugSectionKind::DebugAddr);
...
Error CompileUnit::cloneAndEmitRanges() {
  ...
  emitAranges(getSectionDescriptor(DebugSectionKind::DebugInfo),
              getSectionDescriptor(DebugSectionKind::DebugARanges),
              LinkedFunctionRanges);

  if (getOrigUnit().getVersion() < 5) {
    cloneAndEmitRangeList(getSectionDescriptor(DebugSectionKind::DebugInfo),
...
void CompileUnit::cloneAndEmitRangeList(SectionDescriptor &DebugInfoSection,
...
void CompileUnit::emitRangeListFragment(const AddressRanges &LinkedRanges,
  ...
  for (const AddressRange &Range : LinkedRanges) {
    if (!BaseAddress) {
      ...
      encodeULEB128(getAddrIndex(*BaseAddress), OutRangeSection.OS);
    }
...
void CompileUnit::emitAranges(SectionDescriptor &DebugInfoSection,
...
Error CompileUnit::cloneAndEmitDebugMacro() {
  ...
  if (std::optional<uint64_t> MacroAttr =
    ...
    if (const DWARFDebugMacro *Table =
      ...
      SectionDescriptor &OutSection =
          getSectionDescriptor(DebugSectionKind::DebugMacro);
      emitMacroTableImpl(Table, *MacroAttr, OutSection, true);
    }
  }
  ...
  if (std::optional<uint64_t> MacroAttr =
    ...
    if (const DWARFDebugMacro *Table =
      ...
      SectionDescriptor &OutSection =
          getSectionDescriptor(DebugSectionKind::DebugMacinfo);
      emitMacroTableImpl(Table, *MacroAttr, OutSection, false);
    }
  }
  ...
  for (const DWARFDebugMacro::MacroList &List : MacroTable->MacroLists) {
    if (OffsetToMacroTable == List.Offset) {
      ...
      if (hasDWARFv5Header) {
        ...
        Section.emitIntVal(List.Header.Version, sizeof(List.Header.Version));
        ...
        // Write flags.
        Section.emitIntVal(Flags, sizeof(Flags));

        // Write offset to line table.
        if (StmtListOffset) {
          Section.notePatch(DebugOffsetPatch{
              Section.OS.tell(),
              &getSectionDescriptor(DebugSectionKind::DebugLine)});
          // TODO: check that List.Header.getOffsetByteSize() and
          // DebugOffsetPatch agree on size.
          Section.emitIntVal(0xBADDEF, List.Header.getOffsetByteSize());
        }
      }
      ...
      for (const DWARFDebugMacro::Entry &MacroEntry : List.Macros) {
        if (MacroEntry.Type == 0) {
          encodeULEB128(MacroEntry.Type, Section.OS);
          continue;
        }
        ...
        default: {
          ...
          if (HasVendorSpecificExtension) {
            // Write macinfo type.
            Section.emitIntVal(MacroType, 1);

            // Write vendor extension constant.
            encodeULEB128(MacroEntry.ExtConstant, Section.OS);

            // Write vendor extension string.
            Section.emitString(dwarf::DW_FORM_string, MacroEntry.ExtStr);
          } else
        ...
        case dwarf::DW_MACRO_undef: {
          ...
          // Write source line.
          encodeULEB128(MacroEntry.Line, Section.OS);

          // Write macro string.
          Section.emitString(dwarf::DW_FORM_string, MacroEntry.MacroStr);
        } break;
        ...
        case dwarf::DW_MACRO_undef_strx: {
          ...
          // Write macro string.
          Section.emitString(dwarf::DW_FORM_strp, MacroEntry.MacroStr);
          break;
        }
        case dwarf::DW_MACRO_start_file: {
          ...
          encodeULEB128(MacroEntry.File, Section.OS);
        } break;
...
Error CompileUnit::cloneAndEmit(std::optional<Triple> TargetTriple) {
  ...
  DIE *OutCUDie = cloneDIE(OrigUnitDIE.getDebugInfoEntry(), getHeaderSize(),
