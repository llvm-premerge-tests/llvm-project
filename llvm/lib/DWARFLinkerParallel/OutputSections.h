...
#define ADD_PATCHES_LIST(T)                                                    \
  ...
  void setGlobalData(LinkingGlobalData *GlobalData) {
...
protected:
  ...
  LinkingGlobalData *GlobalData = nullptr;
...
public:
  OutputSections() {
    for (size_t Idx = 0; Idx < SectionKindsNum; ++Idx)
      SectionDescriptors[Idx].SectionKind = static_cast<DebugSectionKind>(Idx);
  }
  ...
  void setOutputFormat(dwarf::FormParams Format,
    ...
    for (size_t Idx = 0; Idx < SectionKindsNum; ++Idx) {
      SectionDescriptors[Idx].Format = Format;
      SectionDescriptors[Idx].Endianess = Endianess;
    }
  }
  ...
  void setGlobalData(LinkingGlobalData *GlobalData) {
    this->GlobalData = GlobalData;
    for (size_t Idx = 0; Idx < SectionKindsNum; ++Idx)
      SectionDescriptors[Idx].setGlobalData(GlobalData);
  }
  ...
  getSectionDescriptor(DebugSectionKind SectionKind) const {
    return SectionDescriptors[static_cast<
  ...
  void eraseSections() {
    for (SectionDescriptor &CurSection : SectionDescriptors)
      CurSection.erase();
  }
  ...
  void forEach(function_ref<void(SectionDescriptor &)> Handler) {
    for (size_t Idx = 0; Idx < SectionKindsNum; ++Idx)
      Handler(SectionDescriptors[Idx]);
  }
  ...
  void assignSectionsOffsetAndAccumulateSize(
    ...
    for (size_t Idx = 0; Idx < SectionKindsNum; ++Idx) {
      SectionDescriptor &Section = SectionDescriptors[Idx];
      Section.StartOffset = SectionSizesAccumulator[Idx];
      SectionSizesAccumulator[Idx] += Section.getContents().size();
    }
  }
  ...
  uint16_t getHeaderSize() const { return Format.Version >= 5 ? 12 : 11; }
...
protected:
  ...
  std::array<SectionDescriptor, SectionKindsNum> SectionDescriptors;
};
