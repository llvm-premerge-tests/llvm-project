...
void DIEAttributeCloner::clone() {
  ...
  for (const auto &AttrSpec : Abbrev->attributes()) {
    ...
    // Note patches for attributes referencing other sections.
    switch (AttrSpec.Attr) {
    case dwarf::DW_AT_stmt_list: {
      DebugInfoOutputSection.notePatchWithOffsetUpdate(
          DebugOffsetPatch{AttrOutOffset, &CU.getSectionDescriptor(
...
bool DIEAttributeCloner::shouldSkipAttribute(
  ...
  case dwarf::DW_AT_str_offsets_base:
    // DWARFLinker does not generate or keep .debug_str_offsets table.
    return true;
  ...
  if (AttrSpec.Form == dwarf::DW_FORM_rnglistx) {
    ...
    DebugInfoOutputSection.notePatchWithOffsetUpdate(
        DebugOffsetPatch{AttrOutOffset,
                         &CU.getSectionDescriptor(DebugSectionKind::DebugAddr),
                         true},
        PatchesOffsets);

    // Use size of .debug_addr header(8) as attribute value. The offset to
