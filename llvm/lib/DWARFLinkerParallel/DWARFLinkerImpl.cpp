...
void DWARFLinkerImpl::addObjectFile(DWARFFile &File, ObjFileLoaderTy Loader,
  ...
  // Count references to source files.
  addBinHolderNameRef(ObjectContexts.back()->InputDWARFFile);
...
Error DWARFLinkerImpl::link() {
  ...
  if (GlobalData.getOptions().Verbose) {
    for (std::unique_ptr<LinkContext> &Context : ObjectContexts) {
      ...
      if (Context->InputDWARFFile.Dwarf) {
  ...
  if (GlobalData.getOptions().Threads == 1) {
    for (std::unique_ptr<LinkContext> &Context : ObjectContexts) {
      ...
      unloadObjectFile(Context->InputDWARFFile);
    }
  } else {
...
static bool isArchive(StringRef Name) { return Name.endswith(")"); }

static StringRef getBinaryHolderName(StringRef Name) {
  if (isArchive(Name))
...
void DWARFLinkerImpl::addBinHolderNameRef(DWARFFile &File) {
  BinaryHolderRefCounter[getBinaryHolderName(File.FileName)]++;
}

void DWARFLinkerImpl::unloadObjectFile(DWARFFile &File) {
  std::unique_lock<std::mutex> Guard(BinaryHolderRefMutex);

  assert(BinaryHolderRefCounter[getBinaryHolderName(File.FileName)] != 0);

  // we could not simply unload object file here.
  // Object file might be a member of an archive.
  // Unloading object file in such case would also
  // unload containing archive. Thus we count references
  // to the archive and unload it when there are no
  // more unprocessed archive members.
  BinaryHolderRefCounter[getBinaryHolderName(File.FileName)]--;

  if (BinaryHolderRefCounter[getBinaryHolderName(File.FileName)] == 0)
    File.unload();
}
...
Error DWARFLinkerImpl::LinkContext::emitInvariantSections() {
  ...
  getSectionDescriptor(DebugSectionKind::DebugLoc).OS
      << InputDWARFFile.Dwarf->getDWARFObj().getLocSection().Data;
  getSectionDescriptor(DebugSectionKind::DebugLocLists).OS
      << InputDWARFFile.Dwarf->getDWARFObj().getLoclistsSection().Data;
  getSectionDescriptor(DebugSectionKind::DebugRange).OS
      << InputDWARFFile.Dwarf->getDWARFObj().getRangesSection().Data;
  getSectionDescriptor(DebugSectionKind::DebugRngLists).OS
      << InputDWARFFile.Dwarf->getDWARFObj().getRnglistsSection().Data;
  getSectionDescriptor(DebugSectionKind::DebugARanges).OS
      << InputDWARFFile.Dwarf->getDWARFObj().getArangesSection();
  getSectionDescriptor(DebugSectionKind::DebugFrame).OS
      << InputDWARFFile.Dwarf->getDWARFObj().getFrameSection().Data;
  getSectionDescriptor(DebugSectionKind::DebugAddr).OS
...
void DWARFLinkerImpl::assignOffsetsToSections() {
  ...
  forEachSectionsSet([&](OutputSections &UnitSections) {
    UnitSections.assignSectionsOffsetAndAccumulateSize(SectionSizesAccumulator);
  });
}

void DWARFLinkerImpl::forEachSectionsSet(
