 ...
 public:
  ...
  bool operator<(const ResourceEvent &rhs) const {
    if (phase_value_ != rhs.phase_value_) {
      ...
      if (rhs.phase_value_ == -1) return true;
...
class InstructionDatabase {
 public:
  ...
  bool IsOperandValid(const InstructionDef *instr, const OperandRef *opnd,
    ...
    if ((iref_type & RefTypes::kAnyUseDef) == 0) return true;
    ...
    if (op->is_input() && (iref_type & RefTypes::kAnyUse) == 0) return false;
    if (op->is_output() && (iref_type & RefTypes::kAnyDef) == 0) return false;
    return true;
  }
