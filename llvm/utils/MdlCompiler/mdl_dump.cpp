...
std::string ResourceDef::ToString() const {
  ...
  if (get_resource_id() > 0) out += formatv("{{{0}}", get_resource_id());
  if (start_phase_) out += formatv("({0}", start_phase_->ToString());
  if (end_phase_) out += formatv("..{0}", end_phase_->ToString());
  if (start_phase_) out += ")";
  if (bit_size_ > 0) out += formatv(":{0}", bit_size_);
  if (pool_size_ > 0) out += formatv("[{0}]", pool_size_);
  if (!members_.empty()) {
    auto sep = group_type() == GroupType::kUseAll ? " & " : " , ";
    out += StringVec<Identifier *>(&members_, " {", sep, "}");
  }

  if (reg_class_) out += formatv(" <{0}>", reg_class()->id()->ToString());
...
std::string ResourceDef::ref_summary() const {
  if (earliest_ref_ == -1 && latest_ref_ == -1 && !phase_expr_seen_) return "";
...
std::string ResourceRef::ToString() const {
  ...
  if (member_) out += formatv(".{0}{{{1}}", member_->ToString(), member_id());
  if (first_ != -1 && last_ != -1) {
    if (last_ != first_)
  ...
  if (pool_count_ != -1) out += formatv(":{0}", pool_count_);
  if (pool_count_name_) out += formatv(":{0}", pool_count_name_->ToString());
  if (value_name_) out += formatv(":{0}", value_name_->ToString());
  if (operand_index_ != -1) out += formatv("-->{0}", operand_index_);
  return out;
}
...
std::string FuncUnitInstance::ToString() const {
  ...
  if (id_) out += formatv(" {0}", id_->ToString());
  out += StringVec<ResourceRef *>(args_, "(", ", ", ")");
  if (pin_any_) out += StringVec<Identifier *>(pin_any_, " -> ", " | ", "");
  if (pin_all_) out += StringVec<Identifier *>(pin_all_, " -> ", " & ", "");
  return out;
}
...
std::string Connect::ToString() const {
  ...
  if (reg_class_) out += formatv(" to {0}", reg_class_->ToString());
  if (resource_) out += formatv(" via {0}", resource_->ToString());
  return out + "\n";
}
...
std::string OperandRef::type_name() const {
  if (operand_decl_) return operand_decl_->type_name();
  if (operand_) return operand_->name();
  if (reg_class_) return reg_class_->name();
  if (op_type_) return op_type_->ToString();
  return "";
}
...
std::string PhaseExpr::ToString() const {
  ...
  switch (operation_) {
    ...
    case kPhase:
      if (phase_name_) return phase_name_->ToString();
      if (phase_) return phase_->ToString();
  }
  return "Unknown";
}
...
std::string PhaseName::FormatProtection() const {
  if (is_hard_) return ".h";
  if (is_protected_) return ".p";
  return ".u";
}
...
std::string Reference::ToString() const {
  ...
  if (IsConditionalRef()) {
    ...
    if (ref_type() == RefTypes::kFus) {
      ...
      if (RefFlags::is_begin_group(fu_flags())) out += "begin-group ";
      if (RefFlags::is_end_group(fu_flags())) out += "end-group ";
      if (RefFlags::is_single_issue(fu_flags())) out += "single-issue ";
      if (RefFlags::is_retire_ooo(fu_flags())) out += "retire-ooo ";
      return out + ")";
    }
    out += formatv("{0}({1}", phase_expr_->FormatProtection(),
                   phase_expr_->ToString());
    if (use_cycles() != 1) out += formatv(":{0}", use_cycles());
    if (repeat() != 1) out += formatv("[{0},{1}]", repeat(), delay());
    if (operand_) out += formatv(", {0}", operand_->ToString());
  ...
  if (predicates_) out += StringVec<Identifier *>(predicates_, "  {", ",", "}");
  return out;
}
...
std::string ConditionalRef::ToString(bool is_else) {
  ...
  for (auto *ref : refs()) out += "           " + ref->ToString() + "\n";
  if (else_clause()) out += else_clause()->ToString(true);
  if (out.back() == '\n') out.pop_back();
  return out + "\n";
}
...
std::string OperandDef::ToString() const {
  ...
  if (operands_ && type_ && !bases_)
    return out + StringVec<OperandDecl *>(operands_, "(", ", ", ")") +
           formatv(" {{ {0} }\n", type);
  if (bases_) out += StringVec<Identifier *>(bases_, ": ", ": ", "");
  out += " {\n";
  if (type_) out += formatv("    {0}\n", type);
...
std::string OperandAttribute::ToString() const {
  ...
  if (values()->size() == 1) {
    ...
    out += "[";
    for (auto value : *values()) out += formatv("{0},", value);
    out += "]";
  }
  if (!predicate_values_->empty()) out += "\n      ";
  if (!type().empty()) out += formatv(" if {0}", type());
  if (!predicate_values_->empty())
    out += StringVec<PredValue *>(predicate_values_, " [", ", ", "]");
  if (predicate_) out += StringVec<Identifier *>(predicate_, "  {", ",", "}");
  return out;
}
...
std::string PredValue::ToString() const {
  if (IsValue()) return FormatValue(value());
  if (IsRange())
    return formatv("{0}..{1}", FormatValue(low()), FormatValue(high()));
  if (IsMask()) return formatv("{{ {0:X8}UL }", mask());
  return "empty";
}
...
std::string DumpInstructionDefs(const InstructionList &instructions) {
  ...
  for (auto &entries : instruction_map) {
    ...
    for (auto *instruction : instructions) out += instruction->ToString();
  }
...
void SubUnitInstantiation::DumpSubUnitInstantiation() {
  ...
  for (auto *ref : references()) out += formatv("    {0}\n", ref->ToString());
...
void MdlSpec::DumpSubUnitInstantiations() {
  ...
  for (const auto &subunit : su_instantiations()) {
    ...
    std::cout << formatv(
        "\nSubunit: {0} ---------------------------------------\n", name);
    for (auto *su : *unit) su->DumpSubUnitInstantiation();
  }
}
...
std::string ResourceDef::resource_format() {
  ...
  if (!IsGroupDef() && pool_size() <= 1) out += std::to_string(id);
  if (IsGroupDef()) {
    out += "[";
  ...
  if (pool_size() > 1) out += formatv("[{0}..{1}]", id, pool_size() + id - 1);

  return out;
}

std::string SubPool::ToString() const {
  if (first() == -1 && last() == -1) return "[group]";
