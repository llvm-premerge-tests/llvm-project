...
std::string FormatReferenceFlags(const Reference *ref) {
  std::string out;
  if (ref == nullptr || ref->phase_expr()->IsDefaultLatency()) return "0";
  if (ref->IsProtected())
...
std::string OutputState::FormatSingleConditionalOperand(ConditionalRef *cond) {
  ...
  for (; cond; cond = cond->else_clause()) {
    ...
    if (pred != nullptr && !pred->IsTrue()) {
      ...
      if (seen_predicates.count(index)) continue;  // Skip duplicate predicates
    ...
    if (pred == nullptr || pred->IsTrue()) break;
  }
  ...
  for (auto *ref : refs) {
    ...
    for (auto *res : *ref->resources()) {
      if (!res->IsNull() && (res->HasCount() == format_pooled_refs)) {
        ...
        auto event = ResourceEvent(ref_type, phase, ref->use_cycles(), res,
                                   ref, subunit);
        if (!res->HasCount())
...
std::string OutputState::FormatFuncUnitReference(
       SubUnitInstantiation *subunit, Reference *ref, bool format_pooled_refs) {
  std::string out;
  auto *cpu = subunit->cpu();
    ...
    if (spec().IsFuncUnitTemplate(res->name())) {
      ...
      if (format_pooled_refs && funits.size() > 1) {
        ...
        for (auto *unit : funits) {
          ...
          ResourceEvent fu_member(RefTypes::kFus, ref->phase_expr(),
                                  ref->use_cycles(), fu, 0,
                                  unit->instance()->is_buffered(),
                                  ref->fu_flags());
...
std::string OutputState::FormatIfElseResourceRef(
  ...
  if (cond->instr_predicate() == nullptr) {
    ...
    auto else_refs = FormatIfElseResourceRef(subunit, cond->else_clause(), type,
                                        output_list, name, format_pooled_refs);
    // Add else clauses to ConditionalRefs forward references.
    if (else_refs != kNull) {
      if (format_pooled_refs)
    ...
    if (else_refs == kNull && then_refs == kNull) return kNull;
    if (else_refs != kNull) else_refs = "&" + else_refs;
    if (then_refs != kNull) then_refs = "&" + then_refs;
...
// and enter into a table so it can be shared between subunits.
std::string OutputState::FormatResourceReferences(InstrInfo *info,
                                                  RefType type,
  ...
  for (auto *ref : info->resource_refs()) {
    if (ref->IsConditionalRef()) {
      auto res = FormatIfElseResourceRef(subunit, ref->conditional_ref(),
                                         type, output_list, name, false);
      if (res != kNull) out += "{&" + res + "},";
...
OperandAttribute *FindAttribute(const std::string &name, const OperandDef *opnd,
  ...
  if (opnd == nullptr || opnd->attributes() == nullptr) return nullptr;
  for (auto *attr : *opnd->attributes())
    if (name == attr->name())
      if (subunit->ValidPredicate(attr->predicate())) return attr;
  return nullptr;
}
...
bool FindDerivation(OperandDef *ref, const OperandDef *decl,
  ...
  opnds.push_back(ref);
  if (ref == decl) return true;
  if (ref->base_operands())
    for (auto *base : *ref->base_operands())
      if (FindDerivation(base, decl, opnds)) return true;
...
std::string FormatAttributePredicate(const OperandAttribute *attr) {
  ...
  if (attr->predicate_values()->size() > 1) return formatv("    if ({0})", out);
...
std::string FormatPooledCountFunction(std::string const &attr,
  ...
  for (auto *opnd : opnds) {
    for (auto op_attr : *opnd->attributes()) {
      if (op_attr->name() == attr &&
        ...
        if (attr.empty()) none = result + "  // none\n";
  ...
  if (!lits.empty()) {
    out +=
        "  if (ins->isOpndLiteral(operand_index)) {\n"
        "    int64_t value = ins->GetOperand(operand_index);\n";
    for (const auto &lit : lits) out += lit;
    out += "  }\n";
  }
  if (!label.empty())
  ...
  if (!none.empty()) out += none;

  if (none.empty()) out += formatv("  return {0};  // default\n", max_value);
  return out;
}
    ...
    if (opnd->attributes()) {
      for (auto op_attr : *opnd->attributes()) {
        if (op_attr->name() == attr &&
          ...
          if (unconditional[tuple_size]) continue;
...
//    };
std::string OutputState::FormatPooledResourceReference(
  ...
  if (ref.IsFuncUnitRef())
    return formatv(
       "{{{0},{1},{2},{3},{4},{5}}", FormatReferenceType(ref.ref_type()),
...
std::string OutputState::FormatPooledResourceReferences(
    InstrInfo *info, OutputSet &output_list, FormatName name) {
...
std::string OutputState::FormatIfElseConstraint(ConditionalRef *cond) {
  ...
  if (then_refs != kNull) forward_constraint_refs_.insert(then_refs);
  ...
  if (cond->instr_predicate() == nullptr) {
    ...
    if (else_refs != kNull) forward_cond_constraint_refs_.insert(else_refs);
...
std::string OutputState::FormatPortReferences(InstrInfo *info) {
  ...
  for (auto *ref : *info->references()) {
    if (ref->IsConditionalRef()) {
      ...
      if (constraint != kNull) out += "{&" + constraint + "},";
    } else {
      auto constraint = FormatConstraint(ref);
      if (!constraint.empty()) out += constraint + ",";
    }
  ...
  for (auto &ref : refs)
    for (auto &old_ref : refs) {
      ...
      if (res->get_final_resource_id() != old_res->get_final_resource_id())
        continue;
      if (ref.ref_type() != old_ref.ref_type()) continue;
      if (res->HasCount() || old_res->HasCount()) continue;
...
std::string OutputState::FormatSubunit(InstrInfo *info) {
  ...
  auto used = FormatResourceReferences(info, RefTypes::kUse,
                               used_resource_refs_, &UsedResourceListName);
  auto held = FormatResourceReferences(info, RefTypes::kHold,
                               held_resource_refs_, &HeldResourceListName);
...
std::string OutputState::FormatSubunits(const std::string &instr,
  ...
  if (out.empty()) return kNull;  // We didn't find subunits for this CPU.
  out.pop_back();                 // throw away trailing comma.
...
void OutputState::WriteVectorTable(const OutputSet &objects,
  ...
  for (auto &[out, index] : objects) {
    ...
    output_c() << formatv("{0}Vec {1} = {{ {2}, {1}_data };\n",
        type, name(index), count_vector_init_items(out));
...
void OutputState::WritePoolCountFunctions(const OutputSet &funcs,
  ...
  for (auto &[func, index] : funcs)
    output_c() << formatv(
        "int {0}(Instr *ins, int operand_index) {{\n{1}}\n",
        name(index), func);
}
...
void OutputState::WritePoolValueFunctions(const OutputSet &funcs,
  ...
  for (auto &[func, index] : funcs)
    output_c() << formatv(
        "bool {0}(Instr *ins, int operand_index, "
        "int size, int values[]) {{\n{1}}\n",
        name(index), func);
}
...
void OutputState::WriteInstructionInfo() const {
  if (!generate_llvm_defs_) return;
  if (database_->spec().cpus().empty()) return;
...
void OutputState::ExpandForwardStmt(FwdNetwork &network, const CpuInstance *cpu,
  ...
  if (!defs.empty()) {
    for (const auto &[to, cycles] : fwd->to_units()) {
      ...
      if (!uses.empty()) {
        for (auto def : defs)
          for (auto use : uses) network.set(def, use, cycles);
      } else {
...
void OutputState::GenerateForwardingInfo() {
  ...
  for (auto &item : networks) output_c() << item;
}
...
void OutputState::WriteCpuList() const {
  ...
  for (auto *cpu : database_->spec().cpus()) {
    ...
    std::string fwd = cpu->forward_stmts()->empty()
                            ? kNull : formatv("&FWD_{0}[0]", cpu->name());
    int resource_factor = CalcResourceFactor(cpu);

    cpu_defs += formatv(
          "CpuConfig<CpuParams<{0},{1},{2}, {3},{4}, {5},{6}, "
...
void OutputState::OpenOutputFiles() {
  ...
  auto AddSlash = [](std::string path_name) {
    if (!path_name.empty() && path_name.back() != '/') path_name += "/";
    return path_name;
  };
...
void OutputState::WriteHeader() {
  ...
  output_c() << formatv(
      "{0}// Machine Description Database.\n"
    ...
    for (auto *res : cpu->all_resources())
      if (res->ref_types() & (RefTypes::kUse | RefTypes::kFus)) {
        ...
        if (res->IsGroupDef()) {
          ...
          if (res->pool_size() > 0) res_id += res->pool_size() - 1;
...
std::string ResourceName(MdlSpec &spec, CpuInstance *cpu,
  ...
  if (cluster && !cluster->IsNull()) name += formatv("{0}::", cluster->name());
  name += res->name();
  return name;
}
...
void AddResourceDef(std::string *out, MdlSpec &spec, CpuInstance *cpu,
                    ...
                    std::string note) {
  std::string prefix = "  ";
  if (cluster && !cluster->IsNull()) prefix += "  ";
...
void OutputState::WriteCpuTable() {
  ...
  WriteTable(cond_resource_refs_, "ConditionalRef<ResourceRef>", "",
            &CondResourceReferenceName, "Conditional Resource Reference Table");
  ...
  output_c() << divider <<
             ...
             "//     - Number of MicroOps\n"
             "//   or (for conditional references)\n"
             "//     - Conditional reference" << divider;

  WriteVectorTable(used_resource_refs_, "ResourceRef",
             &UsedResourceListName, "Used Resource Reference Table");
  WriteVectorTable(held_resource_refs_, "ResourceRef",
             &HeldResourceListName, "Held Resource Reference Table");
  WriteVectorTable(reserved_resource_refs_, "ResourceRef",
             &ReservedResourceListName, "Reserved Resource Reference Table");
  ...
  WriteVectorTable(constraints_, "OperandConstraint",
             &ConstraintListName, "Operand Constraint Table",
             constraint_table_header);
