...
PhaseName *MdlSpec::FindFirstPhase() {
  if (first_phase_name_ != nullptr) return first_phase_name_;
...
PhaseName *MdlSpec::FindFirstExecutePhase(CpuInstance *cpu) {
  ...
  if (auto *phase = SearchPipeReference(&first, cpu)) return phase;
...
RefType StringToRefType(const std::string &ref_type) {
  static std::map<std::string, RefType> *mapping =
      new std::map<std::string, RefType>({{"predicate", RefTypes::kPred},
                                          ...
                                          {"fus", RefTypes::kFus}});
  if (mapping->count(ref_type)) return mapping->at(ref_type);
...
std::string RefTypeToString(RefType ref_type) {
  ...
  if (ref_type < RefTypes::kNull || ref_type > RefTypes::kFus) return "RefNull";
...
std::string FormatReferenceTypes(int ref_type) {
  std::string out;
  if (ref_type & static_cast<int>(RefTypes::kPred)) out += " Predicate";
  if (ref_type & static_cast<int>(RefTypes::kUse)) out += " Use";
  if (ref_type & static_cast<int>(RefTypes::kDef)) out += " Def";
  if (ref_type & static_cast<int>(RefTypes::kKill)) out += " Kill";
  if (ref_type & static_cast<int>(RefTypes::kUseDef)) out += " UseDef";
  if (ref_type & static_cast<int>(RefTypes::kHold)) out += " Hold";
  if (ref_type & static_cast<int>(RefTypes::kReserve)) out += " Reserve";
  if (ref_type & static_cast<int>(RefTypes::kFus)) out += " Fus";
    ...
    if (ref->IsConditionalRef()) {
      ...
      for (auto *res : *ref->resources()) {
        if (FindItem(fu_map_, res->name()) || FindItem(cpus_, res->name())) {
          ...
          for (auto *fu : *group->members()) fus.insert(fu->name());
        } else {
...
std::set<std::string> *MdlSpec::FindLatencyFuncUnits(LatencyTemplate *lat) {
  if (lat->referenced_fus()) return lat->referenced_fus();
    ...
    for (auto *latency : *subunit->latencies())
      for (auto &funit : *latency->get_template()->referenced_fus()) {
        ...
        if (!fu_map_[funit]->is_implicitly_defined()) {
          WarningLog(latency, "Invalid reference to an explicitly defined "
                              "functional unit \"{0}\"", funit);
          continue;
        }
...
static std::string GetPrefix(std::string &regex) {
  ...
  if (first_meta == std::string::npos) return regex;
...
void MdlSpec::TieSubUnitToInstructions(SubUnitTemplate *subunit,
  ...
  for (auto &regex : *regex_bases) {
    ...
    if (!pattern.empty()) {
      if (pattern[0] != '^') pattern = formatv("^({0})", pattern);
    ...
    if (!prefix.empty()) begin = instruction_map_.lower_bound(prefix);
...
ResourceRef *FuncUnitInstantiation::CheckAllocation(ResourceRef *def,
  ...
  if (!ref->HasCount() || count_name) return def;
...
ResourceRef *FuncUnitInstantiation::MergeRefs(ResourceRef *def,
                                              ResourceRef *ref) {
  if (def->IsNull()) return new ResourceRef(*def);
...
RegisterClass *FuncUnitInstantiation::FindRegClass(Identifier *item) {
  if (auto *arg = FindItem(class_args(), item->name())) return arg->regs();
  return nullptr;
}
...
ResourceDef *FuncUnitInstantiation::BindSubUnitPort(ResourceRef *arg) {
  if (arg->IsNull()) return NullPortDef;
  if (auto *port_arg = FindItem(ports(), arg->name())) return port_arg;
...
ResourceRef *FuncUnitInstantiation::BindSubUnitResource(ResourceRef *arg) {
  ...
  if (arg->IsNull()) return NullResourceRef;
...
RegisterClassRef *FuncUnitInstantiation::BindFuncUnitClass(ResourceRef *arg) {
  // If this is a "null" binding, just return null.
  if (arg->IsNull()) return new RegisterClassRef(NullRegisterClass);
...
void FuncUnitInstantiation::BindFuncUnitSlotResources() {
  ...
  if (slots == nullptr && (cluster()->issues() || cpu()->issues())) {
    ...
    for (auto *res : *issues) slots->push_back(res->id());
  }
  ...
  if (implicit_slots != nullptr) delete implicit_slots;
}
...
ResourceDef *SubUnitInstantiation::BindLatPort(ResourceRef *arg) {
  if (arg->IsNull()) return NullPortDef;
  if (auto *port_arg = FindItem(port_args(), arg->name())) return port_arg;
...
// else clauses.
//---------------------------------------------------------------------------
ConditionalRef *SubUnitInstantiation::CopyLatencyCondReference(
       LatencyInstantiation &lat, ConditionalRef *cond) {
...
//---------------------------------------------------------------------------
void SubUnitInstantiation::CopyLatencyReference(
     LatencyInstantiation &lat, ReferenceList &references, Reference *ref) {
  if (!ValidPredicate(ref->predicates())) return;
...
void SubUnitInstantiation::InstantiateLatencyBases(LatencyInstantiation &lat,
  ...
  // or recursive bases).
  if (std::find(bases.begin(), bases.end(), parent) != bases.end()) return;
...
void SubUnitInstantiation::InstantiateLatencies() {
  if (su_template()->latencies() == nullptr) return;
...
FuncUnitInstantiation *MdlSpec::AddFuncUnitInstantiation(
    CpuInstance *cpu, ClusterInstance *cluster, FuncUnitInstance *fu_inst) {
...
FuncUnitInstantiation *MdlSpec::AddFuncUnitBaseInstantiation(
...
void ResourceDef::AddReferenceSizeToPool(const ResourceRef *resource,
  ...
  for (auto *opnd : opnds)
    if ((attr = FindAttribute(count_name, opnd, subunit)) != nullptr) break;
  if (attr == nullptr) {
    pool_info.add_count(resource->pool_size());
    return;
  }
...
void ResourceDef::RecordReference(RefType type, const PhaseExpr *expr,
  ...
  if (earliest_ref_ == -1 || phase < earliest_ref_) earliest_ref_ = phase;
...
static int AssignId(ResourceDef *def, int resource_id) {
  ...
  if (def->pool_size() > 0) return resource_id + def->pool_size();
...
void MdlSpec::AssignResourceIds() {
  for (auto *cpu : cpus()) {
    ...
    for (auto *res : cpu->all_resources())
      if (!res->IsGroupDef()) resource_id = AssignId(res, resource_id);

    // For each CPU, note the resource id of the last functional unit.
    if (last_fu) cpu->set_max_fu_id(last_fu->get_resource_id());
  }
}
