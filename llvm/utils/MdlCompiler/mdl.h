...
A *FindItem(std::vector<A *> &items, const std::string &name) {
  for (auto *a_item : items)
    if (a_item->name() == name) return a_item;
  return nullptr;
}
 ...
 public:
  ...
  bool IsSupersetOf(const RegisterClass *decl) const {
    for (auto *reg : *decl->members_)
      if (!FindItem(*members_, reg->name())) return false;
    return true;
  }
  std::string ToString() const;
...
class SubPoolInfo {
 public:
  ...
  void add_count(int count) {
    if (count) counts_.emplace(count);  // Don't add zeros.
  }
 ...
 public:
  ...
  int GetMemberId(const Identifier *member) const {
    for (auto *mem : members_)
      if (mem->name() == member->name()) return mem->index();
    return -1;
  }
 ...
 public:
  ...
  ResourceRef(const MdlItem &item, Identifier *id, int pool_count,
      ...
      : MdlItem(item),
        ...
        pool_count_name_(pool_count_name),
  ...
  int last() const { return last_; }
  int pool_size() const {
    if (IsGroupDef()) return definition_->members().size();
  ...
  int get_final_resource_id() const {
    if (first() != -1 && first() == last()) return get_resource_id() + first();
...
class CpuInstance : public MdlItem {
 public:
  CpuInstance(const MdlItem &item, Identifier *id, PipeDefList *pipe_phases,
      ...
      : MdlItem(item),
        id_(id),
        pipe_phases_(pipe_phases),
        issues_(issues),
        resources_(res),
        reorder_buffer_size_(reorder_buffer_size),
        clusters_(clusters),
        forward_stmts_(forward_stmts),
...
class ClusterInstance : public MdlItem {
 public:
  ...
  explicit ClusterInstance(FuncUnitInstance *func_unit)
      : MdlItem(),
        id_(new Identifier("__")),
        issues_(new ResourceDefList),
...
class FuncUnitInstance : public MdlItem {
 public:
  ...
  explicit FuncUnitInstance(const std::string type)
      : MdlItem(),
        id_(new Identifier(type)),
        type_(new Identifier(type)),
        args_(new ResourceRefList),
        pin_all_(nullptr),
        pin_any_(nullptr) {}
 ...
 public:
  ...
  FuncUnitTemplate(MdlItem &item, Identifier *id, IdList *bases)
      : MdlItem(item),
        ...
        ports_(new IdList),
        resources_(new ResourceDefList),
        connections_(new ConnectList),
        subunits_(new SubUnitInstList),
  ...
  void clone_params(ParamsList *params) {
    params_ = new ParamsList;
    for (auto *param : *params) params_->push_back(new Params(*param));
  }
...
class FuncUnitUse : public MdlItem {
 public:
  ...
  std::string ToString() const {
    std::string out;
    if (predicate_) out += predicate_->name() + ":";
    out += func_unit_->name();
    if (cycles_ != -1) out += llvm::formatv("<{0}>", cycles_);
    return out;
  }
 ...
 public:
  ...
  SubUnitTemplate(const std::string type, LatencyInstance *latency,
      ...
      : MdlItem(),
        ...
        bases_(nullptr),
        params_(new ParamsList),
        latencies_(new LatencyInstList(1, latency)),
        inline_latency_(inline_latency) {}
  ...
  void add_derived_subunit(SubUnitTemplate *derived) {
    if (FindItem(derived_subunits_, derived->name())) return;
...
class OperandRef : public MdlItem {
 public:
  ...
  explicit OperandRef(std::string default_name)
      : MdlItem(),
        op_type_(new Identifier(default_name)),
...
class PhaseExpr : public MdlItem {
 public:
  ...
  PhaseExpr *clone() {
    if (operation() == kPhase) return new PhaseExpr(phase_, phase_name_);
    if (operation() == kInt) return this;
    if (operation() == kOpnd)
  ...
  bool operator==(const PhaseExpr &item) {
    if (operation_ != item.operation_) return false;
    if (left_ && item.left_ && *left_ != *item.left_) return false;
    if (right_ && item.right_ && *right_ != *item.right_) return false;
    if (operation_ == PhaseOp::kPhase) return phase_name_ == item.phase_name_;
    if (operation_ == PhaseOp::kInt) return number_ == item.number_;
  ...
  PhaseExpr *increment(int increment) {
    if (increment == 0) return this->clone();
...
class PredExpr : public MdlItem {
 public:
  ...
  PredExpr(PredOp opcode, bool negate) : MdlItem(), opcode_(opcode) {
    if (negate) opcode = IsTrue() ? PredOp::kFalse : PredOp::kTrue;
  }
 ...
 public:
  ...
  Reference(const MdlItem &item, IdList *predicates, RefType ref_type,
      ...
      : MdlItem(item),
        ...
        conditional_ref_(nullptr),
        ref_type_(ref_type),
        phase_expr_(phase_expr),
        use_cycles_(use_cycles),
        repeat_(repeat),
        delay_(delay),
        operand_(operand),
  ...
  Reference(MdlItem &item, RefType ref_type, PhaseExpr *phase_expr,
            int cycles, int micro_ops, RefFlags::Item fu_flags,
            ResourceRef *unit)
      : MdlItem(item),
        ref_type_(ref_type),
        phase_expr_(phase_expr),
  ...
  Reference(MdlItem &item, int micro_ops, RefFlags::Item fu_flags)
      : MdlItem(item),
        ref_type_(RefTypes::kFus),
        phase_expr_(new PhaseExpr(new PhaseName("E1"))),
        phase_value_(1),
        use_cycles_(0),
        repeat_(0),
        delay_(0),
  ...
  Reference(RefType ref_type, PhaseName *phase, std::string func_unit_name)
      : MdlItem(),
        ref_type_(ref_type),
        phase_expr_(new PhaseExpr(phase)),
  ...
  Reference(Reference *item, PhaseExpr *phase)
      : MdlItem(*item),
        predicates_(item->predicates()),
        conditional_ref_(item->conditional_ref()),
        ref_type_(item->ref_type()),
        phase_expr_(phase ? phase : item->phase_expr()),
        use_cycles_(item->use_cycles()),
        micro_ops_(item->micro_ops()),
        operand_(item->operand()),
        resources_(new ResourceRefList),
        port_(nullptr),
        base_(item) {
  ...
  Reference(Reference *item, ConditionalRef *cond)
      : MdlItem(*item),
        predicates_(item->predicates()),
        conditional_ref_(cond),
        ref_type_(item->ref_type()),
        phase_expr_(item->phase_expr()),
        resources_(new ResourceRefList) {}
  ...
  Reference(Reference &item, int delay)
      : MdlItem(item),
        ...
        ref_type_(item.ref_type()),
        phase_expr_(item.phase_expr() ?
                    item.phase_expr()->increment(delay) : nullptr),
        use_cycles_(item.use_cycles()),
        repeat_(item.repeat()),
        delay_(item.delay()),
        micro_ops_(item.micro_ops()),
  ...
  bool operator<(const Reference &item) const {
    if (phase_value_ != item.phase_value_) {
      if (phase_value_ == -1) return false;
      if (item.phase_value_ == -1) return true;
    ...
    if (ref_type() != item.ref_type()) return ref_type_ < item.ref_type_;
...
class ConditionalRef : public MdlItem {
 public:
  ...
  explicit ConditionalRef(ConditionalRef *item, ConditionalRef *else_clause)
      : MdlItem(*item),
        predicate_(item->predicate_),
        instr_predicate_(item->instr_predicate_),
        else_clause_(else_clause) {}
  ...
  bool IsSingleRef() const {
    ...
    if (refs_[0]->use_cycles() != 1) return false;   // overly strict, probably.
    // We can't handle nested ifs, nor non-defs/uses.
    if (refs_[0]->IsConditionalRef() ||
        (!refs_[0]->IsDef() && !refs_[0]->IsUse())) return false;
  ...
  bool IsSingleOperand(const std::string &name, RefType ref_type) const {
    if (!IsSingleRef()) return false;
    if (ref_type != refs_[0]->ref_type()) return false;
    if (auto *operand = refs_[0]->operand()) {
      if (operand->name() != name) return false;
  ...
  bool HasOperandRefs() {
    if (HasOperandRefs(refs_)) return true;
    if (else_clause_ != nullptr) return else_clause_->HasOperandRefs();
    return false;
  }
  ...
  bool HasOperandRefs(ReferenceList &refs) {
    for (auto *ref : refs_) {
      if (ref->IsConditionalRef() && ref->conditional_ref()->HasOperandRefs())
        return true;
      if (ref->operand() != nullptr) return true;
    }
    return false;
  }
  ...
  bool HasResourceRefs() {
    if (HasResourceRefs(refs_)) return true;
    if (else_clause_ != nullptr) return else_clause_->HasResourceRefs();
    return false;
  }
  ...
  bool HasResourceRefs(ReferenceList &refs) {
    for (auto *ref : refs_) {
      if (ref->IsFuncUnitRef()) return true;
      if (ref->IsConditionalRef() && ref->conditional_ref()->HasResourceRefs())
        return true;
      if (!ref->resources()->empty()) return true;
    }
    return false;
  }
...
class OperandDecl : public MdlItem {
 public:
  OperandDecl(const MdlItem &item, Identifier *type, Identifier *name,
      ...
      : MdlItem(item),
        types_(new IdList({type})),
        names_(new IdList({name})),
        ellipsis_(ellipsis),
        input_(input),
        output_(output) {}
  ...
  OperandDecl(OperandDecl *item, OperandDecl *parent)
      : MdlItem(),
        ...
        ellipsis_(parent->ellipsis_),
        input_(parent->input_),
        output_(parent->output_),
        operand_(item->operand_),
 ...
 public:
  OperandAttribute(MdlItem &item, Identifier *name, std::vector<int> *values,
      ...
      : MdlItem(item),
        ...
        values_(values),
        type_(type),
        predicate_values_(predicate_values),
        predicate_(predicate) {}
 ...
 public:
  InstructionDef(const MdlItem &item, Identifier *name,
      ...
      : MdlItem(item),
        name_(name),
        operands_(operands),
        subunits_(subunits ? subunits : new IdList),
        derived_(derived) {}
  ...
  OperandDecl *GetOperandDecl(unsigned index) const {
    if (index >= flat_operands_->size()) return nullptr;
  ...
  void add_subunit(SubUnitTemplate *subunit) {
    if (FindItem(*subunits_, subunit->name())) return;
 ...
 public:
  ...
  FuncUnitInstantiation(FuncUnitInstantiation *fu, FuncUnitTemplate *base)
      : spec_(fu->spec()),
        ...
        resource_args_(fu->resource_args()),
        class_args_(fu->class_args()),
        parent_(fu) {
    InstantiateLocalDefs();
  }
...
class MdlSpec {
 public:
  ...
  void FindFunctionalUnitClientCpus(FuncUnitTemplate *funit,
                                    CpuInstance *cpu);
  ...
  FuncUnitInstantiation *AddFuncUnitBaseInstantiation(
      FuncUnitInstantiation *parent, FuncUnitTemplate *base);
 ...
 public:
  ...
  template <typename A>
  void FindDuplicates(const std::vector<A *> &items) {
  ...
  void FindDuplicateMembers(ResourceDefList &items) {
    for (auto *item : items)
      if (item->IsGroupDef()) FindDuplicates(item->members());
  }
  ...
  void EnterPredicate(std::string &name, PredExpr *pred) {
    ...
    if (name == "TruePred" || name == "FalsePred") return;
