...
bool MdlVisitor::ProcessInputFile(std::string file_name) {
  ...
  // in parsing it again.
  if (!imported_files_.emplace(file_name).second) return true;
  ...
  if (error_listener.syntax_error_count()) return false;
...
bool MdlVisitor::VisitImportFile(ImportFileCtx *ctx) {
  ...
  if (!current_dir.empty()) {
    ...
    if (std::filesystem::exists(name)) return ProcessInputFile(name);
  }
...
CpuInstance *MdlVisitor::VisitCpuDef(CpuDefCtx *ctx) {
  ...
  if (aliases.empty()) aliases.push_back(name->name());
...
//-----------------------------------------------------------------------------
ClusterInstance *MdlVisitor::VisitClusterInstantiation(
    ClusterInstantiationCtx *ctx) {
  Identifier *name = VisitIdent(ctx->ident());
  auto *issues = new ResourceDefList;
  auto *resources = new ResourceDefList;
  auto *func_units = new FuncUnitInstList;
...
//-----------------------------------------------------------------------------
FuncUnitInstance *MdlVisitor::VisitFuncUnitInstantiation(
    FuncUnitInstantiationCtx *ctx, ResourceDefList *resources) {
  ...
  if (ctx->all) pin_all = VisitPinAll(ctx->all);
  if (ctx->any) pin_any = VisitPinAny(ctx->any);
...
IdList *MdlVisitor::VisitPinAny(PinAnyCtx *ctx) {
  ...
  for (auto *name_ctx : ctx->ident()) names->push_back(VisitIdent(name_ctx));
  return names;
}
...
ParamsList *MdlVisitor::VisitFuncUnitParams(FuncUnitParamsCtx *ctx) {
  ...
  if (ctx == nullptr) return params;  // Return if zero parameters.
...
//-----------------------------------------------------------------------------
SubUnitInstList *MdlVisitor::VisitSubunitInstantiation(
    SubunitInstantiationCtx *ctx, ResourceDefList *resources) {
...
SubUnitTemplate *MdlVisitor::VisitSubunitTemplate(SubunitTemplateCtx *ctx) {
  ...
  if (!ctx->latency_items().empty()) {
    ...
    inline_lat =
        new LatencyTemplate(item, tname, nullptr, tparams, refs);
...
ParamsList *MdlVisitor::VisitSuDeclItems(SuDeclItemsCtx *ctx) {
  ...
  if (ctx == nullptr) return params;  // Return if no parameters.
...
//-----------------------------------------------------------------------------
ReferenceList *MdlVisitor::VisitLatencyItems(
    std::vector<LatencyItemsCtx *> &ctx) {
...
Reference *MdlVisitor::VisitLatencyRef(LatencyRefCtx *ctx, IdList *predicates) {
  ...
  int cycles = 1;
  if (spec->cycles) cycles = spec->cycles->value;
  int repeat = 1;
  if (spec->repeat) repeat = spec->repeat->value;
  int delay = 1;
  if (spec->delay) delay = spec->delay->value;
...
PhaseExpr *MdlVisitor::VisitExpr(ExprCtx *ctx) {
  ...
  if (ctx->subexpr) return VisitExpr(ctx->subexpr);

  // Handle a literal constant.
  if (ctx->num) return new PhaseExpr(item, kInt, ctx->num->value);
...
void MdlVisitor::VisitFusStatement(ReferenceList *refs, FusStatementCtx *ctx) {
  ...
  for (auto flag : ctx->fus_attribute()) {
    if (flag->getText() == "BeginGroup")  flags |= RefFlags::kBeginGroup;
    else if (flag->getText() == "EndGroup") flags |= RefFlags::kEndGroup;
    else if (flag->getText() == "SingleIssue") flags |= RefFlags::kSingleIssue;
    else if (flag->getText() == "RetireOOO") flags |= RefFlags::kRetireOOO;
  }
...
PipePhases *MdlVisitor::VisitPipeDef(PipeDefCtx *ctx) {
  ...
  if (auto *protect = ctx->protection()) {
    if (protect->UNPROTECTED()) is_protected = false;
    if (protect->HARD()) is_hard = true;
  }
  Identifier *name = VisitIdent(ctx->ident());
...
PhaseNameList *MdlVisitor::VisitPipePhases(PipePhasesCtx *ctx,
  ...
  for (auto *phase_ctx : ctx->phase_id()) {
    ...
    if (is_first && exe_phase == nullptr) exe_phase = phaseset->front();
...
PhaseNameList *MdlVisitor::VisitPhaseId(PhaseIdCtx *ctx, bool is_protected,
  ...
  if (!ctx->range()) {
    ...
    if (first > last) first = last;  // silly, but sanity check it.
  ...
  if (ctx->number()) phases->front()->set_index(ctx->number()->value);
...
//-----------------------------------------------------------------------------
ResourceRefList *MdlVisitor::VisitLatencyResourceRefs(
    LatencyResourceRefsCtx *ctx) {
  auto *refs = new ResourceRefList;
  if (ctx == nullptr) return refs;
...
ResourceRef *MdlVisitor::VisitLatencyResourceRef(LatencyResourceRefCtx *ctx) {
  ...
  if (value_mask) res->set_value_name(value_mask);
  if (ctx->countname) res->set_pool_count_name(countname);
  if (ctx->count) res->set_pool_count(count);
  if (ctx->all) res->set_use_all_members();
  return res;
}
...
// Return a list of resource references.
//-----------------------------------------------------------------------------
ResourceRefList *MdlVisitor::VisitResourceRefs(
    ResourceRefsCtx *ctx, ResourceDefList *resources /* = nullptr */) {
  auto *refs = new ResourceRefList;
...
// Return a single resource reference object.
//-----------------------------------------------------------------------------
ResourceRef *MdlVisitor::VisitResourceRef(
    ResourceRefCtx *ctx, ResourceDefList *resources /* = nullptr */) {
  if (ctx == nullptr) return nullptr;
  Identifier *name = VisitIdent(ctx->name);
  MdlItem item(ctx, current_file_name());
...
IdList *MdlVisitor::VisitNameList(NameListCtx *ctx) {
  ...
  for (auto *name_ctx : ctx->ident()) names->push_back(VisitIdent(name_ctx));
  int index = 0;
  for (auto *name : *names) name->set_index(index++);
  return names;
}
...
void MdlVisitor::VisitSuBaseList(SuBaseListCtx *ctx, IdList *&bases,
  ...
  if (ctx->unit != nullptr) {
    bases = new IdList;
    for (auto *base_ctx : ctx->ident()) bases->push_back(VisitIdent(base_ctx));
  }
...
OperandDecl *MdlVisitor::VisitOperandDecl(OperandDeclCtx *ctx, int opnd_id) {
  ...
  if (name == nullptr) name = new Identifier(item, "");
...
//      | predicate=name_list ':' '{' operand_attribute_stmt* '}' ';'?
//-----------------------------------------------------------------------------
OperandAttributeList *MdlVisitor::VisitOperandAttribute(
    OperandAttributeCtx *ctx) {
...
//-----------------------------------------------------------------------------
OperandAttribute *MdlVisitor::VisitOperandAttributeStmt(
    OperandAttributeStmtCtx *ctx, IdList *predicate) {
  auto *name = VisitIdent(ctx->name);
...
std::vector<int> *MdlVisitor::VisitTuple(TupleCtx *ctx) {
  ...
  for (auto *snumber : ctx->snumber()) values->push_back(snumber->value);
  return values;
}
...
PredValue *MdlVisitor::VisitPredValue(PredValueCtx *ctx) {
  ...
  if (ctx->mask) return new PredValue(item, ctx->mask->value);
...
static PredOp NameToOp(std::string name) {
  ...
  if (predicate_ops->count(name)) return (*predicate_ops)[name];
...
PredExpr *MdlVisitor::VisitPredicateOp(PredicateOpCtx *ctx) {
  ...
  if (ctx->ident()) {
    ...
    if (expr != PredOp::kEmpty) return new PredExpr(item, expr);
  ...
  if (ctx->pred_opcode() == nullptr) return nullptr;
  ...
  for (auto *opnd : ctx->pred_opnd()) opnds.push_back(VisitPredicateOpnd(opnd));
...
PredExpr *MdlVisitor::VisitPredicateOpnd(PredicateOpndCtx *ctx) {
  MdlItem item(ctx, current_file_name());
  if (ctx->name) return new PredExpr(item, PredOp::kName, ctx->name->getText());
  if (ctx->operand())
  ...
  if (ctx->predicate_op()) return VisitPredicateOp(ctx->predicate_op());
