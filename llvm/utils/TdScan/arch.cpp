...
void MachineDescription::PopulateResourceGroup(ProcResource *func) {
  ...
  if (!func->resource_group().empty()) return;
...
void MachineDescription::PopulateCPUsWithFUs() {
  ...
  for (const auto &[name, unit] : proc_resources_) PopulateResourceGroup(unit);
  ...
  for (auto &[name, res] : proc_resources()) CreateSuperUnitNames(name);
}
...
ProcResource *MachineDescription::CreateSuperUnitNames(
    const std::string &unit_name) {
  auto *super = proc_resources_[unit_name];
  auto unit = unit_name;
  if (super->buffer_size() > -1) unit += formatv("<{0}>", super->buffer_size());

  // No need to process a functional unit more than once - we don't necessarily
  // process them in top-down order.
  if (!super->super_names().empty()) return super;
...
void MachineDescription::AddSchedReadWrite(
  ...
  if (!rw_units_.count(name))
    rw_units_[name] =
        new SchedReadWrite(name, sched_model, func_units, res_cycles,
                           start_cycles, latency, micro_ops, is_write,
                           is_begin_group, is_end_group, is_single_issue,
                           retire_ooo);
  else
    rw_units_[name]->AddSchedModel(sched_model, func_units, res_cycles,
                                   start_cycles, latency, micro_ops, is_write,
                                   is_begin_group, is_end_group,
...
bool MachineDescription::IsReadUnit(const std::string &name,
  ...
  if (auto *res = GetSchedReadWrite(name, model)) return res->is_read();
  if (IsSchedVariant(name)) return sched_variants_[name]->is_read();
  return false;
}
...
void MachineDescription::ProcessInstRW() {
  ...
  for (InstRW *instrw : instrw_info_) {
    for (auto regex : instrw->instregex()) {
      ...
      if (!prefix.empty()) begin = instructions_.lower_bound(prefix);
  ...
  for (InstRW *instrw : instrw_info_) {
    ...
    for (const auto &instr : instrw->instrs())
      if (instructions_.count(instr)) instructions_[instr]->add_inst_rw(instrw);
  }
  ...
  for (auto &[name, instr] : instructions_) {
    for (const auto &[sched_model, inst_rw] : instr->inst_rws()) {
      ...
      std::string subunit = formatv("{1}() { latency lat{0}(); }",
                                    lat_id, subunit_base);
#else
...
void ReassociateDisjunctions(std::vector<ResExpr> &exprs) {
  ...
  if (!disjunction.opnds().empty()) exprs.push_back(disjunction);
}
...
bool MachineDescription::IsIssueSlot(const std::string &name) const {
  ...
  if (family() == "R600") {
    if (name == "ALU_W" || name == "ALU_X" ||
        name == "ALU_Y" || name == "ALU_Z" || name == "TRANS") return true;
  }
  return false;
}
...
void MachineDescription::PreprocessItinStages() {
  ...
  for (const auto &[name, stage] : instr_stages_) {
    ...
    for (auto &unit : stage->func_units()) {
      ...
      if (!IsIssueSlot(unit)) stage->clear_issue_flag();
    }
...
void MachineDescription::PreprocessInstrItineraryData() {
  ...
  for (auto [name, itin_data] : instr_itinerary_data_) {
    ...
    for (auto &arg : args) ReassociateDisjunctions(arg.exprs());
...
ItineraryFuncUnit *MachineDescription::add_itinerary_fu(
    InstrItineraryData *itin) {
  auto &stages = itin->stages();
  if (itinerary_fus_.count(stages)) return itinerary_fus_[stages];
