...
class SchedMachineModel {
 public:
  SchedMachineModel(const std::string name, const std::string &issue_width,
                    ...
                    const std::string &high_latency,
                    int micro_op_buffer_size,
                    int mispredict_penalty,
                    const std::string &itineraries, const std::string family)
      : name_(name),
        output_name_(name),
        issue_width_(std::stoi(issue_width)),
        load_latency_(std::stoi(load_latency)),
        high_latency_(std::stoi(high_latency)),
        micro_op_buffer_size_(micro_op_buffer_size),
        mispredict_penalty_(mispredict_penalty),
        itineraries_(itineraries) { fix_up_names(family); }
  ...
  void fix_up_names(const std::string &family) {
    ...
    if (output_name_ == "NoSchedModel") output_name_ = family;
    if (itineraries_ == "NoItineraries") itineraries_ = "";
    size_t itin = output_name_.find("Itineraries");
    if (itin != std::string::npos) output_name_.erase(itin, 11);
    size_t model = output_name_.find("Model");
    if (model != std::string::npos) output_name_.erase(model, 5);
  }
...
class SuperName {
 public:
  ...
  bool operator<(const SuperName &rhs) const {
    if (num_units_ != rhs.num_units_) return num_units_ < rhs.num_units_;
...
class ProcResource {
 public:
  ProcResource(const std::string &name, const std::string &num_instances,
      ...
      : name_(name),
        num_instances_(std::stoi(num_instances)),
        buffer_size_(std::stoi(buffer_size)),
...
struct SchedModelRef {
  ...
  std::string FormatFusFlags() {
    std::string out;
    if (is_begin_group) out += ", BeginGroup";
    if (is_end_group) out += ", EndGroup";
    if (is_single_issue) out += ", SingleIssue";
    if (retire_ooo) out += ", RetireOOO";
    return out;
  }
};
...
class SchedReadWrite {
 public:
  ...
  SchedModelRef *HasModel(const std::string &model) {
    if (sched_model_info_.count(model)) return &(sched_model_info_[model]);
    return nullptr;
  }
 ...
 public:
  MCInstPredicate(const std::string &name, const std::string &function_name,
      ...
      : name_(name),
        ...
        function_mapper_(function_mapper),
        opindex_(opindex),
        immval_(immval),
        predicates_(predicates),
        valid_opcodes_(valid_opcodes),
...
class ResExpr {
 public:
  ...
  std::string ToString(const std::vector<ResExpr> &expr,
    ...
    std::string out;
    for (auto &item : expr) out += (out.empty() ? "" : sep) + item.ToString();
    return out;
  }

  bool operator==(const ResExpr &rhs) const {
    if (op_ != rhs.op()) return false;
    if (op_ == ResOp::kRes) return resource_ == rhs.resource();
    if (opnds_.size() != rhs.opnds_.size()) return false;
    for (unsigned idx = 0; idx < opnds_.size(); idx++)
      if (opnds_[idx] != rhs.opnds_[idx]) return false;
    return true;
  }
  ...
  bool operator<(const ResExpr &rhs) const {
    if (op_ < rhs.op()) return true;
    if (op_ == ResOp::kRes) return resource_ < rhs.resource();
    for (unsigned idx = 0; idx < rhs.opnds_.size(); idx++) {
      if (idx == opnds_.size()) return true;
      if (opnds_[idx] < rhs.opnds_[idx]) return true;
      if (opnds_[idx] > rhs.opnds_[idx]) return false;
    }
    if (opnds_.size() > rhs.opnds_.size()) return false;
    return true;
  ...
  std::string ToString() const {
    if (isRes()) return resource_;
    if (isOr()) return ToString(opnds_, " | ");
    if (isAnd()) return ToString(opnds_, " & ");
    return "nop";
  }
...
class ResArg {
 public:
  ...
  std::string ToString() const {
    ...
    for (auto &expr : exprs_) out += expr.ToString() + " || ";
...
class InstrItineraryData {
 public:
  InstrItineraryData(const std::string name,
      ...
      : name_(name),
        instr_itinerary_class_(instr_itinerary_class),
        num_micro_ops_(num_micro_ops),
        stages_(stages),
        operand_cycles_(operand_cycles),
        bypasses_(bypasses) {}
 ...
 public:
  Instruction(const std::string &name, std::vector<std::string> outs,
      ...
      : name_(name),
        ...
        pseudo_(pseudo),
        generic_(generic) {
    if (itinerary_ == "NoItinerary") itinerary_ = "";
  }
  ...
  std::string opnd_name(unsigned index) const {
    ...
    if (index < flattened_ins_.size()) return opnd_name(flattened_ins_[index]);
    return "";
  }
  ...
  int out_index(unsigned index) const {
    if (index < flattened_outs_.size()) return index;
    if (index - flattened_outs_.size() < implicit_defs_.size())
  ...
  int in_index(unsigned index) const {
    ...
    if (index < ins_size) return index + flattened_outs_.size();
 ...
 public:
  MachineDescription(std::string family, std::string output_name,
      ...
      : family_(family),
        ...
        ignore_subunits_(ignore_subunits),
        no_warnings_(no_warnings),
  ...
  std::vector<std::string> FlattenInstrOperands(
  ...
  ProcessorItineraries *processor_itinerary(std::string &name) {
    if (!processor_itineraries_.count(name)) return nullptr;
  ...
  int add_subunit(const std::string &subunit) {
    if (subunits_.count(subunit)) return subunits_[subunit];
  ...
  int add_subunit_base(const std::string &base) {
    if (subunit_bases_.count(base)) return subunit_bases_[base];
  ...
  int add_latency(const std::string &latency) {
    if (latencies_.count(latency)) return latencies_[latency];
  ...
  std::vector<InstrItineraryData *> &instr_itinerary_class(
