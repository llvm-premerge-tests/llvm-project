...
void MachineDescription::DumpForwardingGraph() {
  ...
  for (const auto &[tuple, latency] : forwarding_network_.graph()) {
    ...
    for (auto lat : latency) std::cout << lat << ",";
    std::cout << ">\n";
  }
}
...
void MachineDescription::DumpInstrForwardingInfo() {
  for (const auto &[name, instr] : instructions_) {
    ...
    for (auto &[sched_model, inst_rw] : instr->inst_rws())
      for (const auto &unit : inst_rw->func_units()) out += unit + ",";
    out += "\n";
...
void MachineDescription::DumpStats() {
  std::cerr << formatv(
      ...
      "Subunits: {7}\n"
      "Latencies: {8}\n",
      instructions_.size(), sched_models_.size(), operands_.size(),
      cpus_.size(), proc_resources_.size(), instrw_info_.size(),
      rw_units_.size(), subunits_.size(), latencies_.size());
}
...
std::string MachineDescription::FormatName(const std::string &name) const {
  ...
  if (IsInstruction(name)) out += ".I";
  if (IsOperand(name)) out += ".O";
  if (IsProcessorModel(name)) out += ".C";
  if (IsSchedModel(name)) out += ".SM";
  if (IsProcResource(name)) out += ".FU";
  if (IsSchedReadWrite(name)) out += ".RW";
  if (IsSchedVariant(name)) out += ".SVS";
  if (IsSchedVar(name)) out += ".sv";
  if (IsMCSchedPredicate(name)) out += ".MSP";
  if (IsSchedPredicate(name)) out += ".SP";
  if (IsMCInstPredicate(name)) out += ".MIP";
  if (IsSchedAliasMatch(name)) out += ".AM";
  if (IsWriteSequence(name)) out += ".WS";
  if (IsProcessorItinerary(name)) out += ".PI";
  if (IsInstrItineraryClass(name)) out += ".IC";
  if (IsInstrItineraryData(name)) out += ".ID";
  if (IsInstrStage(name)) out += ".IS";
  if (IsBypass(name)) out += ".BP";
  return out;
}
...
std::string Instruction::ToString(MachineDescription *md) {
  ...
  if (!flattened_outs().empty()) {
    out += "  Flattened outs: ";
    for (const auto &opnd : flattened_outs()) out += opnd + ", ";
    out += "\n";
  }
  if (!flattened_ins().empty()) {
    out += "  Flattened ins: ";
    for (const auto &opnd : flattened_ins()) out += opnd + ", ";
    out += "\n";
  }
  ...
  for (auto &[type, name, op_id] : opnds)
    if (op_id > last_opnd_idx) last_opnd_idx = op_id;
  ...
  if (!itinerary_.empty()) {
    for (auto *itin : md->instr_itinerary_class(itinerary_)) {
      ...
      if (cycle_size) {
        ...
        if (last_opnd_idx >= cycle_size) out += " Too few operand cycles.";
...
std::string SchedMachineModel::ToString(const MachineDescription *md) const {
  auto out = formatv(
      "SchedMachineModel {0}, issue_width={1}, load_latency={2}, "
  ...
  for (auto &cpu : cpus_) out += formatv("{0}, ", cpu);

  out += "\n     FUS: ";
  for (const auto &fu : func_units_) out += formatv("{0}, ", fu);
  return out + "\n\n";
}
...
std::string InstrStage::ToString(const MachineDescription *md) const {
  ...
  for (const auto &func_unit : func_units_) out += formatv("{0}, ", func_unit);
  out += formatv("], timeinc={0}", timeinc_);

  if (!resource_expr_.isNop()) out += ",  " + resource_expr_.ToString();
  return out + "\n";
}
...
std::string SchedVariant::ToString(MachineDescription *md,
  ...
  out += formatv("={0}, schedModel={1}, ",
                 md->FormatName(name_),
                 sched_model_);

  if (is_variadic()) out += "Variadic ";
  out += "Variants[=";
...
std::string ProcResource::ToString(MachineDescription *md) const {
  auto out = formatv(
      "Funcunit {0}<{1}>, {2} super={3} is resource grp={4} "
      "Buffer Size={5}",
      name_, num_instances_, sched_model_, fu_super_, is_proc_res_group_,
      buffer_size_);
  if (!group_names_.empty()) {
    out += ", Resources=[";
    for (const auto &fu : group_names_) out += formatv("{0} ", fu);
    out += "]";
  }
  return out + "\n";
}
...
std::string SchedReadWrite::ToString(MachineDescription *md,
  ...
  if (all_info) {
    ...
    for (auto &[sched_model, sched_model_info] : sched_model_info_) {
      ...
      if (sched_model_info.is_begin_group) unit += " BeginGroup";
      if (sched_model_info.is_end_group) unit += " EndGroup";
      if (sched_model_info.is_single_issue) unit += " SingleIssue";
      if (sched_model_info.retire_ooo) unit += " RetireOOO";
  ...
  for (auto &[model, info] : sched_model_info_) {
    ...
    if (func_units.size() == 0 || micro_ops == (int)func_units.size()) continue;
    ...
    for (auto &unit : func_units) {
      ...
      if (md->IsProcResourceGroup(unit))
        out += formatv("({0})",
                       md->proc_resources()[unit]->group_names().size());
    ...
    out += "] \t[ ";
    for (auto cycles : res_cycles) out += formatv("{0} ", cycles);
    if (!start_cycles.empty()) {
      out += "] \t@[ ";
      for (auto cycles : start_cycles) out += formatv("{0} ", cycles);
    }
    out += formatv("] \t{0}", micro_ops);
    if (info.is_begin_group) out += " BeginGroup";
    if (info.is_end_group) out += " EndGroup";
    if (info.is_single_issue) out += " SingleIssue";
    if (info.retire_ooo) out += " RetireOOO";

    std::string com;
    if (!res_cycles.empty() && res_cycles.size() != func_units.size())
      com += formatv(" F{0}{2}R{1}",
                func_units.size(), res_cycles.size(),
                func_units.size() < res_cycles.size() ? "<" : ">");
    if (res_cycles.empty() && micro_ops != 0 &&
        func_units.size() != (unsigned)micro_ops)
      com += formatv(" F{0}{2}M{1}",
                func_units.size(), micro_ops,
                (int)func_units.size() < micro_ops ? "<" : ">");
    if (!res_cycles.empty() && micro_ops != 0) {
      int sum_cycles = 0;
      for (auto &cycles : res_cycles) sum_cycles += cycles;
      if (sum_cycles != micro_ops)
        com += formatv(" R{0}{2}M{1}",
                  sum_cycles, micro_ops,
                  sum_cycles < micro_ops ? "<" : ">");
    }
    if (!com.empty()) out += "\t   ///" + com;
    out += "\n";
  }
...
std::string MCSchedPredicate::ToString(MachineDescription *md) const {
  ...
  if (!sched_model_.empty()) out += formatv(": SchedModel={0}", sched_model_);
...
std::string MCInstPredicate::ToString(MachineDescription *md,
                                      int indent) const {
  std::string out;
  if (indent == 0 && str_contains(name_, "anonymous_")) return "";
  ...
  if (name_ == kTrue || name_ == kFalse) return out + "\n";

  out += "[";
  for (auto &attr : attributes_) out += formatv("{0},", attr);
  out += "]";

  if (!function_name_.empty()) out += formatv(", FN={0}", function_name_);
  if (!function_mapper_.empty())
    out += formatv(", FuncMapper={0}", function_mapper_);
  if (!opindex_.empty()) out += formatv(", opindex={0}", opindex_);
  if (!immval_.empty()) out += formatv(", immval={0}", immval_);
  if (!register_name_.empty()) out += formatv(", Register={0}", register_name_);
  if (!valid_opcodes_.empty()) {
    out += ", valid_opcodes=[";
    for (auto &opcode : valid_opcodes_) out += formatv("{0},", opcode);
    out += "]";
  }
  ...
  if (!predicates_.empty()) {
    for (auto &pred : predicates_) out += MCPredFormat(pred, md, indent + 1);
  }
  return out;
}
...
std::string MCStatement::ToString(MachineDescription *md, int indent) const {
  ...
  if (!predicate_.empty()) out = MCPredFormat(predicate_, md, indent + 1);

  if (!cases_.empty()) {
    PredIndent(&out, indent);
    out += "  Cases=\n";
    for (auto &opcode : cases_) out += MCPredFormat(opcode, md, indent + 2);
  }

  if (!default_.empty()) out += MCPredFormat(default_, md, indent + 1);
  return out;
}
...
std::string MCOpcodeSwitchCase::ToString(MachineDescription *md,
  ...
  PredIndent(&out, indent);
  out += "  Opcodes=[";
  for (auto &opcode : cases_) out += formatv("{0},", opcode);
  out += "]\n";
...
std::string InstRW::ToString(const MachineDescription *md, bool full) const {
  ...
  for (const auto &regex : instregex_) out += formatv("\"{0}\",", regex);
  out += "], instrs[";
  if (full)
    for (const auto &instr : instrs_) out += formatv("{0},", instr);
...
std::string InstRW::FwdString(const MachineDescription *md) const {
  if (forwarding_info_.empty()) return "";

  std::string out;
  for (auto [res_id, latency, def, use, pred] : forwarding_info_) {
    if (pred == "NoSchedPred") pred = "";
...
std::string ItinRW::ToString(const MachineDescription *md) const {
  ...
  for (const auto &rw_unit : rw_units_)
    out += formatv("{0},", md->FormatName(rw_unit));
  out += "]\n    Itin Classes=[";
  for (const auto &unit : itin_classes_) out += unit + ",";
  return out + "]\n";
}
...
std::string ProcessorItineraries::ToString(const MachineDescription *md) const {
  ...
  for (const auto &resource : resources_) out += formatv("{0}, ", resource);
  out += "], Bypass=[";
  for (const auto &bypass : bypass_) out += formatv("{0}, ", bypass);

  out += "], IID=[";
  for (const auto &iid : iid_) out += formatv("{0}, ", iid);
  return out + "]\n\n";
}
...
std::string InstrItineraryData::ToString(MachineDescription *md) const {
  ...
  for (const auto &operand_cycle : operand_cycles_)
    out += formatv("{0}, ", operand_cycle);
  out += "], bypasses=[";
  for (const auto &bypass : bypasses_) out += formatv("{0}, ", bypass);

  out += "], stages=[";
  for (const auto &stage : stages_) out += formatv("{0}, ", stage);
  out += "]\n";
  ...
  if (!issue_stages_.empty()) {
    out += "    ==> Issue resources: ";
    for (auto &expr : issue_stages_) out += expr.ToString() + " || ";
...
void MachineDescription::DumpProcessorModel() {
  ...
  for (const auto &[name, cpu] : cpus_) cpu->Dump(this);
}
...
void MachineDescription::DumpSchedMachineModel() {
  ...
  for (const auto &[name, model] : sched_models_) model->Dump(this);
}
...
void MachineDescription::DumpFUInfo() {
  ...
  for (const auto &[name, fu] : proc_resources_) fu->Dump(this);
}
...
void MachineDescription::DumpSchedReadWrite(bool all_info) {
  ...
  for (const auto &[name, rwunit] : rw_units_) rwunit->Dump(this, all_info);
}
...
void MachineDescription::DumpInstRW() {
  ...
  for (const auto *instrw : instrw_info_) instrw->Dump(this, true);
}
...
void MachineDescription::DumpForwardingInfo() {
  ...
  for (auto *instrw : instrw_info_)
    if (!instrw->forwarding_info().empty()) count++;

  DumpHeader("Forwarding Descriptions", count);
  for (auto *instrw : instrw_info_)
    if (!instrw->forwarding_info().empty()) instrw->Dump(this, false);
}
...
void MachineDescription::DumpItinRW() {
  ...
  for (const auto *itinrw : itinrw_info_) itinrw->Dump(this);
}
...
void MachineDescription::DumpSchedVariant() {
  ...
  for (const auto &[name, variant] : sched_variants_) variant->Dump(this);
}
...
void MachineDescription::DumpSchedVar() {
  ...
  for (const auto &[name, var] : sched_vars_) var->Dump(this);
}
...
void MachineDescription::DumpPredicates() {
  ...
  for (const auto &[name, sp] : mc_sched_predicates_) sp->Dump(this);

  DumpHeader("SchedPredicate Descriptions", sched_predicates_.size());
  for (const auto &[name, sp] : sched_predicates_) sp->Dump(this);

  DumpHeader("MCInstPredicate Descriptions", mc_inst_predicates_.size());
  for (const auto &[name, fp] : mc_inst_predicates_) fp->Dump(this);

  DumpHeader("MCStatement Descriptions", statements_.size());
  for (const auto &[name, stmt] : statements_) stmt->Dump(this);

  DumpHeader("MCOpcodeSwitchCase Descriptions", switch_cases_.size());
  for (const auto &[name, stmt] : switch_cases_) stmt->Dump(this);
}
...
void MachineDescription::DumpSchedAlias() {
  ...
  for (const auto &[name, alias] : sched_alias_) alias->Dump(this);
}
...
void MachineDescription::DumpInstrStage() {
  ...
  for (const auto &[name, instr_stage] : instr_stages_) instr_stage->Dump(this);
}
...
void MachineDescription::DumpInstrItineraryClasses() {
  ...
  for (const auto &[name, members] : instr_itinerary_class_) {
    ...
    for (auto *member : members) member->Dump(this);
    std::cout << "\n";
  }
}
...
void MachineDescription::DumpBypasses() {
  ...
  for (const auto &name : bypasses_) std::cout << name << ", ";
  std::cout << "\n";
}
...
void MachineDescription::DumpInstructions() {
  ...
  for (const auto &[name, instr] : instructions_) instr->Dump(this);
}
