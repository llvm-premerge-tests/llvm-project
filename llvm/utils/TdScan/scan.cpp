...
cl::opt<std::string> family_name(
    "family_name", cl::init(""), cl::value_desc("family"),
...
cl::opt<bool> gen_arch_spec("gen_arch_spec", cl::init(false),
...
cl::opt<bool> generate_base_subunits("gen_base_subunits", cl::init(false),
                      cl::desc("Generate instruction bases for each subunit"));
cl::opt<bool> generate_forwarding_info("gen_forwarding", cl::init(false),
                      cl::desc("Generate forwarding information each CPU"));
cl::opt<bool> generate_operand_indexes("gen_indexes", cl::init(false),
...
int main(int argc, char **argv) {
  ...
  if (gen_arch_spec) ignore_subunits = true;
  mpact::mdl::scan::MachineDescription md_info(family_name, output_name,
                                               ignore_subunits, no_warnings,
                                               generate_base_subunits,
                                               generate_forwarding_info,
  ...
  if (gen_arch_spec) {
    ...
    if (dump_debug) md_info.DumpTableGenInfo();
    if (dump_forwarding) md_info.DumpForwardingGraph();
    if (dump_instr_forwarding) md_info.DumpInstrForwardingInfo();
  }
...
char *MachineDescription::GetLine(std::ifstream &in, std::string &input) {
  ...
  if (found != std::string::npos) return &input[found];
  return &input[0];
}
...
StringVec MachineDescription::ScanList(char *input) {
  ...
  while (char *name = strtok_r(input, " ,]);", &input)) result.push_back(name);
  return result;
}
...
StringSet MachineDescription::ScanSet(char *input) {
  ...
  while (char *name = strtok_r(input, " ,]);", &input)) result.insert(name);
  return result;
}
...
std::string MachineDescription::ScanCode(std::ifstream &in, std::string code) {
  ...
  auto it = code.find("}]");
  if (it != std::string::npos) return code.substr(2, it - 2);
...
std::vector<std::string> MachineDescription::ScanInstructOpnds(char *input) {
  ...
  while (char *opnd = strtok_r(input, " ,);", &input)) {
    ...
    if (char *colon = strchr(opnd, ':')) *colon = ' ';
    if (char *dollar = strchr(opnd, '$')) *dollar = ' ';
...
void MachineDescription::ScanOperand(std::ifstream &in,
  ...
  //---------------------------------------------------------------------
  if (ops.size() == 1 && ops[0] == name) ops.clear();
...
void MachineDescription::ScanProcessorModel(std::ifstream &in) {
  ...
  if (!itineraries.empty() && itineraries != "NoItineraries") {
    ...
    if (sched_models_.count(itineraries) == 0)
      sched_models_[itineraries] = new SchedMachineModel(
                                          itineraries, itineraries, family());
...
void MachineDescription::ScanSchedMachineModel(std::ifstream &in,
  ...
  sched_models_[name] = new SchedMachineModel(
      name, issue_width, load_latency, high_latency, micro_op_size,
      mispredict, itineraries, family());
}
...
void MachineDescription::ScanSchedVariant(std::ifstream &in,
  ...
  sched_variants_[name] = new SchedVariant(name, sched_model, variants,
                                           strstr(paren, " SchedWriteVariant"),
                                           is_variadic);
}
...
void MachineDescription::ScanMCInstPredicate(std::ifstream &in,
  ...
  for (auto &attr : pred_attributes)
    if (strstr(paren, attr.c_str())) attributes.push_back(attr);
...
void MachineDescription::ScanMCStatement(std::ifstream &in,
  ...
  if (strstr(paren, "MCReturnStatement")) attribute = "ReturnStatement";
  if (strstr(paren, "MCOpcodeSwitchStatement")) attribute = "OpcodeSwitch";
...
void MachineDescription::ScanInstruction(std::ifstream &in,
  ...
  if (name_space != family() && name_space != "TargetOpcode") return;
  ...
  //---------------------------------------------------------------------
  auto inst = new Instruction(name, outs, ins, assy, pat, parent, subunit,
                              rw_units, implicit_uses, implicit_defs,
                              base_instr, itinerary, load, pseudo,
                              strstr(paren, "GenericInstruction"));
  if (parent.empty())
