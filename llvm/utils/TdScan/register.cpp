...
RegSet MachineDescription::ParseExpr(char *&input) {
  ...
  *(input += len) = delim;
  ...
  if (register_dict_.count(name)) regs.emplace_back(name);
...
RegSet MachineDescription::RegClassSub(char *&input) {
  ...
  while (input[0] == ',') {
    ...
    for (auto &item : sub) {
      ...
      if (found != regs.end()) regs.erase(found, found + 1);
    }
  }
  input = strchr(input, ')') + 1;
  return regs;
...
RegSet MachineDescription::RegClassShl(char *&input) {
  auto regs = ParseExpr(input);
  unsigned count;
  if (sscanf(input, ",%u)", &count) != 1) return regs;
  input = strchr(input, ')') + 1;

  if (count > regs.size()) count = regs.size();
...
RegSet MachineDescription::RegClassRotl(char *&input) {
  ...
  if (count >= regs.size()) return regs;
  auto rot = regs;
...
RegSet MachineDescription::RegClassTrunc(char *&input) {
  ...
  if (count < regs.size()) regs.resize(count);  // truncate to first n regs
  return regs;
}
...
RegSet MachineDescription::RegClassSequence(char *&input) {
  ...
  char format[100];
  unsigned first, last;
  if (sscanf(input, "\"%s %u,%u)", format, &first, &last) != 3) return regs;
  if (last < first) return regs;
  input = strchr(input, ')') + 1;
...
RegSet MachineDescription::GetMemberList(RegisterClass *reg_class) {
  if (reg_class->parsed()) return reg_class->registers();
...
std::string GetRegisterPrefix(const std::string &reg, int &regid) {
  ...
  while (std::isdigit(reg.at(suffix - 1))) suffix--;
