...
bool OperandTypeChecker::check(InstructionPattern *P) {
  ...
  for (auto &O : P->named_operands()) {
    const Record *Ty = O.getType();
    if (!Ty)
      continue;

    auto &Info = Types[O.getOperandName()];
    ...
    if (Info.Type != Ty) {
      PrintError(DiagLoc, "conflicting types for operand '" +
                              O.getOperandName() + "': first seen with '" +
...
void OperandTypeChecker::setAllOperandTypes() {
  for (auto *P : Pats) {
    for (auto &O : P->named_operands()) {
      if (auto &Info = Types[O.getOperandName()]; Info.Type)
        O.setType(Info.Type);
    }
  }
}
...
/// Maps operands to their definitions. This allows us to tie different patterns
/// of a PatFrag alternative together.
class PatFragOperandTable {
public:
  bool addPattern(ArrayRef<SMLoc> DiagLoc, InstructionPattern *P,
                  StringRef ErrSuffix) {
    for (const auto &Operand : P->named_operands()) {
      StringRef OpName = Operand.getOperandName();
      auto &Def = Table[OpName]; // Always create an entry

      if (!Operand.isDef())
        continue;

      if (Def) {
        PrintError(DiagLoc, "Operand '" + OpName +
                                "' is defined multiple times " + ErrSuffix);
        return false;
      }
  ...
  const InstructionPattern *getDef(StringRef OpName) const {
    if (auto It = Table.find(OpName); It != Table.end())
      return It->second;
    return nullptr;
  }

  bool contains(StringRef OpName) const { return Table.contains(OpName); }

  // TODO: print/dump ?

private:
  MapVector<StringRef, InstructionPattern *> Table;
};
...
unsigned PatFrag::getParamIdx(StringRef Name) const {
  for (const auto &[Idx, O] : enumerate(Params)) {
    if (O.Name == Name)
      return Idx;
  }
...
bool PatFrag::checkSemantics() {
  ...
  StringSet<> SeenOps;
  for (const auto &O : in_params()) {
    if (SeenOps.count(O.Name)) {
      PrintError("duplicate parameter '" + O.Name + "'");
      return false;
    }

    // Check this operand is NOT defined in any alternative's patterns.
    for (const auto &A : Alts) {
      if (A.OpTable.getDef(O.Name)) {
        PrintError("input parameter '" + O.Name + "' cannot be redefined!");
        return false;
      }
    }

    if (O.Kind == PK_Root) {
      PrintError("input parameterr '" + O.Name + "' cannot be a root!");
      return false;
    }

    SeenOps.insert(O.Name);
  }

  for (const auto &O : out_params()) {
    if (O.Kind != PK_Root && O.Kind != PK_MachineOperand) {
      PrintError("output parameter '" + O.Name + "' must be 'root' or 'gi_mo'");
      return false;
    }
    ...
    for (const auto &A : Alts) {
      if (!A.OpTable.getDef(O.Name)) {
        PrintError("output parameter '" + O.Name +
...
void PatFrag::print(raw_ostream &OS, StringRef Indent) const {
  ...
  for (const auto &A : Alts) {
    OS << Indent << "    [\n";
    for (const auto &P : A.Pats) {
      OS << Indent << "      ";
      P->print(OS, /*PrintName=*/true);
      OS << ",\n";
    }
    OS << Indent << "    ],\n";
  }
...
bool PatFragPattern::mapInputCodeExpansions(const CodeExpansions &ParentCEs,
  ...
  for (const auto &[Idx, O] : enumerate(operands())) {
    ...
    if (O.hasImmValue()) {
      PatFragCEs.declare(ParamName, to_string(O.getImmValue()));
      continue;
    }
...
/// The RuleOperandTable holds information about the operands of the match &
/// apply patterns of a Rule.
///
/// It's a bit more complicated than just mapping an operand name to a Pattern
/// because it needs to keep track of both Match & Apply at the same time.
///
/// TODO: Use a pair of simpler structures like PatFragOperandTable?
class RuleOperandTable {
public:
  struct Entry {
    ...
    CodeGenInstructionPattern *ApplyPat = nullptr;
    // If MatchPat is null, whether this is a live-in of the match pattern.
    bool IsMatchLiveIn = false;
  };

  bool addMatchPattern(ArrayRef<SMLoc> DiagLoc, InstructionPattern *P);
  bool addApplyPattern(ArrayRef<SMLoc> DiagLoc, CodeGenInstructionPattern *P);

  auto begin() const { return Table.begin(); }
  auto end() const { return Table.end(); }
  auto find(StringRef OpName) const { return Table.find(OpName); }

  InstructionPattern *getMatchDefPattern(StringRef OpName) const {
    if (auto It = Table.find(OpName); It != Table.end())
      return It->second.MatchPat;
    return nullptr;
  }

  CodeGenInstructionPattern *getApplyDefPattern(StringRef OpName) const {
    if (auto It = Table.find(OpName); It != Table.end())
      return It->second.ApplyPat;
    return nullptr;
  }

  bool isMatchLiveInOrDef(StringRef OpName) const;

  void print(raw_ostream &OS, StringRef Indent = "") const;

private:
  MapVector<StringRef, Entry> Table;
};

bool RuleOperandTable::addMatchPattern(ArrayRef<SMLoc> DiagLoc,
                                       InstructionPattern *P) {
  for (const auto &Operand : P->named_operands()) {
    StringRef OpName = Operand.getOperandName();
    auto &Entry = Table[OpName]; // Always create an entry.
    ...
    if (Operand.isDef()) {
      ...
      Entry.MatchPat = P;
      Entry.IsMatchLiveIn = false;
    } else if (!Entry.MatchPat) {
      Entry.IsMatchLiveIn = true;
    }
  }
...
bool RuleOperandTable::addApplyPattern(ArrayRef<SMLoc> DiagLoc,
                                       CodeGenInstructionPattern *P) {
  for (const auto &Operand : P->named_operands()) {
    StringRef OpName = Operand.getOperandName();
    auto &Entry = Table[OpName]; // Always create an entry.

    // TODO: isNamedImmediate needed?
    if (Operand.isNamedImmediate() || !Operand.isDef())
      continue;

    if (!Entry.MatchPat && Entry.IsMatchLiveIn) {
      PrintError(DiagLoc, "Cannot define live-in operand '" + OpName +
...
bool RuleOperandTable::isMatchLiveInOrDef(StringRef OpName) const {
  const auto It = Table.find(OpName);
  if (It == Table.end())
    return false;
  const auto &Entry = It->second;
  return Entry.MatchPat || Entry.IsMatchLiveIn;
}

void RuleOperandTable::print(raw_ostream &OS, StringRef Indent) const {
  // TODO: Rename
  OS << Indent << "(RuleOperandTable ";
  if (Table.empty()) {
    OS << "<empty>)\n";
    return;
  }
...
private:
  ...
  // Operand table for the whole rule.
  RuleOperandTable RuleOpTable;
...
void CombineRuleBuilder::print(raw_ostream &OS) const {
  ...
  RuleOpTable.print(OS, /*Indent=*/"  ");

  if (PermutationsToEmit.size() > 1) {
    OS << "  (PermutationsToEmit\n";
    for (const auto &P : PermutationsToEmit) {
      OS << "    ";
      print(OS, P);
      OS << ",\n";
    }
...
void CombineRuleBuilder::verify() const {
  ...
  for (const auto &[Name, Op] : RuleOpTable) {
    ...
    if (Op.MatchPat && Op.IsMatchLiveIn) {
      dump();
      PrintFatalError(
          "Operand " + Name +
          " is defined in match pattern but marked as match-live-in");
    }
  }
...
RuleMatcher &CombineRuleBuilder::addRuleMatcher(const PatternAlternatives &Alts,
  ...
  if (!AdditionalCommentStr.empty())
    CommentOS << "; " << AdditionalCommentStr;
...
bool CombineRuleBuilder::findRoots() {
  const auto Finish = [&]() {
    ...
    for (unsigned K = 0; K < IPRoot->getNumInstDefs(); ++K) {
      ...
      auto *ApplyRedef = RuleOpTable.getApplyDefPattern(Name);
      if (!ApplyRedef) {
...
bool CombineRuleBuilder::buildRuleOperandsTable() {
  ...
  for (auto &P : values(MatchPats)) {
    ...
    if (IP && !RuleOpTable.addMatchPattern(RuleDef.getLoc(), IP))
      return false;
  }

  for (auto &P : values(ApplyPats)) {
    // Only CodeGenInstructionPatterns should be in the "apply" pattern.
    auto *IP = dyn_cast<CodeGenInstructionPattern>(P.get());
    if (IP && !RuleOpTable.addApplyPattern(RuleDef.getLoc(), IP))
      return false;
  }
...
bool CombineRuleBuilder::emitMatchPattern(CodeExpansions &CE,
  ...
  const auto FindOperandDef = [&](StringRef Op) -> InstructionPattern * {
    return RuleOpTable.getMatchDefPattern(Op);
  };
...
bool CombineRuleBuilder::emitCodeGenInstructionApplyPattern(
  ...
  for (auto &O : P.named_operands()) {
    ...
    StringRef OpName = O.getOperandName();
    if (const auto *DefPat = RuleOpTable.getApplyDefPattern(OpName)) {
      if (!emitCodeGenInstructionApplyPattern(CE, M, *DefPat, SeenPats,
                                              OperandToTempRegID))
        return false;
    } else {
      // If we have no def, check this exists in the MatchRoot.
      if (!O.isNamedImmediate() && !RuleOpTable.isMatchLiveInOrDef(OpName)) {
  ...
  for (auto &O : P.operands()) {
    if (O.isNamedImmediate()) {
      PrintError("invalid output operand '" + O.getOperandName() +
    ...
    if (O.hasImmValue()) {
      if (!emitCodeGenInstructionApplyImmOperand(M, DstMI, P, O))
        return false;
      continue;
    }
    ...
    if (!O.isDef()) {
      if (auto It = OperandToTempRegID.find(OpName);
        ...
        // If not, it means the register hasn't been allocated.
        assert(RuleOpTable.isMatchLiveInOrDef(OpName) &&
               !RuleOpTable.getApplyDefPattern(OpName) &&
    ...
    // Redefining a register defined by a matched instruction.
    if (const auto *MatchDef = RuleOpTable.getMatchDefPattern(OpName)) {
