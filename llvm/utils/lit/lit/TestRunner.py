...
def executeScriptInternal(test, litConfig, tmpBase, commands, cwd):
    ...
    for i, ln in enumerate(commands):
        ...
        if match:
            ...
            # TODO: Previously, this case was reachable for two possible
            # scenarios: preamble_commands (but now we add %dbg for that case),
            # and ALLOW_RETRIES (but we no longer overwrite the original
            # 'commands' array here, so %dbg remains for the retry, and so we
            # avoid accumulating "command line" here at every retry).  Is there
            # any case left, perhaps for some custom format?  If so, drop the
            # assert below and modify this comment to explain why this case
            # still exists.  If not, keep only the assert.
            dbg = "command line"
            command = ln
            assert False, "expected %dbg(...) before command line"
        ln = f"@echo '# {dbg}' "
        if command:
