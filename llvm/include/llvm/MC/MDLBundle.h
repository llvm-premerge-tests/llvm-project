...
SlotSet CpuConfig<CpuParams>::bundleCandidates(const SlotSet *Candidates) {
  ...
  for (auto &Cand : *Candidates) addToBundle(Bundle, Cand, Res);
...
bool CpuConfig<CpuParams>::addToBundle(SlotSet &Bundle,
  ...
  if (Candidate.getSubunits() == nullptr) return true;
...
template <typename CpuParams>
BundleStatus CpuConfig<CpuParams>::attemptToBundle(SlotSet &Bundle,
  ...
  if (WhichSlot == (int)Bundle.size()) return allocatePools(Bundle, Res);
  ...
  if (auto *Refs = WhichSubunit.getUsedResourceReferences()) {
    for (auto const &Ref : ReferenceIter<ResourceRef>(Refs, Slot.getInst())) {
      if (Ref.isUse() && !Ref.isDuplicate() && !Ref.isUnreserved())
        if (res.testSet(Ref.getResourceId(),
                        Ref.getPhase(Slot.getInst()),
                        Ref.getCycles()))
          return false;
    }
  }
...
template <typename CpuParams>
void CpuConfig<CpuParams>::findStaticResources(SlotSet &Bundle,
...
template <typename CpuParams>
BundleStatus CpuConfig<CpuParams>::allocatePools(SlotSet &Bundle,
                                          ReservationsConfig<CpuParams> &Res) {
...
inline bool getOperandValues(PoolRequest &Item, int Count, int Values[]) {
  ...
  if (Inst->isOpndVirtualRegister(OperandId)) return false;
...
template <typename CpuParams>
bool CpuConfig<CpuParams>::allocateResource(PoolRequest &Item,
                                            int Id, int Count,
                                            ReservationsConfig<CpuParams> &Res,
                                            ResourceValues<CpuParams> &Values) {
  ...
  bool Shared =
    Cycles == 1 && Item.isShared() && getOperandValues(Item, Count, OpndValues);
  if (Shared) {
  ...
  for (int Off = 0; Off < Count; Off++)
    if (Res.test(Id + Off, Phase, Cycles)) return false;
  ...
  if (Shared) Values.set(Id, OpndValues, Count);
  return true;
}
...
bool CpuConfig<CpuParams>::allocatePool(PoolRequestSet &Pool,
  ...
  if (Count != 1) {
    if (Last % Count != 0) Last -= Last % Count;
    if (First % Count != 0) First += Count - (First % Count);
  }
...
inline void CpuInfo::dumpBundle(std::string Cpu, std::string Msg,
  ...
  std::cout << "\n"
            << Msg << " " << Cpu << "  " << Bundle.size()
            << " -----------------------------------------\n";
  for (auto &Slot : Bundle) std::cout << dumpSlot("  ", Slot);
}
...
inline std::string CpuInfo::dumpSlot(std::string Msg, SlotDesc &Slot) {
  ...
  if (!Slot.getSubunits()) return Out + "\n";
...
bool CpuConfig<CpuParams>::validateBundle(std::string Cpu, SlotSet &Bundle) {
  ...
  for (auto &Slot : Bundle) {
    if (!Slot.getSubunits()) continue;
    if (auto *Refs = Slot.getSubunit()->getUsedResourceReferences()) {
      for (auto &Ref : ReferenceIter<ResourceRef>(Refs, Slot.getInst())) {
        if (Ref.isFus()) continue;
        int Phase = Ref.getPhase(Slot.getInst());
        int Id = Ref.getResourceId();
        if (Ref.isUnreserved()) continue;      // Skip unreserved resources.
