...
/// we are trying to match. In this pattern, we are only ever selecting between
/// two values: 1) an initial start value \p StartVal of the reduction PHI, and
/// 2) a loop invariant value. If any of lane value in \p Left, \p Right is not
/// equal to \p StartVal, select the loop invariant value. This is done by
/// selecting \p Right iff \p Left is equal to \p StartVal.
