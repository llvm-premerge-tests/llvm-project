...
; RUN: opt -passes='licm' -licm-fpassociation-upper-limit=1 -S < %s | FileCheck %s --check-prefix=LICM_ONLY_CONSTRAINED
; RUN: opt -passes='reassociate,loop-mssa(licm)' -S < %s | FileCheck %s --check-prefix=LICM_AFTER_REASSOCIATE
; RUN: opt -passes='reassociate,loop-mssa(licm)' -licm-fpassociation-upper-limit=1 -S < %s | FileCheck %s --check-prefix=LICM_AFTER_REASSOCIATE_CONSTRAINED
