...
; Negative test: more than one use, https://alive2.llvm.org/ce/z/4QqC-i
define i32 @neg_sub_both_nsw_other_use(i32 %x, i32 %y) {
; CHECK-LABEL: @neg_sub_both_nsw_other_use(
; CHECK-NEXT:    [[SUB:%.*]] = sub nsw i32 [[X:%.*]], [[Y:%.*]]
; CHECK-NEXT:    [[NEG:%.*]] = sub nsw i32 0, [[SUB]]
; CHECK-NEXT:    call void @foo(i32 [[SUB]])
; CHECK-NEXT:    ret i32 [[NEG]]
;
  %sub = sub nsw i32 %x, %y
  %neg = sub nsw i32 0, %sub
  call void @foo (i32 %sub)
  ret i32 %neg
}

; Negative test: only process neg sub sequences, https://alive2.llvm.org/ce/z/Q_MiP4
define i32 @neg_and_shl_sub(i32 %x, i32 %y) {
; CHECK-LABEL: @neg_and_shl_sub(
; CHECK-NEXT:    [[NEG:%.*]] = sub i32 0, [[X:%.*]]
; CHECK-NEXT:    [[SHL_NEG:%.*]] = shl i32 -1, [[NEG]]
; CHECK-NEXT:    [[SUB_NEG:%.*]] = sub i32 [[SHL_NEG]], [[Y:%.*]]
; CHECK-NEXT:    ret i32 [[SUB_NEG]]
;
  %neg = sub i32 0, %x
  %shl = shl i32 1, %neg
  %sub = add nsw i32 %shl, %y
  %neg2 = sub nsw i32 0, %sub
  ret i32 %neg2
}
