...
define i1 @or_eq_notY(i8 %x, i8 %y) {
; CHECK-LABEL: @or_eq_notY(
...
define i1 @or_eq_notY_fail_eq_X(i8 %x, i8 %y) {
; CHECK-LABEL: @or_eq_notY_fail_eq_X(
; CHECK-NEXT:    [[NY:%.*]] = xor i8 [[Y:%.*]], -1
; CHECK-NEXT:    [[OR:%.*]] = or i8 [[NY]], [[X:%.*]]
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq i8 [[OR]], [[X]]
; CHECK-NEXT:    ret i1 [[CMP]]
;
  %ny = xor i8 %y, -1
  %or = or i8 %x, %ny
