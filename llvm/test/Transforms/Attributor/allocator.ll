...
; RUN: opt -aa-pipeline=basic-aa -passes=attributor -debug-only=attributor -attributor-manifest-internal  -attributor-annotate-decl-cs  -S < %s | FileCheck %s --check-prefixes=CHECK,TUNIT
; RUN: opt -aa-pipeline=basic-aa -passes=attributor-cgscc -debug-only=attributor -attributor-manifest-internal  -attributor-annotate-decl-cs -S < %s | FileCheck %s --check-prefixes=CHECK,CGSCC
...
;
entry:
  ;TODO The bytes allocated in the malloc can be reduced to the size of i32.
...
; Function Attrs: noinline nounwind uwtable
define dso_local { i64, ptr } @foo_positive_test_not_a_single_start_offset(i32 noundef %val) #0 {
; CHECK: Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(none)
; CHECK-LABEL: define dso_local { i64, ptr } @foo_positive_test_not_a_single_start_offset
...
; Function Attrs: noinline nounwind uwtable
define dso_local void @positive_test_reduce_allocation_size_array() {
; CHECK-LABEL: define dso_local void @positive_test_reduce_allocation_size_array() {
; CHECK-NEXT:  entry:
...
; CHECK-NEXT:    [[CALL:%.*]] = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(15) @.str, i32 noundef [[TMP5]])
; CHECK-NEXT:    ret void
;
entry:
