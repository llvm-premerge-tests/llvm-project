; REQUIRES: asserts
; RUN: opt -passes=loop-vectorize -debug-only=loop-accesses -force-vector-width=4 %s | FileCheck %s
...
; the same store group). This can cause a use-after-free error when we
; incorrectly release a store group.
