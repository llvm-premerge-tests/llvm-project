...
; Test we bail out when using an alloca of a fixed-length vector (VLS) that was
; bitcasted to a scalable vector.
define void @select_alloca_to_svdouble_t() {
; CHECK-LABEL: @select_alloca_to_svdouble_t(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[Z:%.*]] = alloca <16 x half>, align 32
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq i32 0, 0
; CHECK-NEXT:    [[COND:%.*]] = select i1 [[CMP]], ptr [[Z]], ptr null
; CHECK-NEXT:    [[TMP0:%.*]] = load <vscale x 2 x double>, ptr [[COND]], align 16
; CHECK-NEXT:    ret void
;
entry:
  %z = alloca <16 x half>
  ...
  %0 = load <vscale x 2 x double>, ptr %cond, align 16
  ret void
}
