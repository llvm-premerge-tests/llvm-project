...
; RUN: opt < %s -passes=vector-combine -S -mtriple=x86_64 -mattr=AVX2 -S %s | FileCheck %s

;; Whether these tests can be optimized depends on x86_64's TTI.

define void @load_insert_x2_store(ptr %A, i16 %B, i16 %C) {
; CHECK-LABEL: define void @load_insert_x2_store
; CHECK-SAME: (ptr [[A:%.*]], i16 [[B:%.*]], i16 [[C:%.*]]) #[[ATTR0:[0-9]+]] {
; CHECK-NEXT:  entry:
...
; CHECK-SAME: (ptr [[A:%.*]], float [[B:%.*]], float [[C:%.*]]) #[[ATTR0]] {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[T0:%.*]] = load <8 x float>, ptr [[A]], align 32
; CHECK-NEXT:    [[T1:%.*]] = insertelement <8 x float> [[T0]], float [[B]], i32 4
