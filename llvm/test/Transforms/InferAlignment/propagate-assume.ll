...
define i32 @simple_forwardpropagate(ptr %a) {
; CHECK-LABEL: define i32 @simple_forwardpropagate
; CHECK-SAME: (ptr [[A:%.*]]) {
...
define i32 @simple_backpropagate(ptr %a) {
; CHECK-LABEL: define i32 @simple_backpropagate
; CHECK-SAME: (ptr [[A:%.*]]) {
; CHECK-NEXT:    [[LOAD_A:%.*]] = load i32, ptr [[A]], align 32
; CHECK-NEXT:    [[PTRINT:%.*]] = ptrtoint ptr [[A]] to i64
; CHECK-NEXT:    [[MASKEDPTR:%.*]] = and i64 [[PTRINT]], 31
...
define i32 @complex_backpropagate(ptr %a, ptr %b, ptr %c) {
; CHECK-LABEL: define i32 @complex_backpropagate
