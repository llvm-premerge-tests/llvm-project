// RUN: llvm-mc -triple=aarch64 %s --defsym=MCONLY=1 | FileCheck %s --check-prefix=ASM
...
// ASM:          .xword (_g8@AUTH(ia,42))+1
.quad _g8@AUTH(ia,42) + 1

// ASM:          .xword 1+(_g9@AUTH(ia,42))
.quad 1 + _g9@AUTH(ia,42)

// ASM:          .xword (1+(_g10@AUTH(ia,42)))+1
.quad 1 + _g10@AUTH(ia,42) + 1

// ASM:          .xword (_g11@AUTH(ia,42))+(_g12@AUTH(ia,42))
.quad _g11@AUTH(ia,42) + _g12@AUTH(ia,42)
...
.ifdef ERROBJ
// RUN: not llvm-mc -triple=aarch64 -filetype=obj --defsym=ERROBJ=1 %s 2>&1 | \
