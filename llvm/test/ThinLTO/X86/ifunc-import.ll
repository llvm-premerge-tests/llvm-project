...
; RUN:   -r a.bc,foo2.ifunc,pl -r a.bc,use,pl -r a.bc,use2,pl -r a.bc,__cpu_model,lx \
; RUN:   -r b.bc,main,plx -r b.bc,use,l -r b.bc,use2,l
; RUN: llvm-dis < t.1.3.import.bc | FileCheck %s --check-prefix=A
; RUN: llvm-dis < t.2.3.import.bc | FileCheck %s --check-prefix=B --implicit-check-not='@foo.resolver'

; A: define internal nonnull ptr @foo.resolver()
; A: define internal i32 @foo.default.1(i32 %n)

;; The ifunc implementations of foo.ifunc are internal in A, so they cannot be
;; referenced by B. Our implementation actually ensures that the ifunc resolver
...
@foo2.ifunc = ifunc i32 (i32), ptr @foo.resolver

define dso_local i32 @use(i32 %n) local_unnamed_addr {
entry:
  %call = call i32 @foo.ifunc(i32 %n)
  ret i32 %call
}

define dso_local i32 @use2(i32 %n) local_unnamed_addr {
entry:
  %call = call i32 @foo2.ifunc(i32 %n)
  ret i32 %call
}
...
entry:
  ...
  %1 = and i32 %0, 4
  %.not = icmp eq i32 %1, 0
  %foo.popcnt.0.foo.default.1 = select i1 %.not, ptr @foo.default.1, ptr @foo.popcnt.0
  ret ptr %foo.popcnt.0.foo.default.1
}

define internal i32 @foo.popcnt.0(i32 %n) {
entry:
...
define internal i32 @foo.default.1(i32 %n) {
entry:
  %0 = call i32 @llvm.ctpop.i32(i32 %n)
  ret i32 %0
}
