...
; Take the information about non-equality from a nearby assumption intrinsic call - ne
; This test tries to take advantage of the following simplification: icmp eq|ne X, Y -> false|true if X != Y
define i1 @from_assumption_ne(i16 %x) {
; CHECK-LABEL: @from_assumption_ne(
; CHECK-NEXT:    [[NE:%.*]] = icmp ne i16 [[X:%.*]], 56
...
;
  ...
  %res1 = icmp eq i16 %x, 56
  call void @use.i1(i1 %res1)
  %res2 = icmp ne i16 56, %x
  call void @use.i1(i1 %res2)
  %res = or i1 %res1, %res2
  ret i1 %res
}

; Take the information about non-equality from a nearby assumption intrinsic call - eq
; This test tries to take advantage of the following simplification: icmp eq|ne X, Y -> false|true if X != Y
define i1 @from_assumption_eq(i64 %x, i64 %y) {
; CHECK-LABEL: @from_assumption_eq(
; CHECK-NEXT:    [[EQ:%.*]] = icmp eq i64 [[Y:%.*]], [[X:%.*]]
; CHECK-NEXT:    call void @llvm.assume(i1 [[EQ]])
; CHECK-NEXT:    call void @use.i1(i1 true)
; CHECK-NEXT:    call void @use.i1(i1 false)
; CHECK-NEXT:    ret i1 true
;
  ...
  %res1 = icmp eq i64 %x, %y
  call void @use.i1(i1 %res1)
  %res2 = icmp ne i64 %y, %x
  call void @use.i1(i1 %res2)
  %res = or i1 %res1, %res2
  ret i1 %res
}
