...
As a developer, you can get such files by the ``convert-for-testing`` subcommand
of ``llvm-cov``. For example:

.. code-block:: bash

  llvm-cov convert-for-testing main -o main.covmapping

``convert-for-testing`` is a hidden option for internal use only. You can't see
it in any other public documents or in the ``--help`` output of ``llvm-cov``.

The ``.covmapping`` files contain complete information in the original
binary files for code coverage purpose. It can be read back by ``llvm-cov`` as
ordinary binary files:

.. code-block:: bash

  llvm-cov show main.covmapping --instr-profile main.profdata
...
``coverageMapping`` contains the records of the source files. In Version1, only
one record is stored:

``[padding : bytes, coverageMappingData : bytes]``

Version2 relaxes this restriction by encoding the size of
``coverageMappingData`` as a LEB128 number before the data:

``[coverageMappingSize : LEB128, padding : bytes, coverageMappingData : bytes]``

This is the only difference between Version1 and Version2. Since Version2 is a
