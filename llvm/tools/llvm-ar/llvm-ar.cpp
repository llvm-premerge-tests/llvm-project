  ...
  if (OldArchive->hasSymbolTable()) {
    ...
    if (OldArchive->kind() == object::Archive::K_AIXBIG) {
      ...
      if (BigArc->has32BitGlobalSymtab() &&
          Symtab == SymtabWritingMode::Bit32Only)
        return;

      if (BigArc->has64BitGlobalSymtab() &&
          Symtab == SymtabWritingMode::Bit64Only)
        return;
...
static int ranlib_main(int argc, char **argv) {
  ...
  const char *BitModeOption = nullptr;
  bool IsXOption = false;

  if (object::Archive::getDefaultKindForHost() == object::Archive::K_AIXBIG)
    BitModeOption = getenv("OBJECT_MODE");
      ...
      while (!arg.empty()) {
        if (arg.front() == 'D') {
          ...
          if (object::Archive::getDefaultKindForHost() ==
            ...
            if (Xarg[0] == '\0') {
              if (argv[i + 1][0] != '-') {
                BitModeOption = argv[++i];
                IsXOption = true;
              }
            } else {
              BitModeOption = arg.data();
              IsXOption = true;
            }
  ...
  if (object::Archive::getDefaultKindForHost() == object::Archive::K_AIXBIG) {
    if (BitModeOption) {
      BitMode = getBitMode(BitModeOption);
      if (BitMode == BitModeTy::Unknown) {
        if (IsXOption)
          fail("the specified object mode is not valid. Specify -X32, "
               "-X64, -X32_64, or -Xany");
