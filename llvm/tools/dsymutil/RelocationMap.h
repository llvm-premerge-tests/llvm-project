...
class ValidReloc : public Relocation {
public:
  yaml::Hex64 Offset;
  yaml::Hex32 Size;
  yaml::Hex64 Addend;
  ...
  ValidReloc(uint64_t Offset, uint32_t Size, uint64_t Addend,
      ...
      : Relocation(RK_Relocation), Offset(Offset), Size(Size), Addend(Addend),
        SymbolName(SymbolName), SymbolMapping(SymbolMapping) {}

  bool operator<(const ValidReloc &RHS) const { return Offset < RHS.Offset; }

  /// For YAMLIO support.
  ValidReloc() : Relocation(RK_Relocation) {}

  static bool classof(const Relocation *R) {
    return R->getKind() == RK_Relocation;
  }
};
...
public:
  ...
  /// relocation map.
  void addRelocationMapEntry(const Relocation *Relocation) override;
