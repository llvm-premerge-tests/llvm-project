...
template <typename AddressesMapBase>
void DwarfLinkerForBinary::AddressManager<AddressesMapBase>::saveValidRelocs(
    int64_t LinkedOffset, uint64_t StartOffset, uint64_t EndOffset) {
  std::vector<ValidReloc> Relocs =
      getRelocations(ValidDebugInfoRelocs, StartOffset, EndOffset);
  for (ValidReloc R : Relocs) {
    StoredValidDebugInfoRelocs.emplace_back(R.Offset + LinkedOffset, R.Size,
...
void DwarfLinkerForBinary::AddressManager<AddressesMapBase>::addValidRelocs(
  ...
  for (const auto &InfoReloc : StoredValidDebugInfoRelocs) {
    RM->addRelocationMapEntry(dyn_cast<const Relocation>(&InfoReloc));
  }
  // FIXME: Support relocations debug_addr (DWARF5).
}
