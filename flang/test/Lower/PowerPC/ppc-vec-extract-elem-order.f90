! RUN: %flang_fc1 -emit-fir -fno-ppc-native-vector-element-order -triple ppc64le-unknown-linux %s -o - | FileCheck --check-prefixes="FIR" %s
! RUN: %flang_fc1 -emit-llvm -fno-ppc-native-vector-element-order -triple ppc64le-unknown-linux %s -o - | FileCheck --check-prefixes="LLVMIR" %s
