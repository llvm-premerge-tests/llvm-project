...
!CHECK-NEXT: omp.target  map_entries(%[[MAP_ENTRY0]]{{.*}} {
...
!CHECK-NEXT: omp.target  map_entries(%[[MAP_ENTRY1]]{{.*}} {
...
!CHECK: %[[ENTRY:.*]] = omp.map_entry var_ptr(%[[ARG1]] : !fir.ref<!fir.array<10xi32>>)   map_clauses(tofrom, target_param) capture(ByRef) bounds(%[[BOUNDS]]) -> !fir.ref<!fir.array<10xi32>> {name = "sp_write(2:5)"}
