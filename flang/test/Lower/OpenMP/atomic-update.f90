...
!CHECK: %[[BVAL:.*]] = fir.load %[[LOADED_B]] : !fir.ptr<i32>
!CHECK: omp.atomic.update   %[[LOADED_A]] : !fir.ptr<i32> {
!CHECK: ^bb0(%[[ARG:.*]]: i32):
!CHECK:   %[[RESULT:.*]] = arith.addi %[[ARG]], %[[BVAL]] : i32
...
!CHECK:  ^bb0(%[[ARG:.*]]: i32):
!CHECK:    %[[RESULT:.*]] = arith.addi %[[ARG]], %[[CST_1]] : i32
!CHECK:    omp.yield(%[[RESULT]] : i32)
!CHECK:  }
    !$omp atomic
        y = y + 1
...
!CHECK:  ^bb0(%[[ARG:.*]]: i32):
!CHECK:    %[[RESULT:.*]] = arith.subi %[[ARG]], %[[CST_1]] : i32
!CHECK:    omp.yield(%[[RESULT]] : i32)
!CHECK:  }
...
!CHECK:  ^bb0(%[[ARG:.*]]: i32):
!CHECK:   %[[RESULT:.*]] = arith.muli %[[CST_10]], %[[ARG]] : i32
!CHECK:    omp.yield(%[[RESULT]] : i32)
!CHECK:  }
...
!CHECK:  ^bb0(%[[ARG:.*]]: i32):
!CHECK:    %[[RESULT:.*]] = arith.addi %[[CST_10]], %[[ARG]] : i32
!CHECK:   omp.yield(%[[RESULT]] : i32)
!CHECK:  }
