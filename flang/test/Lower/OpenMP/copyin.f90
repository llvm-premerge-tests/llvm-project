...
!CHECK: %[[val_4:.*]] = fir.coordinate_of %[[val_3]], %[[val_c4]] : (!fir.ref<!fir.array<?xi8>>, index) -> !fir.ref<i8>
!CHECK: %[[val_5:.*]] = fir.convert %[[val_4]] : (!fir.ref<i8>) -> !fir.ref<i32>
!CHECK: omp.parallel {
!CHECK: %[[val_6:.*]] = fir.alloca i32 {adapt.valuebyref, pinned}
!CHECK: %[[val_7:.*]] = omp.threadprivate %[[val_1]] : !fir.ref<!fir.array<8xi8>> -> !fir.ref<!fir.array<8xi8>>
!CHECK: %[[val_8:.*]] = fir.convert %[[val_7]] : (!fir.ref<!fir.array<8xi8>>) -> !fir.ref<!fir.array<?xi8>>
!CHECK: %[[val_c0:.*]] = arith.constant 0 : index
!CHECK: %[[val_9:.*]] = fir.coordinate_of %[[val_8]], %[[val_c0]] : (!fir.ref<!fir.array<?xi8>>, index) -> !fir.ref<i8>
!CHECK: %[[val_10:.*]] = fir.convert %[[val_9]] : (!fir.ref<i8>) -> !fir.ref<i32>
!CHECK: %[[val_11:.*]] = fir.convert %[[val_7]] : (!fir.ref<!fir.array<8xi8>>) -> !fir.ref<!fir.array<?xi8>>
!CHECK: %[[val_c4_0:.*]] = arith.constant 4 : index
!CHECK: %[[val_12:.*]] = fir.coordinate_of %[[val_11]], %[[val_c4_0]] : (!fir.ref<!fir.array<?xi8>>, index) -> !fir.ref<i8>
!CHECK: %[[val_13:.*]] = fir.convert %[[val_12]] : (!fir.ref<i8>) -> !fir.ref<i32>
!CHECK: %[[val_14:.*]] = fir.load %[[val_5]] : !fir.ref<i32>
!CHECK: fir.store %[[val_14]] to %[[val_13]] : !fir.ref<i32>
!CHECK: omp.barrier
!CHECK: %[[val_c1_i32:.*]] = arith.constant 1 : i32
!CHECK: %[[val_15:.*]] = fir.load %[[val_10]] : !fir.ref<i32>
!CHECK: %[[val_c1_i32_1:.*]] = arith.constant 1 : i32
!CHECK: omp.wsloop for (%[[val_arg0:.*]]) : i32 = (%[[val_c1_i32]]) to (%[[val_15]]) inclusive step (%[[val_c1_i32_1]]) {
!CHECK: fir.store %[[val_arg0]] to %[[val_6]] : !fir.ref<i32>
!CHECK: %[[val_16:.*]] = fir.load %[[val_13]] : !fir.ref<i32>
!CHECK: %[[val_17:.*]] = fir.load %[[val_6]] : !fir.ref<i32>
!CHECK: %[[val_18:.*]] = arith.addi %[[val_16]], %[[val_17]] : i32
!CHECK: fir.store %[[val_18]] to %[[val_13]] : !fir.ref<i32>
!CHECK: omp.yield
!CHECK: }
!CHECK: omp.terminator
!CHECK: }
