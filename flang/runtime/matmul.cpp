...
template <TypeCategory RCAT, int RKIND, typename XT, typename YT,
    bool X_HAS_STRIDED_COLUMNS = false, bool Y_HAS_STRIDED_COLUMNS = false>
...
inline void MatrixTimesMatrixHelper(CppTypeFor<RCAT, RKIND> *RESTRICT product,
  ...
  if (!xColumnByteStride) {
    if (!yColumnByteStride) {
      MatrixTimesMatrix<RCAT, RKIND, XT, YT>(product, rows, cols, x, y, n);
    } else {
    ...
    if (!yColumnByteStride) {
      MatrixTimesMatrix<RCAT, RKIND, XT, YT, true>(
...
template <TypeCategory RCAT, int RKIND, typename XT, typename YT,
    bool X_HAS_STRIDED_COLUMNS = false>
...
inline void MatrixTimesVectorHelper(CppTypeFor<RCAT, RKIND> *RESTRICT product,
  ...
  if (!xColumnByteStride) {
    MatrixTimesVector<RCAT, RKIND, XT, YT>(product, rows, n, x, y);
  } else {
...
template <TypeCategory RCAT, int RKIND, typename XT, typename YT,
    bool Y_HAS_STRIDED_COLUMNS = false>
...
inline void VectorTimesMatrixHelper(CppTypeFor<RCAT, RKIND> *RESTRICT product,
  ...
  if (!yColumnByteStride) {
    VectorTimesMatrix<RCAT, RKIND, XT, YT>(product, n, cols, x, y);
  } else {
