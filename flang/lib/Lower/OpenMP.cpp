...
// with USM mode.
static bool mapUsesReferencePtr(fir::FirOpBuilder &firOpBuilder,
...
createMapEntryOp(fir::FirOpBuilder &builder, mlir::Location loc,
  ...
  mlir::omp::MapEntryOp op =
      builder.create<mlir::omp::MapEntryOp>(loc, retTy, baseAddr);
  ...
  return findRepeatableClause<ClauseTy::Map>(
      [&](const ClauseTy::Map *mapClause,
        ...
        for (const Fortran::parser::OmpObject &ompObject :
          ...
          // at the moment, but there may also be other special casing that
          // Fortran could utilise.
          llvm::omp::OpenMPOffloadMappingFlags perValMapTypeBit = mapTypeBits;
          bool usesRefPtr = mapUsesReferencePtr(firOpBuilder, baseAddr);
          if (usesRefPtr)
            perValMapTypeBit |=
          ...
          // makes subsequent elements by-pass the target_param flag.
          bool isCaptureFirstInfo = true;
          if (isCaptureFirstInfo && !usesRefPtr)
            perValMapTypeBit |=
          ...
          // Explicit map captures are captured ByRef by default, optimisation
          // passes may alter this to ByCopy or other capture types to optimise
          mapOperands.push_back(createMapEntryOp(
              firOpBuilder, clauseLocation, baseAddr, asFortran, bounds,
              mapType, mlir::omp::VariableCaptureKind::ByRef, false,
              baseAddr.getType()));
        }
      });
}
