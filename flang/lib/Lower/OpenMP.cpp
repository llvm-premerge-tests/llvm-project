...
genOMP(Fortran::lower::AbstractConverter &converter,
  ...
  default: {
    ...
    if ((llvm::omp::allTeamsSet & llvm::omp::blockConstructSet).test(directive.v)) {
      TODO(currentLocation, "Teams construct");
      combinedDirective = true;
    }
    if ((llvm::omp::allParallelSet & llvm::omp::blockConstructSet).test(directive.v)) {
      bool outerCombined = directive.v != llvm::omp::Directive::OMPD_target_parallel;
      genParallelOp(converter, eval, currentLocation, beginClauseList,
                    outerCombined);
      combinedDirective = true;
    }
    if ((llvm::omp::workShareSet & llvm::omp::blockConstructSet).test(directive.v)) {
      TODO(currentLocation, "Workshare construct");
      combinedDirective = true;
    }
