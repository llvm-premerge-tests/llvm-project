...
public:
  ...
  // implemented yet. It triggers a compilation error if the given clauses are
  // found.
...
private:
  ...
  /// Utility to find one of the given clauses within a range in the clause
  /// list.
  template <typename... Ts>
  static ClauseIterator findClause(ClauseIterator begin, ClauseIterator end) {
    for (ClauseIterator it = begin; it != end; ++it) {
      const void *found[sizeof...(Ts)] = {std::get_if<Ts>(&it->u)...};
      for (const void *v : found)
        if (v)
          return it;
    }
...
void ClauseProcessor::processTODO(mlir::Location currentLocation,
                                  llvm::omp::Directive directive) const {
  ClauseIterator it = findClause<Ts...>(clauses.v.begin(), clauses.v.end());
  if (it != clauses.v.end()) {
    TODO(currentLocation,
         "Unhandled clause in " +
