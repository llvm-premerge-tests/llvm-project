...
parseOptimizationRemark(clang::DiagnosticsEngine &diags,
                        llvm::opt::ArgList &args, llvm::StringRef rGroupName) {
  assert((rGroupName == "pass" || rGroupName == "pass-missed" ||
          rGroupName == "pass-analysis") &&
         "Unknown Remark given.");
    ...
    if (a->getOption().matches(clang::driver::options::OPT_R_Joined)) {
      ...
      if (value == rGroupName) {
        ...
        result.Regex = std::make_shared<llvm::Regex>(result.Pattern);
                 ...
                 std::make_pair(llvm::StringRef("no"), rGroupName)) {
