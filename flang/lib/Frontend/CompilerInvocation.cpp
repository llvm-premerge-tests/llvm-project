    ...
    if (a->getOption().matches(clang::driver::options::OPT_R_Joined)) {
      ...
      if (value == name) {
        ...
        result.Regex = std::make_shared<llvm::Regex>(result.Pattern);
                 ...
                 std::make_pair(llvm::StringRef("no"), name)) {
...
static void parseCodeGenArgs(Fortran::frontend::CodeGenOptions &opts,
  ...
  // contain regex values used in optimizationRemarkHandler in
  ...
  // Given a pass, -Rpass-analysis shows when it determined whether or
  // not to make a transformation.
