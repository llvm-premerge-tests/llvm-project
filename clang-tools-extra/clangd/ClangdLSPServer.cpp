...
void ClangdLSPServer::onCommandApplyTweak(const TweakArgs &Args,
  ...
  Inputs.File = Args.file.file().str();
  Inputs.Selection = Args.selection;
  Inputs.RequestedActionKinds = Args.requestedActionKinds;
  Server->applyTweak(Args.tweakID, Inputs, std::move(Action));
}
