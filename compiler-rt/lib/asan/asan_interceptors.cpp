...
static Ret interceptor_strto_base(Fn real, const char *nptr, char **endptr,
                                  int base) {
  void *ctx;
  ASAN_INTERCEPTOR_ENTER(ctx, func);
  ENSURE_ASAN_INITED();
  ...
  char *real_endptr;
  Ret res = real(nptr, &real_endptr, base);
...
#  define INTERCEPTOR_STRTO_BASE(ret_type, func)                               \
    INTERCEPTOR(ret_type, func, const char *nptr, char **endptr, int base) {   \
      return interceptor_strto_base<ret_type>(REAL(func), nptr, endptr, base); \
    }
