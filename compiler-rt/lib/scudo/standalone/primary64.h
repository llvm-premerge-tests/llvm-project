...
public:
  ...
  TransferBatch *popBatch(CacheT *C, uptr ClassId) {
    ...
    if (conditionVariableEnabled()) {
      B = popBatchWithCV(C, ClassId, Region, PrintStats);
    } else {
      while (true) {
        // When two threads compete for `Region->MMLock`, we only want one of
        // them does the populateFreeListAndPopBatch(). To avoid both of them
...
private:
  ...
  TransferBatch *popBatchWithCV(CacheT *C, uptr ClassId, RegionInfo *Region,
                                bool &PrintStats) {
    TransferBatch *B = nullptr;
    ...
    while (true) {
      ...
      // check if someone is still populating freelist.
      ScopedLock FL(Region->FLLock);
