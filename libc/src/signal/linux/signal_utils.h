...
LIBC_INLINE int block_all_signals(sigset_t &set) {
  ...
  return static_cast<int>(__llvm_libc::syscall_impl(
      SYS_rt_sigprocmask, SIG_BLOCK, &full, &set, sizeof(sigset_t)));
}

LIBC_INLINE int restore_signals(const sigset_t &set) {
  return static_cast<int>(__llvm_libc::syscall_impl(
      SYS_rt_sigprocmask, SIG_SETMASK, &set, nullptr, sizeof(sigset_t)));
}
