...
using ExponentWriter = IntegerTo<DecString::WithWidth<2>::WithSign, intmax_t>;
...
public:
  ...
  void write_first_block(BlockInt block, bool exp_format = false) {
    const IntegerTo<DecString, intmax_t> buf(block);
    const cpp::string_view int_to_str(buf);
  ...
  int write_middle_block(BlockInt block) {
    if (block == MAX_BLOCK) { // Buffer max blocks in case of rounding
      ...
      const IntegerTo<DecString, intmax_t> buf(block + (MAX_BLOCK + 1));
      const cpp::string_view int_to_str(buf);
      // TODO: Replace with memcpy
  ...
  int write_last_block_exp(uint32_t block, size_t block_digits,
    ...
    if (round == RoundDirection::Up ||
      ...
      if (has_carry /* && !has_written */) {
        ...
        const ExponentWriter buf(exponent);
        const cpp::string_view int_to_str(buf);
    ...
    RET_IF_RESULT_NEGATIVE(writer->write(exp_char));
    const ExponentWriter buf(exponent);
    RET_IF_RESULT_NEGATIVE(writer->write(static_cast<cpp::string_view>(buf)));
...
LIBC_INLINE int convert_float_dec_exp_typed(Writer *writer,
  ...
  size_t block_width = IntegerTo<DecString, intmax_t>(digits).size();
  ...
  size_t exponent_width =
      IntegerTo<DecString, intmax_t>(positive_exponent).size();
  ...
  if (digits_written == 0) {
    ...
    last_block_size = IntegerTo<DecString, intmax_t>(digits).size();
  }
...
LIBC_INLINE int convert_float_dec_auto_typed(Writer *writer,
  ...
  size_t block_width = 0;
  {
    // TODO: Find a better way to calculate the number of digits in the
    // initial block and exponent.
    block_width = IntegerTo<DecString, intmax_t>(digits).size();
  }
  ...
  if (block_width <= exp_precision + 1) {
    const IntegerTo<DecString, intmax_t> buf(digits);
    const cpp::string_view int_to_str(buf);
