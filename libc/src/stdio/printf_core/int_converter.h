...
using HexFmt = IntegerTo<HexString, uintmax_t>;
using HexFmtUppercase = IntegerTo<HexString::Uppercase, uintmax_t>;
using OctFmt = IntegerTo<OctString, uintmax_t>;
using DecFmt = IntegerTo<DecString, uintmax_t>;
...
num_to_strview(uintmax_t num, cpp::span<char> bufref, char conv_name) {
  if (to_lower(conv_name) == 'x') {
    if (is_lower(conv_name))
      return details::HexFmt::write_to(bufref, num);
    else
      return details::HexFmtUppercase::write_to(bufref, num);
  } else if (conv_name == 'o') {
    return details::OctFmt::write_to(bufref, num);
  } else {
    return details::DecFmt::write_to(bufref, num);
  }
}
