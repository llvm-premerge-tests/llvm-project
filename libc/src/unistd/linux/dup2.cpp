...
LLVM_LIBC_FUNCTION(int, dup2, (int oldfd, int newfd)) {
  ...
  int ret = static_cast<int>(__llvm_libc::syscall_impl(SYS_dup2, oldfd, newfd));
...
#if SYS_fcntl
    int ret =
        static_cast<int>(__llvm_libc::syscall_impl(SYS_fcntl, oldfd, F_GETFD));
#elif defined(SYS_fcntl64)
    // Same as fcntl but can handle large offsets
    static_assert(sizeof(off_t) == 8);
    int ret = static_cast<int>(
        __llvm_libc::syscall_impl(SYS_fcntl64, oldfd, F_GETFD));
#else
#error "SYS_fcntl and SYS_fcntl64 syscalls not available."
  ...
  int ret =
      static_cast<int>(__llvm_libc::syscall_impl(SYS_dup3, oldfd, newfd, 0));
#else
