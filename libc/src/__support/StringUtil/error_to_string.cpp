...
constexpr size_t int_buffer_size() {
  return IntegerTo<DecString, int>::buffer_size();
}
...
constexpr size_t max_buff_size() {
  ...
  return (unknown_str_len + 1 + int_buffer_size()) * sizeof(char);
}
...
cpp::string_view build_error_string(int err_num, cpp::span<char> buffer) {
  ...
  if (buffer.size() < (sizeof("Unknown error") + 1 + int_buffer_size()))
