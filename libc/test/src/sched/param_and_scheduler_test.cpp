...
TEST(LlvmLibcSchedParamAndSchedulerTest, SchedTest) {
  libc_errno = 0;
  ...
  constexpr int policiesRootRequired[] = {SCHED_FIFO, SCHED_RR};
  for (auto policy : policiesRootRequired)
    testSched(policy, /*can_set=*/__llvm_libc::getuid() == 0);

  constexpr int policiesNoRootRequired[] = {SCHED_OTHER, SCHED_BATCH,
                                            SCHED_IDLE};
  for (auto policy : policiesNoRootRequired)
    testSched(policy, /*can_set=*/true);
}
