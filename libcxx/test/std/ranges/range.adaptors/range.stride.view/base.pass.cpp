...
constexpr bool test() {
  ...
  // Check the const& overload
  {
    ...
    std::ranges::stride_view<Range> const view(range, 3);
  ...
  {
    Range range(buff, buff + 8);
    std::ranges::stride_view<Range> view(range, 3);
  ...
  {
    Range range(buff, buff + 8);
    std::same_as<Range> decltype(auto) result = std::ranges::stride_view<Range>(range, 3).base();
    assert(result.wasMoveInitialized);
    assert(result.begin() == buff);
    assert(result.end() == buff + 8);
  }
