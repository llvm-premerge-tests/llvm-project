...
static_assert(std::ranges::range<RangeView> && std::ranges::view<RangeView>);

struct SizedRangeView : public RangeView {
  std::size_t size_;

  constexpr std::size_t size() { return size_; }
};

static_assert(std::ranges::sized_range<SizedRangeView>);
