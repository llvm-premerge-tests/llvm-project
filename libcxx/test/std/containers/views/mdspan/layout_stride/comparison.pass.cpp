...
constexpr void test_comparison_different_rank() {
  ...
  using arr_0 = std::array<int, 0>;
  using arr_1 = std::array<int, 1>;
  using arr_2 = std::array<int, 2>;

  // sanity check same rank
  static_assert(compare_layout_mappings(std::extents<T1, D>(5), arr_1{1}, std::extents<T2, D>(5), arr_1{1}));
  static_assert(compare_layout_mappings(std::extents<T1, 5>(), arr_1{1}, std::extents<T2, D>(5), arr_1{1}));
  static_assert(compare_layout_mappings(std::extents<T1, D>(5), arr_1{1}, std::extents<T2, 5>(), arr_1{1}));
  static_assert(compare_layout_mappings(std::extents<T1, 5>(), arr_1{1}, std::extents<T2, 5>(), arr_1{1}));

  // not equality comparable when rank is not the same
  static_assert(compare_layout_mappings(std::extents<T1>(), arr_0{}, std::extents<T2, D>(1), arr_1{1}).does_not_match());
  static_assert(compare_layout_mappings(std::extents<T1>(), arr_0{}, std::extents<T2, 1>(), arr_1{1}).does_not_match());

  static_assert(compare_layout_mappings(std::extents<T1, D>(1), arr_1{1}, std::extents<T2>(), arr_0{}).does_not_match());
  static_assert(compare_layout_mappings(std::extents<T1, 1>(), arr_1{1}, std::extents<T2>(), arr_0{}).does_not_match());

  static_assert(compare_layout_mappings(std::extents<T1, D>(5), arr_1{1}, std::extents<T2, D, D>(5, 5), arr_2{1, 5}).does_not_match());
  static_assert(compare_layout_mappings(std::extents<T1, 5>(), arr_1{1}, std::extents<T2, 5, D>(5), arr_2{1, 5}).does_not_match());
  static_assert(compare_layout_mappings(std::extents<T1, 5>(), arr_1{1}, std::extents<T2, 5, 1>(), arr_2{1, 5}).does_not_match());

  static_assert(compare_layout_mappings(std::extents<T1, D, D>(5, 5), arr_2{1, 5}, std::extents<T2, D>(5), arr_1{1}).does_not_match());
  static_assert(compare_layout_mappings(std::extents<T1, 5, D>(5), arr_2{1, 5}, std::extents<T2, D>(5), arr_1{1}).does_not_match());
  static_assert(compare_layout_mappings(std::extents<T1, 5, 5>(), arr_2{1, 5}, std::extents<T2, 5>(), arr_1{1}).does_not_match());
}
...
constexpr void test_comparison_same_rank() {
  ...
  test_comparison(true, std::extents<T1>(), std::extents<T2>(),
                        std::array<int, 0>{}, std::array<int, 0>{});

  test_comparison(true, std::extents<T1, D>(5), std::extents<T2, D>(5),
                        std::array<int, 1>{1}, std::array<int, 1>{1});
  test_comparison(true, std::extents<T1, 5>(), std::extents<T2, D>(5),
                        std::array<int, 1>{3}, std::array<int, 1>{3});
  test_comparison(true, std::extents<T1, D>(5), std::extents<T2, 5>(),
                        std::array<int, 1>{1}, std::array<int, 1>{1});
  test_comparison(true, std::extents<T1, 5>(), std::extents< T2, 5>(),
                        std::array<int, 1>{1}, std::array<int, 1>{1});
  test_comparison(false, std::extents<T1, 5>(), std::extents<T2, D>(5),
                         std::array<int, 1>{2}, std::array<int, 1>{1});
  test_comparison(false, std::extents<T1, D>(5), std::extents<T2, D>(5),
                         std::array<int, 1>{2}, std::array<int, 1>{1});
  ...
  test_comparison(true, std::extents<T1, D, D, D, D, D>(5, 6, 7, 8, 9), std::extents<T2, D, D, D, D, D>(5, 6, 7, 8, 9),
                        std::array<int, 5>{2, 20, 200, 2000, 20000}, std::array<int, 5>{2, 20, 200, 2000, 20000});
  ...
  test_comparison(true, std::extents<T1, 5, 6, 7, 8, 9>(5, 6, 7, 8, 9), std::extents<T2, 5, 6, 7, 8, 9>(),
                        std::array<int, 5>{2, 20, 200, 2000, 20000}, std::array<int, 5>{2, 20, 200, 2000, 20000});
  test_comparison(false, std::extents<T1, 5, 6, 7, 8, 9>(5, 6, 7, 8, 9), std::extents<T2, 5, 6, 7, 8, 9>(),
                         std::array<int, 5>{2, 20, 200, 20000, 2000}, std::array<int, 5>{2, 20, 200, 2000, 20000});
  test_comparison(
...
constexpr void test_comparison_with(bool expect_equal, E1 e1, std::array<typename E1::index_type, E1::rank()> strides, E2 e2, OtherArgs ... other_args){
...
constexpr void test_comparison_with() {
  constexpr size_t D = std::dynamic_extent;
  bool is_left_based = std::is_same_v<OtherLayout, std::layout_left> ||
                       std::is_same_v<OtherLayout, always_convertible_layout>;
  ...
  test_comparison_with<OtherLayout>(is_left_based, std::extents<int, D, D>(5, 7), std::array<int, 2>{1, 5}, std::extents<unsigned, D, D>(5, 7));
  test_comparison_with<OtherLayout>(!is_left_based, std::extents<int, D, D>(5, 7), std::array<int, 2>{7, 1}, std::extents<unsigned, D, D>(5, 7));
  test_comparison_with<OtherLayout>(false, std::extents<int, D, D>(5, 7), std::array<int, 2>{8, 1}, std::extents<unsigned, D, D>(5, 7));

  if constexpr (std::is_same_v<OtherLayout, always_convertible_layout>) {
    // test layout with strides not equal to product of extents
    test_comparison_with<OtherLayout>(true, std::extents<int, D, D>(5, 7), std::array<int, 2>{2, 10}, std::extents<unsigned, D, D>(5, 7), 0, 2);
    // make sure that offset != 0 results in false
    test_comparison_with<OtherLayout>(false, std::extents<int, D, D>(5, 7), std::array<int, 2>{2, 10}, std::extents<unsigned, D, D>(5, 7), 1, 2);
  }
}
