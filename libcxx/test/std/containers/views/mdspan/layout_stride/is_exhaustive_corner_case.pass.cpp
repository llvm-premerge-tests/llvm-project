...
//     stride(p0) equals 1, and stride(pi) equals stride(pi−1) * extents().extent(pi−1) for i in the
...
constexpr bool test() {
  ...
  test_layout_mapping_stride(std::extents<int, D>(0), std::array<int,1>{3}, true);
  ...
  test_layout_mapping_stride(std::extents<int, D, D, D, D>(3,3,0,3),std::array<int,4>{3,1,27,9}, true);
