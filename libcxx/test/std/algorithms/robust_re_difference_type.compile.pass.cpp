...
TEST_CONSTEXPR_CXX20 bool all_the_algorithms()
{
    ...
    if (!TEST_IS_CONSTANT_EVALUATED_CXX20) (void)std::inplace_merge(first, mid, last);
    if (!TEST_IS_CONSTANT_EVALUATED_CXX20) (void)std::inplace_merge(first, mid, last, std::less<void*>());
    ...
    if (!TEST_IS_CONSTANT_EVALUATED_CXX20) (void)std::stable_partition(first, last, UnaryTrue());
    if (!TEST_IS_CONSTANT_EVALUATED_CXX20) (void)std::stable_sort(first, last);
    if (!TEST_IS_CONSTANT_EVALUATED_CXX20) (void)std::stable_sort(first, last, std::less<void*>());
