...
static void test_sentry_failure() {
  ...
  TEST_THROWS_TYPE(std::ios_base::failure,
                   std::vprint_unicode(os, "throws exception at run-time {0:{0}}", std::make_format_args(-10)));

  os.exceptions(std::stringstream::goodbit);
  os.setstate(std::stringstream::failbit);
  std::vprint_unicode(
      os, "not called when the os.good() is false, so no exception is thrown {0:{0}}", std::make_format_args(-10));
}
...
static void test_format_exception() {
  ...
  TEST_THROWS_TYPE(std::format_error, std::vprint_unicode(sstr, "no output {0:{0}}", std::make_format_args(-10)));
  assert(sstr.good());
...
static void test_write_failure() {
  ...
  TEST_THROWS_TYPE(
      std::format_error, std::vprint_unicode(os, "throws exception at run-time {0:{0}}", std::make_format_args(-10)));
