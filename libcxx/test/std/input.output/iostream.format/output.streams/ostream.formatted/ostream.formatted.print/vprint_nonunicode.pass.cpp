...
static void test_sentry_failure() {
  ...
  TEST_THROWS_TYPE(std::ios_base::failure,
                   std::vprint_nonunicode(os, "throws exception at run-time {0:{0}}", std::make_format_args(-10)));
...
static void test_format_exception() {
  ...
  TEST_THROWS_TYPE(std::format_error, std::vprint_nonunicode(sstr, "no output {0:{0}}", std::make_format_args(-10)));
  assert(sstr.good());
