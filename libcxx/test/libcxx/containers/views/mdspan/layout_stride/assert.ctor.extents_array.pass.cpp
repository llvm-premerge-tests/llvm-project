...
//     such that s[pi] >= s[pi−1] * e.extent(pi−1) is true for all i in the range [1, rank_), where
...
int main(int, char**) {
  ...
  // value out of range
  {
    ...
    TEST_LIBCPP_ASSERT_FAILURE(
        ([=] {
          std::layout_stride::template mapping<std::extents<char, D, 5>>
            m(std::extents<char, D, 5>(20), std::array<int, 2>{20, 1});
        }()),
    ...
    TEST_LIBCPP_ASSERT_FAILURE(
        ([=] {
          std::layout_stride::template mapping<std::extents<unsigned char, D, 5>>
            m(std::extents<unsigned char, D, 5>(20), std::array<unsigned, 2>{257, 1});
        }()),
        "layout_stride::mapping ctor: required span size is not representable as index_type.");
    ...
    TEST_LIBCPP_ASSERT_FAILURE(
        ([=] {
          std::layout_stride::template mapping<std::extents<unsigned, D, 5>>
            m(std::extents<unsigned, D, 5>(20), std::array<int, 2>{20, -1});
        }()),
    ...
    TEST_LIBCPP_ASSERT_FAILURE(
        ([=] {
          std::layout_stride::template mapping<std::extents<unsigned, D, 5>>
            m(std::extents<unsigned, D, 5>(20), std::array<unsigned, 2>{20, 0});
        }()),
        "layout_stride::mapping ctor: all strides must be greater than 0");
    // strides must result in unique layout
    TEST_LIBCPP_ASSERT_FAILURE(
        ([=] {
