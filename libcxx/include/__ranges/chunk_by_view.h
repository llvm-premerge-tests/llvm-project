...
class chunk_by_view : public view_interface<chunk_by_view<_View, _Pred>> {
  ...
  {
    _LIBCPP_ASSERT(__current != ranges::begin(__base_), "Trying to call __find_prev() on a begin iterator.");
    _LIBCPP_ASSERT(
...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr value_type operator*() const {
    _LIBCPP_ASSERT(__current_ != __next_, "Trying to dereference past-the-end chunk_by_view iterator.");
    return {__current_, __next_};
  }

  _LIBCPP_HIDE_FROM_ABI constexpr __iterator& operator++() {
    _LIBCPP_ASSERT(__current_ != __next_, "Trying to increment past end chunk_by_view iterator.");
    __current_ = __next_;
