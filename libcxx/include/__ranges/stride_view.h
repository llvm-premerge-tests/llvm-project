...
template <class _I>
_LIBCPP_HIDE_FROM_ABI constexpr static _I div_ceil(_I __left, _I __right) {
  _I __r = __left / __right;
  if (__left % __right) {
    __r++;
  }
  return __r;
...
public:
  _LIBCPP_HIDE_FROM_ABI constexpr explicit stride_view(_View __base, difference_type __stride)
