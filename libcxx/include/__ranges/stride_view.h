...
namespace ranges {
  ...
  public:
    ...
    constexpr explicit stride_view(_View __base, difference_type __stride) : __base_(std::move(__base)), __stride_(__stride) {}
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr auto stride() { return __stride_; }
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr auto begin() const requires ranges::range<const _View> {
    ...
    constexpr auto end() const requires (range<const _View> &&
      ...
      sized_range<const _View> &&
      forward_range<const _View>
    )
    {
      auto __missing = (__stride_ - ranges::distance(__base_) % __stride_) % __stride_;
  ...
  struct __stride_iterator_category<_View> {
    ...
    using iterator_category =
      _If<derived_from<_Cat, random_access_iterator_tag>, random_access_iterator_tag,
      /* else */                                          _Cat
    >;
  };
  ...
  class stride_view<_View>::__iterator : public __stride_iterator_category<_View> {
    ...
    template <class I>
    constexpr static I div_ceil(I __left, I __right) {
      I __r = __left / __right;
      if (__left % __right) {
        __r++;
      }
      return __r;
  ...
  public:
    ...
    using iterator_concept =
      _If<random_access_range<_Base>, random_access_iterator_tag,
      _If<bidirectional_range<_Base>, bidirectional_iterator_tag,
      _If<forward_range<_Base>,       forward_iterator_tag,
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr __iterator(_Parent &__parent, ranges::iterator_t<_Base> __current, difference_type __missing = 0)
      : __current_(std::move(__current)), __end_(ranges::end(__parent.__base_)), __stride_(__parent.__stride_), __missing_(__missing)
    { }
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr __iterator operator++(int) requires forward_range<_Base> {
      auto __tmp = *this;
      ++*this;
      return __tmp;
    }
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr __iterator& operator--() requires bidirectional_range<_Base> {
      ranges::advance(__current_, __missing_ - __stride_);
      __missing_ = 0;
      return *this;
    }
    _LIBCPP_HIDE_FROM_ABI
    constexpr __iterator operator--(int) requires bidirectional_range<_Base> {
      auto __tmp = *this;
      --*this;
      return __tmp;
    }
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr __iterator& operator+=(difference_type __s) requires random_access_range<_Base> {
      if (__s > 0) { 
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr __iterator& operator[](difference_type __s) const requires random_access_range<_Base> {
      return *(*this + __s);
    }
    ...
    _LIBCPP_HIDE_FROM_ABI
    friend constexpr __iterator operator+(__iterator const& __i, difference_type __s)
      requires random_access_range<_Base>
    {
      auto __r = __i;
      __r += __s;
    ...
    _LIBCPP_HIDE_FROM_ABI
    friend constexpr __iterator operator+(difference_type __s, __iterator const &__i)
      requires random_access_range<_Base>
    {
      auto __r = __i;
      __r += __s;
    ...
    _LIBCPP_HIDE_FROM_ABI
    friend constexpr difference_type operator-( __iterator const &__x, __iterator const &__y)
      ...
      requires sized_sentinel_for<iterator_t<_Base>, iterator_t<_Base>>
    {
      auto __n = __x.__current_ - __y.__current_;
      if (__n < 0) {
        return -div_ceil(-__n, __x.__stride_);
      }
      return div_ceil(__n, __x.__stride_);
    }
    ...
    _LIBCPP_HIDE_FROM_ABI
    friend constexpr difference_type operator-(default_sentinel_t, __iterator const &__x)
      requires sized_sentinel_for<sentinel_t<_Base>, iterator_t<_Base>>
    {
      return div_ceil(__x.__end_ - __x.__current_, __x.__stride_);
    }
    _LIBCPP_HIDE_FROM_ABI
    friend constexpr difference_type operator-( __iterator const &__x, default_sentinel_t __y)
      requires sized_sentinel_for<sentinel_t<_Base>, iterator_t<_Base>>
    {
      return -(__y - __x);
    }
  ...
  public:
    ...
    _LIBCPP_HIDE_FROM_ABI
    constexpr explicit __sentinel(stride_view<_View>& __parent)
    ...
    operator==(__iterator<true> const& __x, __sentinel const& __y) {
