...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr auto size() requires ranges::sized_range<_View> {
    return __to_unsigned_like(ranges::__div_ceil(ranges::distance(__base_), __stride_));
  }

  _LIBCPP_HIDE_FROM_ABI constexpr auto size() const requires ranges::sized_range<const _View> {
    return __to_unsigned_like(ranges::__div_ceil(ranges::distance(__base_), __stride_));
  }
