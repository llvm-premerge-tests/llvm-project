...
struct __fn {
  ...
  operator()(_Iter __first, _Sent __last, const _Type& __value, _Proj __proj = {}) const {
    return std::__count_impl<_RangeAlgPolicy>(std::move(__first), std::move(__last), __value, __proj);
  }
  ...
  operator()(_Range&& __r, const _Type& __value, _Proj __proj = {}) const {
    return std::__count_impl<_RangeAlgPolicy>(ranges::begin(__r), ranges::end(__r), __value, __proj);
  }
};
} // namespace __count
