...
# Invoke nc first to make sure something is listening to ports 5037 and 5038.
# Otherwise, invoking adb might fork an adb server.
#
...
until (nc -z localhost 5037 && nc -z localhost 5038 &&
       adb wait-for-device); do
    sleep 0.5
done
'
