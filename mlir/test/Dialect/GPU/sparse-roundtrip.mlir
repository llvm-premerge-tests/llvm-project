...
module attributes {gpu.container_module} {
  ...
  // CHECK:           %{{.*}}, %{{.*}} = gpu.spgemm_work_estimation_or_compute async [%{{.*}}] WORK_ESTIMATION, %{{.*}}, %{{.*}}, %{{.*}},  ALG2, %{{.*}}, %{{.*}}, %{{.*}} : f32 into memref<0xi8>
  // CHECK:           %{{.*}}, %{{.*}} = gpu.alloc async [%{{.*}}] (%{{.*}}) : memref<?xi8>
  // CHECK:           %{{.*}}, %{{.*}} = gpu.spgemm_work_estimation_or_compute async [%{{.*}}] WORK_ESTIMATION, %{{.*}}, %{{.*}}, %{{.*}},  ALG2, %{{.*}}, %{{.*}}, %{{.*}} : f32 into memref<?xi8>
  ...
  // CHECK:           %{{.*}}, %{{.*}} = gpu.alloc async [%{{.*}}] (%{{.*}}) : memref<?xi8>
  // CHECK:           %{{.*}}, %{{.*}} = gpu.spgemm_work_estimation_or_compute async [%{{.*}}] COMPUTE, %{{.*}}, %{{.*}}, %{{.*}},  ALG2, %{{.*}}, %{{.*}}, %{{.*}} : f32 into memref<?xi8>
  ...
  func.func @spgemm(%arg0: index) {
    ...
    %bufferSz1, %token7 = gpu.spgemm_work_estimation_or_compute async [%token6] WORK_ESTIMATION,
    ...
    %bufferSz1_1, %token9 = gpu.spgemm_work_estimation_or_compute async [%token8] WORK_ESTIMATION, %spmatA, %spmatB, %spmatC, ALG2, %spgemmDesc, %bufferSz1, %buf1: f32 into memref<?xi8>
    ...
    %bufferSz2_2, %token14 = gpu.spgemm_work_estimation_or_compute async [%token13] COMPUTE, %spmatA, %spmatB, %spmatC, ALG2, %spgemmDesc, %bufferSz2, %buf2: f32 into memref<?xi8>
