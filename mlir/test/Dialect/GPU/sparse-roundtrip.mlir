...
module attributes {gpu.container_module} {
  ...
  // CHECK:           %{{.*}}, %{{.*}} = gpu.spgemm_work_estimation async [%{{.*}}] %{{.*}}, %{{.*}}, %{{.*}},  ALG2, %{{.*}}, %{{.*}}, %{{.*}} : f32 into memref<0xi8>
  // CHECK:           %{{.*}}, %{{.*}} = gpu.alloc async [%{{.*}}] (%{{.*}}) : memref<?xi8>
  // CHECK:           %{{.*}}, %{{.*}} = gpu.spgemm_work_estimation async [%{{.*}}] %{{.*}}, %{{.*}}, %{{.*}},  ALG2, %{{.*}}, %{{.*}}, %{{.*}} : f32 into memref<?xi8>
  ...
  // CHECK:           %{{.*}}, %{{.*}} = gpu.alloc async [%{{.*}}] (%{{.*}}) : memref<?xi8>
  // CHECK:           %{{.*}}, %{{.*}} = gpu.spgemm_compute async [%{{.*}}] %{{.*}}, %{{.*}}, %{{.*}},  ALG2, %{{.*}}, %{{.*}}, %{{.*}} : f32 into memref<?xi8>
  ...
  func.func @spgemm(%arg0: index) {
    ...
    %bufferSz1, %token7 = gpu.spgemm_work_estimation async [%token6] 
