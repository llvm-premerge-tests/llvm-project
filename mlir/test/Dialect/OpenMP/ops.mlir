...
func.func @omp_target(%if_cond : i1, %device : si32,  %num_threads : i32, %map1: memref<?xi32>, %map2: memref<?xi32>) -> () {
    ...
    // CHECK: %[[MAP_A:.*]] = omp.map_entry var_ptr(%[[VAL_1:.*]] : memref<?xi32>)   map_clauses(tofrom, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: %[[MAP_B:.*]] = omp.map_entry var_ptr(%[[VAL_2:.*]] : memref<?xi32>)   map_clauses(exit_release_or_enter_alloc, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: omp.target map_entries(%[[MAP_A]], %[[MAP_B]] : memref<?xi32>, memref<?xi32>) {
    %mapv1 = omp.map_entry var_ptr(%map1 : memref<?xi32>)   map_clauses(tofrom, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    %mapv2 = omp.map_entry var_ptr(%map2 : memref<?xi32>)   map_clauses(exit_release_or_enter_alloc, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    omp.target map_entries(%mapv1, %mapv2 : memref<?xi32>, memref<?xi32>){}
    // CHECK: %[[MAP_C:.*]] = omp.map_entry var_ptr(%[[VAL_1:.*]] : memref<?xi32>)   map_clauses(to, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: %[[MAP_D:.*]] = omp.map_entry var_ptr(%[[VAL_2:.*]] : memref<?xi32>)   map_clauses(always, from, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: omp.target map_entries(%[[MAP_C]], %[[MAP_D]] : memref<?xi32>, memref<?xi32>) {
    %mapv3 = omp.map_entry var_ptr(%map1 : memref<?xi32>)   map_clauses(to, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    %mapv4 = omp.map_entry var_ptr(%map2 : memref<?xi32>)   map_clauses(always, from, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
...
func.func @omp_target_data (%if_cond : i1, %device : si32, %device_ptr: memref<i32>, %device_addr: memref<?xi32>, %map1: memref<?xi32>, %map2: memref<?xi32>) -> () {
    // CHECK: %[[MAP_A:.*]] = omp.map_entry var_ptr(%[[VAL_2:.*]] : memref<?xi32>)   map_clauses(always, from, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: omp.target_data if(%[[VAL_0:.*]] : i1) device(%[[VAL_1:.*]] : si32) map_entries(%[[MAP_A]] : memref<?xi32>)
    %mapv1 = omp.map_entry var_ptr(%map1 : memref<?xi32>)   map_clauses(always, from, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    omp.target_data if(%if_cond : i1) device(%device : si32) map_entries(%mapv1 : memref<?xi32>){}

    // CHECK: %[[MAP_A:.*]] = omp.map_entry var_ptr(%[[VAL_2:.*]] : memref<?xi32>)   map_clauses(close, present, to, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: omp.target_data map_entries(%[[MAP_A]] : memref<?xi32>) use_device_ptr(%[[VAL_3:.*]] : memref<i32>) use_device_addr(%[[VAL_4:.*]] : memref<?xi32>)
    %mapv2 = omp.map_entry var_ptr(%map1 : memref<?xi32>)   map_clauses(close, present, to, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    ...
    %mapv3 = omp.map_entry var_ptr(%map1 : memref<?xi32>)   map_clauses(tofrom, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    %mapv4 = omp.map_entry var_ptr(%map2 : memref<?xi32>)   map_clauses(exit_release_or_enter_alloc, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    omp.target_data map_entries(%mapv3, %mapv4 : memref<?xi32>, memref<?xi32>) {}

    // CHECK: %[[MAP_A:.*]] = omp.map_entry var_ptr(%[[VAL_3:.*]] : memref<?xi32>)   map_clauses(exit_release_or_enter_alloc, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    // CHECK: omp.target_enter_data if(%[[VAL_0:.*]] : i1) device(%[[VAL_1:.*]] : si32) nowait map_entries(%[[MAP_A]] : memref<?xi32>) 
    %mapv5 = omp.map_entry var_ptr(%map1 : memref<?xi32>)   map_clauses(exit_release_or_enter_alloc, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
    ...
    %mapv6 = omp.map_entry var_ptr(%map2 : memref<?xi32>)   map_clauses(exit_release_or_enter_alloc, target_param) capture(ByRef) -> memref<?xi32> {name = ""}
...
func.func @omp_targets_with_map_bounds(%arg0: !llvm.ptr<array<10 x i32>>, %arg1: !llvm.ptr<array<10 x i32>>) -> () {
  ...
  // CHECK: %[[MAP0:.*]] = omp.map_entry var_ptr(%[[ARG0]] : !llvm.ptr<array<10 x i32>>)   map_clauses(tofrom, target_param, non_contig, member_of) capture(ByRef) bounds(%[[BOUNDS0]]) -> !llvm.ptr<array<10 x i32>> {name = ""}
    ...
    %mapv1 = omp.map_entry var_ptr(%arg0 : !llvm.ptr<array<10 x i32>>)   map_clauses(tofrom, target_param, non_contig, member_of) capture(ByRef) bounds(%4) -> !llvm.ptr<array<10 x i32>> {name = ""}
  ...
  // CHECK: %[[MAP1:.*]] = omp.map_entry var_ptr(%[[ARG1]] : !llvm.ptr<array<10 x i32>>)   map_clauses(exit_release_or_enter_alloc, target_param, implicit, ompx_hold) capture(ByCopy) bounds(%[[BOUNDS1]]) -> !llvm.ptr<array<10 x i32>> {name = ""}
    %6 = llvm.mlir.constant(9 : index) : i64
    %7 = llvm.mlir.constant(1 : index) : i64
    %8 = llvm.mlir.constant(2 : index) : i64
    %9 = llvm.mlir.constant(2 : index) : i64
    %10 = omp.bounds   lower_bound(%7 : i64) upper_bound(%6 : i64) stride(%8 : i64) start_idx(%9 : i64)
    %mapv2 = omp.map_entry var_ptr(%arg1 : !llvm.ptr<array<10 x i32>>)   map_clauses(exit_release_or_enter_alloc, target_param, implicit, ompx_hold) capture(ByCopy) bounds(%10) -> !llvm.ptr<array<10 x i32>> {name = ""}
    ...
    // CHECK: %[[MAP2:.*]] = omp.map_entry var_ptr(%[[ARG0]] : !llvm.ptr<array<10 x i32>>)   map_clauses(exit_release_or_enter_alloc, target_param, private, literal) capture(VLAType) bounds(%[[BOUNDS0]]) -> !llvm.ptr<array<10 x i32>> {name = ""}
    // CHECK: omp.target_enter_data map_entries(%[[MAP2]] : !llvm.ptr<array<10 x i32>>)
    %mapv3 = omp.map_entry var_ptr(%arg0 : !llvm.ptr<array<10 x i32>>)   map_clauses(exit_release_or_enter_alloc, target_param, private, literal) capture(VLAType) bounds(%4) -> !llvm.ptr<array<10 x i32>> {name = ""}
    omp.target_enter_data map_entries(%mapv3 : !llvm.ptr<array<10 x i32>>){}
   
    // CHECK: %[[MAP3:.*]] = omp.map_entry var_ptr(%[[ARG1]] : !llvm.ptr<array<10 x i32>>)   map_clauses(exit_release_or_enter_alloc, ptr_and_obj, target_param, return_param) capture(This) bounds(%[[BOUNDS1]]) -> !llvm.ptr<array<10 x i32>> {name = ""}
    // CHECK: omp.target_exit_data map_entries(%[[MAP3]] : !llvm.ptr<array<10 x i32>>)
    %mapv4 = omp.map_entry var_ptr(%arg1 : !llvm.ptr<array<10 x i32>>)   map_clauses(exit_release_or_enter_alloc, ptr_and_obj, target_param, return_param) capture(This) bounds(%10) -> !llvm.ptr<array<10 x i32>> {name = ""}
