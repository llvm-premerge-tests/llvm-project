...
func.func @main() {
  ...
  // initialize hdst on host
  %hdst = memref.alloc(%count) : memref<?xi32>
  %hdst_unranked = memref.cast %hdst : memref<?xi32> to memref<*xi32>
  gpu.host_register %hdst_unranked : memref<*xi32>
  %hdst_dev_ptr = gpu.get_device_ptr %hdst_unranked : memref<*xi32> into memref<*xi32>
  ...
  // CHECK: [84, 84]
  call @printMemrefI32(%hdst_unranked) : (memref<*xi32>) -> ()
  return
}
