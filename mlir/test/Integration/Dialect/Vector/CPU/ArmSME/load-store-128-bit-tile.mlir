...
/// This test is disabled until a fixed version of QEMU is available.
/// FIXME: Enable this test once QEMU is confirmed to be fixed.
// UNSUPPORTED: {{.*}}

func.func @print_i8s(%arg0: memref<?xi8>, %len: index) {
  %c0 = arith.constant 0 : index
  %c16 = arith.constant 16 : index
  scf.for %i = %c0 to %len step %c16 {
    %v = vector.load %arg0[%i] : memref<?xi8>, vector<16xi8>
...
func.func @printStr(%s: !llvm.ptr<array<17 x i8>>) {
  %c0 = llvm.mlir.constant(0 : index) : i64
  %str_bytes = llvm.getelementptr %s[%c0, %c0]
    : (!llvm.ptr<array<17 x i8>>, i64, i64) -> !llvm.ptr<i8>
  llvm.call @printCString(%str_bytes) : (!llvm.ptr<i8>) -> ()
  return
}

func.func @vector_copy_i128(%arg0: memref<?x?xi128>, %arg1: memref<?x?xi128>) {
  %c0 = arith.constant 0 : index
  %tile = vector.load %arg0[%c0, %c0] : memref<?x?xi128>, vector<[1]x[1]xi128>
  vector.store %tile, %arg1[%c0, %c0] : memref<?x?xi128>, vector<[1]x[1]xi128>
  return
}
...
func.func @test_load_store_zaq0() {
  ...
  /// Calculate the size of an 128-bit tile in bytes:
  ...
  /// Allocate memory two 128-bit tiles (A and B) and fill them a constant.
  %tile_a_bytes = memref.alloc(%zaq_size_bytes) : memref<?xi8>
  %tile_b_bytes = memref.alloc(%zaq_size_bytes) : memref<?xi8>
  ...
  func.call @printStr(%init_a_str) : (!llvm.ptr<array<17 x i8>>) -> ()
  ...
  func.call @printStr(%init_b_str) : (!llvm.ptr<array<17 x i8>>) -> ()
  ...
  // CHECK: ( 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 )
  func.call @printStr(%final_a_str) : (!llvm.ptr<array<17 x i8>>) -> ()
  func.call @print_i8s(%tile_a_bytes, %zaq_size_bytes) : (memref<?xi8>, index) -> ()
  vector.print punctuation <newline>

  // CHECK-LABEL: FINAL TILE B:
  // CHECK: ( 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 )
  func.call @printStr(%final_b_str) : (!llvm.ptr<array<17 x i8>>) -> ()
  func.call @print_i8s(%tile_b_bytes, %zaq_size_bytes) : (memref<?xi8>, index) -> ()
