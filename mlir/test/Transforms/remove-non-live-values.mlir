...
// %arg5 doesn't get removed from the @main even though it isn't live
// because the signature of a public function is always left untouched.
//
...
// %arg0 is not live because it is never
// used. %arg1 is not live because its user
// `arith.addi` doesn't have any uses and the value that it is forwarded to (%non_live_0)
// also doesn't have any uses.
//
// Note that this cleanup cannot be done by the `canonicalize` pass.
//
...
// removed despite %c0 being live because neither %non_live nor %non_live_1 were live
// (removal doesn't depend on the liveness of the operand itself but on the
...
//
//  (2) %arg5 is not live. Yet, it is kept because %live_0 forwards to it, which also
//  forwards to %live, which is live.
//
// Values not kept:
//  (1) %arg1 is not kept as an operand of `scf.while` because it only forwards to
//  %arg3, which is not kept. %arg3 is not kept because %arg3 is not live and
//  only %arg1 and %arg6 forward to it, such that neither of them forward anywhere
//  else. Thus, %arg6 is also not kept in the `scf.yield` op.
//
// Note that this cleanup cannot be done by the `canonicalize` pass.
//
...
// CHECK-NEXT:    return
// CHECK-NEXT:  }
// CHECK:       func.func private @clean_region_branch_op_dont_remove_results(%[[arg0:.*]]: i1, %[[arg2:.*]]: i32) -> i32 {
...
// CHECK-NEXT:  }
// CHECK:       func.func @main(%[[arg7:.*]]: i1, %[[arg8:.*]]: i32, %[[arg9:.*]]: i32) -> i32 {
// CHECK-NEXT:    %[[live_2:.*]] = call @clean_region_branch_op_dont_remove_results(%[[arg7]], %[[arg9]]) : (i1, i32) -> i32
// CHECK-NEXT:    return %[[live_2]]
// CHECK-NEXT:  }
func.func private @f(%arg10 : i32) {
    return
}
func.func private @clean_region_branch_op_dont_remove_results(%arg0: i1, %arg1: i32, %arg2: i32) -> (i32, i32) {
  %live, %non_live = scf.while (%arg3 = %arg1, %arg4 = %arg2) : (i32, i32) -> (i32, i32) {
    %live_0 = arith.addi %arg4, %arg4 : i32
    func.call @f(%arg3) : (i32) -> ()
    scf.condition(%arg0) %live_0, %arg4 : i32, i32
  } do {
  ^bb0(%arg5: i32, %arg6: i32):
    %live_1 = arith.addi %arg6, %arg6 : i32
    scf.yield %arg6, %live_1 : i32, i32
  }
  return %live, %non_live : i32, i32
}

func.func @main(%arg7: i1, %arg8: i32, %arg9: i32) -> (i32) {
  %live_2, %non_live_0 = func.call @clean_region_branch_op_dont_remove_results(%arg7, %arg8, %arg9) : (i1, i32, i32) -> (i32, i32)
  return %live_2 : i32
}
...
//
//  (2) %non_live is not live. Yet, it is kept because %arg3 (the first instance in
//  `scf.condition`) forwards to it and this instance of %arg3 has to be kept.
//  The first instance of %arg3 in `scf.condition` has to be kept because it
//  forwards to %arg6, which forwards to %arg4, which forwards to %live, which is
//  live.
//
// Values not kept:
//  (1) %non_live_0 is not kept because the second instance of %arg3 in `scf.condition`
//  forwards to it, which forwards to only %non_live_0 and %arg7, where both these are
//  not live and have no other value forwarding to them.
//
//  (2) %non_live_1 is not kept because the third instance of %arg3 in `scf.condition`
//  forwards to it, which forwards to only %non_live_1 and %arg8, where both these are
//  not live and have no other value forwarding to them.
//
// Note that this cleanup cannot be done by the `canonicalize` pass.
//
...
// CHECK-NEXT:    return
// CHECK-NEXT:  }
// CHECK:       func.func private @clean_region_branch_op_remove_last_two_results(%[[arg2:.*]]: i1) -> i32 {
// CHECK-NEXT:    %[[c0:.*]] = arith.constant 0
...
// CHECK-NEXT:  }
// CHECK:       func.func @main(%[[arg9:.*]]: i1) -> i32 {  
// CHECK-NEXT:    %[[live_0:.*]] = call @clean_region_branch_op_remove_last_two_results(%[[arg9]]) : (i1) -> i32
// CHECK-NEXT:    return %[[live_0]]
// CHECK-NEXT:  }
func.func private @f(%arg0 : i32, %arg1 : i32) {
    return
}
func.func private @clean_region_branch_op_remove_last_two_results(%arg2: i1) -> (i32, i32) {
  %c0 = arith.constant 0 : i32
  %c1 = arith.constant 1 : i32
  %live, %non_live, %non_live_0, %non_live_1 = scf.while (%arg3 = %c0, %arg4 = %c1) : (i32, i32) -> (i32, i32, i32, i32) {
    %non_live_3 = arith.addi %arg3, %arg4 : i32
    scf.condition(%arg2) %arg4, %arg3, %non_live_3, %non_live_3 : i32, i32, i32, i32
  } do {
  ^bb0(%arg5: i32, %arg6: i32, %arg7: i32, %arg8: i32):
    func.call @f(%arg7, %arg8) : (i32, i32) -> ()
    scf.yield %arg5, %arg6 : i32, i32
  }
  return %live, %non_live_1 : i32, i32
}

func.func @main(%arg9 : i1) -> (i32) {
  %live_0, %non_live_2 = func.call @clean_region_branch_op_remove_last_two_results(%arg9) : (i1) -> (i32, i32)
  return %live_0 : i32
}
...
// Note that this cleanup cannot be done by the `canonicalize` pass.
//
// CHECK:       func.func private @clean_region_branch_op_remove_result(%[[arg0:.*]]: index, %[[arg1:.*]]: memref<i32>) {
...
// CHECK-NEXT:    return
// CHECK-NEXT:  }
// CHECK:       func.func @main(%[[arg2:.*]]: memref<i32>, %[[arg3:.*]]: index) {
// CHECK-NEXT:    call @clean_region_branch_op_remove_result(%[[arg3]], %[[arg2]]) : (index, memref<i32>) -> ()
// CHECK-NEXT:    return
// CHECK-NEXT:  }
func.func private @clean_region_branch_op_remove_result(%arg0 : index, %arg1 : memref<i32>) -> (i32) {
...
func.func @main(%arg2 : memref<i32>, %arg3 : index) {
  %non_live_0 = func.call @clean_region_branch_op_remove_result(%arg3, %arg2) : (index, memref<i32>) -> (i32)
  return
}
