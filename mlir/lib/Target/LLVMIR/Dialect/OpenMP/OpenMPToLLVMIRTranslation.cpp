...
static void genMapInfos(llvm::IRBuilderBase &builder,
  ...
  for (const auto &mapOp : mapOperands) {
    ...
    if (!mapEntryOp.getType().isa<LLVM::LLVMPointerType>())
      return fail();

    llvm::Value *mapOpValue =
        moduleTranslation.lookupValue(mapEntryOp.getVarPtr());
    ...
    combinedInfo.Names.emplace_back(LLVM::createMappingInformation(
        mapEntryOp.getVarPtr().getLoc(), *ompBuilder));
    ...
    combinedInfo.Sizes.emplace_back(
        builder.getInt64(getSizeInBytes(DL, mapEntryOp.getVarPtr().getType())));
    index++;
  }
...
LogicalResult OpenMPDialectLLVMIRTranslationInterface::convertOperation(
  ...
  return llvm::TypeSwitch<Operation *, LogicalResult>(op)
      ...
      .Case<omp::MapEntryOp, omp::DataBoundsOp>([&](auto op) {
