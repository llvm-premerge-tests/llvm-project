...
private:
  ...
  /// Checks all the preconditions that have to hold for the deallocation to be
  /// applicable.
  /// (1) Checks that there are not explicit control flow loops.
  /// (2) Checks if all operations that have at least one attached region
  ...
  /// results.
  /// (3) Checks that no deallocations already exist. Especially deallocations
  ...
  // (4) Checks that terminators with more than one successor except
  // `cf.cond_br` are not present and that either BranchOpInterface or
  // RegionBranchTerminatorOpInterface is implemented.
  static LogicalResult verifyPreconditions(FunctionOpInterface op);
...
LogicalResult BufferDeallocation::verifyPreconditions(FunctionOpInterface op) {
  ...
  // (2) Check that the control flow structures are supported.
  WalkResult result = op.getFunctionBody().walk([&](Operation *operation) {
  ...
  // (3) The pass does not work properly when deallocations are already present.
  // Alternatively, we could also remove all deallocations as a pre-pass.
  result = op->walk([](Operation *op) {
    return isa<DeallocOp, memref::DeallocOp>(op) ? WalkResult::interrupt()
                                                 : WalkResult::advance();
  });
  if (result.wasInterrupted())
    return emitError(
  ...
  // RegionBranchTerminatorOpInterface is implemented.
  result = op->walk([](Block *block) {
    if (block->getParentOp()->hasTrait<OpTrait::NoTerminator>())
      return WalkResult::interrupt();
    ...
    if (!isa<BranchOpInterface, RegionBranchTerminatorOpInterface>(
            terminator) &&
        !(isa<BranchOpInterface>(terminator) &&
  ...
  if (result.wasInterrupted())
    return emitError(
        ...
        "are not supported (except cf.cond_br) and either BranchOpInterface or "
        "RegionBranchTerminatorOpInterface has to be implemented (but not "
        "both)!");
...
LogicalResult BufferDeallocation::deallocate(FunctionOpInterface op) {
  ...
  if (failed(verifyPreconditions(op)))
    return failure();
...
/// to be always 'true'.
static Value getMemrefWithGuaranteedOwnership(OpBuilder &builder, DeallocationState &state,
                                                           Value memref) {
...
BufferDeallocation::handleInterface(RegionBranchTerminatorOpInterface op) {
  ...
  if (isFunctionWithoutDynamicOwnership(op->getParentOp())) {
    for (OpOperand &val : op->getOpOperands()) {
      ...
      val.set(getMemrefWithGuaranteedOwnership(builder, state, val.get()));
    }
  }
