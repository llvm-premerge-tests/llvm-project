...
/// Example:
/// ```mlir
/// %0 = bufferization.dealloc (%arg0 : ...) if (%arg1)
///                     retain (%arg0, %arg1, %arg2 : ...)
/// ```
/// is canonicalized to
/// ```mlir
/// %0 = bufferization.dealloc retain (%arg0, %arg1, %arg2 : ...)
/// ```
/// given that `%arg1` and `%arg2` may not alias with `%arg0`.
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    for (auto memrefAndCond :
      ...
      auto replaceResultsIfNoInvalidAliasing = [&](Value memref) -> bool {
        ...
        // memref, it either has NoAlias or AlwaysAlias. If it's NoAlias, then
        // just proceed, if it's AlwaysAlias, we also need to update the
        // condition for that alias. Currently, we only support a more
        // restrictive case where exactly one memref in `retained` must alias
        // and all the other memrefs must not alias.
...
struct RemoveRetainedMemrefsGuaranteedToNotAlias
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    for (auto retainedMemref : deallocOp.getRetained()) {
      ...
      replacements.push_back(falseValue);
    }
...
/// Split off memrefs to separate (newly created) dealloc operations to reduce
/// the number of runtime checks required and enable further canonicalization of
/// the new and simpler dealloc operations. A memref can be split off if it is
/// guaranteed to not alias with any other memref in the `memref` operand list.
/// The results of the old and the new dealloc operation have to be combined by
/// computing the element-wise disjunction of them.
///
/// Example:
/// ```mlir
