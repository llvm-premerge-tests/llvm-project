...
struct SkipExtractMetadataOfAlloc : public OpRewritePattern<DeallocOp> {
  ...
  LogicalResult matchAndRewrite(DeallocOp deallocOp,
    ...
    for (Value memref : deallocOp.getMemrefs()) {
      Value allocMemref;
      MemoryEffectOpInterface allocOp;
      if (matchPattern(memref,
                       m_Op<memref::ExtractStridedMetadataOp>(m_AllOf(
                           matchers::m_Any(&allocMemref),
                           m_AnyOpOfType<MemoryEffectOpInterface>(allocOp)))) &&
          allocOp.getEffectOnValue<MemoryEffects::Allocate>(allocMemref)) {
        newMemrefs.push_back(allocMemref);
        continue;
      }
      newMemrefs.push_back(memref);
    }
