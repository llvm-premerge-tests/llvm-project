...
struct ConvertMemRefAlloc final : OpConversionPattern<memref::AllocOp> {
  ...
  matchAndRewrite(memref::AllocOp op, OpAdaptor adaptor,
    ...
    MemRefType currentType = op.getMemref().getType().cast<MemRefType>();
...
struct ConvertMemRefLoad final : OpConversionPattern<memref::LoadOp> {
  ...
  matchAndRewrite(memref::LoadOp op, OpAdaptor adaptor,
    ...
    auto loc = op.getLoc();
    SmallVector<OpFoldResult> indices = getAsOpFoldResult(adaptor.getIndices());
    ...
    auto stridedMetadata =
        rewriter.create<memref::ExtractStridedMetadataOp>(loc, op.getMemRef());
