...
struct ExpandReallocOpPattern : public OpRewritePattern<memref::ReallocOp> {
  ExpandReallocOpPattern(MLIRContext *ctx, bool doNotDealloc)
      : OpRewritePattern(ctx), doNotDealloc(doNotDealloc) {}
  ...
  LogicalResult matchAndRewrite(memref::ReallocOp op,
    ...
    if (ShapedType::isDynamic(inputSize)) {
      Value dimZero = materialize(rewriter.getIndexAttr(0), rewriter, loc);
    ...
    Value lhs = materialize(currSize, rewriter, loc);
    Value rhs = materialize(targetSize, rewriter, loc);
  ...
  Value materialize(OpFoldResult value, PatternRewriter &rewriter,
                    Location loc) const {
    if (value.is<Value>())
      return value.get<Value>();

    Attribute attr = value.get<Attribute>();
    assert(isa<IntegerAttr>(attr) &&
           "Only materializing integer attributes is supported.");
    return rewriter.create<arith::ConstantOp>(loc, cast<IntegerAttr>(attr));
  }
...
struct ExpandReallocPass
  ...
  ExpandReallocPass(bool doNotDealloc)
      : memref::impl::ExpandReallocBase<ExpandReallocPass>() {
    doNotDeallocate.setValue(doNotDealloc);
  }
  void runOnOperation() override {
    MLIRContext &ctx = getContext();

    RewritePatternSet patterns(&ctx);
    memref::populateExpandReallocPatterns(patterns, doNotDeallocate.getValue());
...
void mlir::memref::populateExpandReallocPatterns(RewritePatternSet &patterns,
                                                 bool doNotDealloc) {
  patterns.add<ExpandReallocOpPattern>(patterns.getContext(), doNotDealloc);
}

std::unique_ptr<Pass> mlir::memref::createExpandReallocPass(bool doNotDealloc) {
  return std::make_unique<ExpandReallocPass>(doNotDealloc);
}
