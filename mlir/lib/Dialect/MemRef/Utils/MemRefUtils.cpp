...
static bool valueIsNotRead(Operation *op, std::vector<Operation *> &uses) {
  std::vector<Operation *> opUses;
  for (OpOperand &use : op->getUses()) {
    Operation *useOp = use.getOwner();
    if (isa<memref::DeallocOp>(useOp) ||
        (useOp->getNumResults() == 0 &&
         !mlir::hasEffect<MemoryEffects::Read>(useOp)) ||
        (isa<memref::SubViewOp>(useOp) && valueIsNotRead(useOp, opUses))) {
...
void eraseDeadAllocAndStores(RewriterBase &rewriter, Operation *parentOp) {
  ...
  parentOp->walk([&](memref::AllocOp op) {
    ...
    if (valueIsNotRead(op, candidates)) {
