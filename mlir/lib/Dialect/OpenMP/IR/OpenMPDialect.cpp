...
/// `to` | `from` | `delete` | `delete` | `exit_release_or_enter_alloc` )`
/// `, `? `, `? `ptr_and_obj, `? `target_param, `? `return_param, `?
/// `private, `? `literal, `? `implicit, `? `ompx_hold, `? `non_contig, `?
/// `member_of`? `)`
static ParseResult parseMapClause(OpAsmParser &parser, IntegerAttr &mapType) {
  ...
  auto parseTypeAndMod = [&]() -> ParseResult {
    ...
    if (mapTypeMod == "target_param")
      mapTypeBits |= llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_TARGET_PARAM;

    if (mapTypeMod == "return_param")
      mapTypeBits |= llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_RETURN_PARAM;

    if (mapTypeMod == "private")
      mapTypeBits |= llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_PRIVATE;
    ...
    if (mapTypeMod == "non_contig")
      mapTypeBits |= llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_NON_CONTIG;

    if (mapTypeMod == "member_of")
      mapTypeBits |= llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_MEMBER_OF;
...
static void printMapClause(OpAsmPrinter &p, Operation *op,
  ...
  // handling of non-specification runtime related flags
  if (mapTypeToBitFlag(
          mapTypeBits,
  ...
  if (mapTypeToBitFlag(
          mapTypeBits,
          llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_NON_CONTIG))
    mapTypeStrs.push_back("non_contig");

  if (mapTypeToBitFlag(mapTypeBits,
                       llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_MEMBER_OF))
    mapTypeStrs.push_back("member_of");
