...
static LogicalResult checkDeclareOperands(Op &op,
  ...
  for (mlir::Value operand : operands) {
    ...
    // If the variable is marked with implicit attribute, the matching declare data
    // action must also be marked implicit. The reverse is not checked since implicit
    // data action may be inserted to do actions like updating device copy, in which
    // case the variable is not necessarily implicitly declare'd.
...
bool
mlir::acc::getImplicitFlag(mlir::Operation *accDataEntryOp) {
  auto implicit{
      llvm::TypeSwitch<mlir::Operation *, bool>(
          accDataEntryOp)
          .Case<ACC_DATA_ENTRY_OPS>(
