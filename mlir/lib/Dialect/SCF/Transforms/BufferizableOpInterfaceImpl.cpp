...
static FailureOr<BaseMemRefType> computeLoopRegionIterArgBufferType(
  ...
  auto iterRanked = cast<RankedTensorType>(iterArg.getType());
#ifndef NDEBUG
  auto initRanked = llvm::cast<MemRefType>(*initArgBufferType);
  assert(llvm::all_equal({yieldedRanked.getShape(), initRanked.getShape(),
                          iterRanked.getShape()}) &&
         "expected same shape");
  assert(yieldedRanked.getMemorySpace() == initRanked.getMemorySpace() &&
         "expected same memory space");
#endif // NDEBUG
  return getMemRefTypeWithFullyDynamicLayout(iterRanked,
