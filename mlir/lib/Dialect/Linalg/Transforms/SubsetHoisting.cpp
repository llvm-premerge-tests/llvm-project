...
getUnrolledInsertSlices(scf::ForOp forOp, BlockArgument bbArg,
  ...
  for (vector::InsertStridedSliceOp op : res) {
    SmallVector<int64_t> offsets = mlir::getI64SubArray(op.getOffsets());
...
scf::ForOp
mlir::linalg::hoistUnrolledVectorExtractInsertSlice(RewriterBase &rewriter,
                                                    scf::ForOp forOp) {
  LLVM_DEBUG(DBGS() << "Enter hoistUnrolledVectorExtractInsertSlice scf.for\n");
  Operation *yield = forOp.getBody()->getTerminator();
