...
LogicalResult NVVM::WgmmaMmaSyncOp::verify() {
  if ((!getResults() && !getInouts()) || (getResults() && getInouts()))
    return emitOpError() << "expected output or inout";
  ...
  for (Type t : stype.getBody()) {
    if (t != outputType)
      return emitOpError() << "all elements in struct must be same type " << t;
  }

  if (!outputType.isF32() && !outputType.isInteger(32) && !outputType.isF16()) {
    emitOpError() << "does not support the given output type " << outputType;
  }
  if (outputType.isInteger(32) && (getScaleA() == NVVM::WGMMAScaleIn::neg ||
                                   getScaleB() == NVVM::WGMMAScaleIn::neg)) {
    emitOpError() << "has s32 output, scaleA and scaleB cannot be neg";
  }

  // Check M
  if (getShape().getM() != 64)
    emitOpError() << "shape 'm' must be 64";
  ...
  // f32 += tf32 * tf32| f16 += f16 * f16| f32 += f16 * f16| f16 += bf16 *
  // bf16
  if ((outputType.isF32() || outputType.isF16()) &&
  ...
  if (!outputType.isInteger(32) && getSatfinite().value_or(false)) {
    return emitOpError()
...
std::string NVVM::WgmmaMmaSyncOp::getPtx() {
  ...
  if (getSatfinite().value_or(false))
    ss << ".satfinite";
  ss << " {";
  int regCnt = 0;
