...
public:
  ...
  /// TODO: When index-reduction-based codegen is complete, we will only use it
  /// for locating a constant index (i.e., isIdxLocate = true). We also want a
  /// faster implementation (e.g., bi-search).
  ...
  /// Generates code to locate the tensor level to the given coordinate.
  /// For non-dense tensor, it might be implemented as binary search.
  void genIdxLocate(OpBuilder &builder, Location loc, TensorLevel tidLvl,
                    AffineConstantExpr crd, MutableArrayRef<Value> reduc = {});
  ...
  /// Get the range of values for all induction variables (which are generated
  /// by user-requested loops, i.e., not index locating loops).
...
private:
  ...
  struct LoopInfo final {
    ...
    Operation *const loop;      // the loop operation
