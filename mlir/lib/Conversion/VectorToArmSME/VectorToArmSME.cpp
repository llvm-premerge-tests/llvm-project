...
struct ConstantOpToArmSMELowering : public OpRewritePattern<arith::ConstantOp> {
  ...
  LogicalResult matchAndRewrite(arith::ConstantOp constantOp,
    ...
    auto tile = getTile(rewriter, loc, tileType);

    auto forOp = generateTileSliceLoop(rewriter, loc, tileElementType);
...
struct BroadcastOpToArmSMELowering
  ...
  LogicalResult matchAndRewrite(vector::BroadcastOp broadcastOp,
    ...
    // Create 'arm_sme.vector_to_tile_slice' to broadcast the value to each
    // tile slice.
    rewriter.create<arm_sme::VectorToTileSliceOp>(loc, tileType, broadcastOp1D,
                                                  tile, tileSliceIndex);
