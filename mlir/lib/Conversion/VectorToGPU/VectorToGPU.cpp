...
// Return true if the given ext op is only used by vector transfer write ops.
static bool fpExtendSupportsMMAMatrixType(arith::ExtFOp extOp) {
  return llvm::all_of(extOp->getUsers(), [](Operation *op) {
    return isa<vector::TransferWriteOp>(op);
  });
}
