...
public:
  Operation *createBranchRegionOp(OpBuilder &builder,
  ...
  void createBranchRegionTerminatorOp(Location loc, OpBuilder &builder,
                                      Operation *branchRegionOp,
                                      ValueRange results) override {
    builder.create<scf::YieldOp>(loc, results);
  }

  Operation *createDoWhileLoopOp(OpBuilder &builder, Operation *replacedOp,
  ...
  void createCFGSwitchOperation(Location loc, OpBuilder &builder, Value flag,
  ...
  Operation *createUnreachableTerminator(Location loc, OpBuilder &builder,
    ...
    //       poison values.
    auto funcOp = cast<func::FuncOp>(region.getParentOp());

    return builder.create<func::ReturnOp>(
        loc, llvm::map_to_vector(funcOp.getResultTypes(), [&](Type type) {
