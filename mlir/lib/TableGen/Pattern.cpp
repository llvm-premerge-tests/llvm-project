...
std::string SymbolInfoMap::SymbolInfo::getValueAndRangeUse(
  ...
  case Kind::Operand: {
    ...
    // the value itself.
    if (operand->isVariableLength() && !hasRangeIndex()) {
      auto repl = formatv(fmt, name);
...
bool SymbolInfoMap::bindOpArgument(DagNode node, StringRef symbol,
  ...
  auto symInfo = op.getArg(argIndex).is<NamedAttribute *>()
                     ...
                     : SymbolInfo::getOperand(node, &op, argIndex, rangeIndex);
...
SymbolInfoMap::findBoundSymbol(StringRef key, DagNode node, const Operator &op,
                               int argIndex, int rangeIndex) const {
  return findBoundSymbol(
      key, SymbolInfo::getOperand(node, &op, argIndex, rangeIndex));
}
  ...
  if (tree.isOperation()) {
    ...
    auto collectSymbolInVariadic = [&](DagNode parent, DagNode tree,
      ...
      if (!treeName.empty()) {
        ...
        verifyBind(infoMap.bindOpArgument(
                       parent, treeName, op, opArgIdx,
                       SymbolInfoMap::SymbolInfo::kInvalidRangeIndex),
                   treeName);
      }
