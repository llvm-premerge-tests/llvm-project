...
SymbolicLexOpt
PresburgerRelation::findSymbolicIntegerLexOpt(const PresburgerRelation &rel,
                                              bool isMin) {
  PWMAFunction lexopt(rel.getSpace());
  PresburgerSet unboundedDomain =
      PresburgerSet::getUniverse(PresburgerSpace::getSetSpace(
          rel.getNumDomainVars(), rel.getNumSymbolVars()));
  for (const IntegerRelation &cs : rel.disjuncts) {
  ...
  result.lexopt = lexopt;
  result.unboundedDomain = unboundedDomain;
  return result;
}
