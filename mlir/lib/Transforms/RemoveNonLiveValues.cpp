...
//===----------------------------------------------------------------------===//
//
// This pass optimizes the IR by removing non-live values. The removal of such
// values involves removal of extraneous operands, extraneous arguments,
// extraneous return values, and even extraneous ops, all of which unnecessarily
// increase the runtime, in theory. The pass relies on the information provided
// by the liveness analysis utility to accomplish its optimization goal.
//
//===----------------------------------------------------------------------===//
...
/// information in `la`.
static bool hasLive(const SmallVector<Value, 4> &values,
...
static BitVector markLives(const SmallVector<Value, 4> &values,
                           RunLivenessAnalysis &la) {
...
/// and the IR in `irOp`. Here, cleaning means:
...
static void cleanFuncOp(FunctionOpInterface funcOp, Operation *irOp,
                        RunLivenessAnalysis &la) {
  if (funcOp.isPublic())
    return;

  // Get the list of unnecessary (non-live) arguments in `nonLiveArgs`.
  SmallVector<Value, 4> arguments(funcOp.getArguments());
  ...
  // Do (3).
  SymbolTable::UseRange uses = *funcOp.getSymbolUses(irOp);
...
static void cleanRegionBranchOp(RegionBranchOpInterface regionBranchOp,
  ...
  auto markLiveArgs = [&](DenseMap<Region *, BitVector> &liveArgs) {
    for (Region &region : regionBranchOp->getRegions()) {
      SmallVector<Value, 4> arguments(region.front().getArguments());
  ...
  auto getSuccessors = [&](Region *region = nullptr) {
    ...
    regionBranchOp.getSuccessorRegions(index, operandAttributes, successors);
    return successors;
  };
  ...
  auto getForwardedOpOperands = [&](const RegionSuccessor &successor,
    ...
    OperandRange operands =
        terminator ? *getRegionBranchSuccessorOperands(terminator, index)
                   : regionBranchOp.getSuccessorEntryOperands(index);
...
struct RemoveNonLiveValues : public impl::RemoveNonLiveValuesBase<RemoveNonLiveValues> {
...
void RemoveNonLiveValues::runOnOperation() {
  ...
  WalkResult acceptableIR =
      irOp->walk([&](Operation *op) {
        if (isa<BranchOpInterface>(op) ||
            (isa<SymbolOpInterface>(op) && !isa<FunctionOpInterface>(op)) ||
            (isa<SymbolUserOpInterface>(op) && !isa<CallOpInterface>(op))) {
          op->emitWarning()
              << "Unacceptable IR encountered for the optimization pass "
                 "`remove-non-live-values`. Pass won't execute.\n";
