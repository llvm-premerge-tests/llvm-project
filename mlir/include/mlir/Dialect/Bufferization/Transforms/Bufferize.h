...
/// Bufferize the signature of `block` and its callers. All block argument types
/// are changed to memref types. All corresponding operands of all callers
/// (i.e., ops that have the given block as a successor) are wrapped in
/// bufferization.to_memref ops.
///
...
/// `BufferizableOpInterface`. The buffer types of tensor block arugments are
