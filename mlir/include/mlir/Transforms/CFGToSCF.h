...
public:
  ...
  /// by `createBranchRegionOp`. `branchRegionOp` is the operation returned by
  /// `createBranchRegionOp` while `results` are the values that should be
  /// returned by the branch region.
  virtual void createBranchRegionTerminatorOp(Location loc, OpBuilder &builder,
  ...
  /// values of the loop.
  virtual Operation *
  createDoWhileLoopOp(OpBuilder &builder, Operation *replacedOp,
                      ValueRange loopValuesInit, Value condition,
                      ValueRange loopValuesNextIter, Region &&loopBody) = 0;

  /// Creates a constant operation with a result representing `value` that is
  /// suitable as flag for `createCFGSwitchOperation`.
  ...
  virtual Operation *createUnreachableTerminator(Location loc,
                                                 OpBuilder &builder,
                                                 Region &region) = 0;

  /// Helper function to create a unconditional branch using
  /// `createCFGSwitchOperation`.
  void createSingleDestinationBranch(Location loc, OpBuilder &builder,
                                     Value dummyFlag, Block *destination,
                                     ValueRange arguments) {
    createCFGSwitchOperation(loc, builder, dummyFlag, {}, {}, {}, destination,
                             arguments);
  }
  ...
  void createConditionalBranch(Location loc, OpBuilder &builder,
    ...
    createCFGSwitchOperation(loc, builder, condition, {0}, {falseDest},
                             {falseArgs}, trueDest, trueArgs);
  }
};
...
/// Returns failure if any of these preconditions are not met.
