...
public:
  /// Creates a `scf.if` op if `controlFlowCondOp` is a `cf.cond_br` op or
  /// a `scf.index_switch` if `controlFlowCondOp` is a `cf.switch`.
  ...
  /// Creates a `scf.yield` op returning the given results.
  LogicalResult
  ...
  /// Creates a `scf.while` op. The loop body is made the before-region of the
  /// while op and terminated with a `scf.condition` op. The after-region does
