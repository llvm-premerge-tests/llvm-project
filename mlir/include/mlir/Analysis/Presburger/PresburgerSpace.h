...
/// is no way to distinguish if the variables in the two spaces are
/// different. So, we attach an Identifier corresponding to the loop iteration
/// variable to them. Now,
///
/// space(S0) = {d0(id = i), d1(id = j)}
/// space(S1) = {d0(id = i), d1(id = k)}.
///
/// Using the identifier, we can check that the first iteration variable in both
/// the spaces are same, while they are different for second iteration variable.
/// The type attached can be different for different variables in the space
///
/// The equality of Identifiers is checked by comparing the value of the
/// identifier. Checking equality asserts that the type of the equal
/// identifiers is same. Identifiers storing null pointers are treating as
/// having no attachment and are considered inequal to any other identifier,
/// including identifiers with no attachments.
class Identifier {
public:
  Identifier() = default;

  // Create an identifier from a pointer. The type of the pointer must have
  // a `llvm::PointerLikeTypeTraits` specialization.
  template <typename T>
...
/// non-local variable in space. This is disabled by default. `resetIds` is
/// used to enable/reset these identifiers. The user can identify each variable
/// in the space as corresponding to some Identifier. Some example use cases are
/// described in the `Identifier` documentation above.
...
public:
  ...
  /// Check if the spaces are compatible, as well as each non-local variable at
  /// the same position have equal identifiers. If the space is not using
