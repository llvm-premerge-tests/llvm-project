...
public:
  ...
  // an `llvm::PointerLikeTypeTraits` specialization.
  template <typename T>
...
public:
  ...
  /// Check if the spaces are compatible, and the non-local variables at the
  /// same position have equal identifiers. If the space is not using
