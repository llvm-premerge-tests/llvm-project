...
from enum import IntEnum, auto, IntFlag, _decompose
...
static void emitEnumClass(EnumAttr enumAttr, raw_ostream &os) {
  ...
  if (enumAttr.isBitEnum()) {
    os << llvm::formatv("    def __iter__(self): return "
                        "iter(_decompose(self.__class__, self.value)[0])\n");
    os << llvm::formatv(
        "    def __len__(self): return self.value.bit_count()\n");
    os << "\n";
  }
  ...
  if (enumAttr.isBitEnum())
    os << llvm::formatv(
        "        if len(self) > 1: return '{0}'.join(map(str, self))\n",
