...
static void emitBitEnumClass(const llvm::Record *def, raw_ostream &os) {
...
/// `false` on success, `true` on failure.
static bool emitBitEnumAttributeBuilder(const llvm::Record *def,
                                        raw_ostream &os) {
  EnumAttr enumAttr(*def->getValueAsDef("enum"));
  os << llvm::formatv("@_register_attribute_builder(\"{0}\")\n",
                      def->getName());
  ...
  os << llvm::formatv(
      ...
      def->getValueAsDef("dialect")->getValueAsString("name"),
...
static bool emitPythonEnums(const llvm::RecordKeeper &recordKeeper,
  ...
  for (const llvm::Record *def : defs) {
    ...
    emitBitEnumClass(def, os);
    emitBitEnumAttributeBuilder(def, os);
    ++emitted;
  }
