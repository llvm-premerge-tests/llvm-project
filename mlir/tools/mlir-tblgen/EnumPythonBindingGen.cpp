...
/// `false` on success, `true` on failure.
static bool emitBitEnumAttributeBuilder(const llvm::StringRef name,
                                        const llvm::StringRef dialect,
                                        const llvm::StringRef mnemonic,
                                        const EnumAttr &enumAttr,
                                        raw_ostream &os) {
  os << llvm::formatv("@register_attribute_builder(\"{0}\")\n", name);
  os << llvm::formatv(
      "def _{0}(x, context):\n",
  ...
  os << llvm::formatv(
      "    return "
      "_ods_ir.Attribute.parse(f'#{0}.{1}<{{str(x)}>', context=context)\n\n",
      dialect, mnemonic);
  return false;
}
...
static bool emitPythonEnums(const llvm::RecordKeeper &recordKeeper,
                            raw_ostream &os) {
  os << fileHeader;
  llvm::SmallSet<mlir::tblgen::EnumAttr *, 16> seenEnums;
  std::vector<llvm::Record *> defs =
      recordKeeper.getAllDerivedDefinitionsIfDefined("EnumAttr");
  for (const llvm::Record *def : defs) {
    EnumAttr enumAttr(*def->getValueAsDef("enum"));
    emitBitEnumClass(def->getName(), enumAttr.getSummary(),
                     enumAttr.getAllCases(), os);
    emitBitEnumAttributeBuilder(
        def->getName(), def->getValueAsDef("dialect")->getValueAsString("name"),
        def->getValueAsString("mnemonic"), enumAttr, os);
    seenEnums.insert(&enumAttr);
  }

  defs = recordKeeper.getAllDerivedDefinitionsIfDefined("EnumAttrInfo");
  for (const llvm::Record *def : defs) {
    EnumAttr enumAttr(*def);
    if (enumAttr.isBitEnum())
      continue;
    if (seenEnums.contains(&enumAttr))
      continue;
