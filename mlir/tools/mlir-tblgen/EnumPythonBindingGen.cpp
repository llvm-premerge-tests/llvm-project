...
/// `false` on success, `true` on failure.
static bool emitBitEnumAttributeBuilder(StringRef attrDefName,
                                        StringRef dialect, StringRef mnemonic,
                                        raw_ostream &os) {
  os << llvm::formatv("@_register_attribute_builder(\"{0}\")\n", attrDefName);
  ...
  os << llvm::formatv(
      "    return "
      "_ods_ir.Attribute.parse(f'#{0}.{1}<{{x}>', context=context)\n\n",
      dialect, mnemonic);
  return false;
}
...
static bool emitPythonEnums(const llvm::RecordKeeper &recordKeeper,
  ...
  llvm::SmallMapVector<llvm::StringRef, llvm::Record *, 16> correctlyNamedEnums;
  ...
  auto generateClas = [&os](EnumAttr *enumAttr, Attribute attr) {
    if (enumAttr->isBitEnum() || attr.isSubClassOf("EnumAttr"))
      emitBitEnumClass(enumAttr->getEnumClassName(), enumAttr->getSummary(),
  ...
  for (auto &it : correctlyNamedEnums) {
    ...
    generateClas(enumAttr, attr);
    if (enumAttr->isBitEnum())
      emitBitEnumAttributeBuilder(
          attr.getAttrDefName(),
          attr.getDef().getValueAsDef("dialect")->getValueAsString("name"),
          attr.getDef().getValueAsString("mnemonic"), os);
    else if (attr.isSubClassOf("EnumAttr"))
