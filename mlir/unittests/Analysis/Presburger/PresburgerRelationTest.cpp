...
TEST(IntegerRelationTest, symbolicLexOpt) {
  ...
  PresburgerRelation rel2 = parsePresburgerRelationFromPresburgerSet(
      ...
      // lexmin = (x, 0, 1 - x)
      // lexmax = (x, 1, 1)
      {"(x, y, z) : (x >= 0, y >= 0, z >= 0, 1 - x >= 0, 1 - y >= 0, 1 - z >= "
       "0, x + y + z - 1 >= 0)",
       // (x or y) and (y or z) and (z or x)
       // lexmin = (x, 1 - x, 1)
       // lexmax = (x, 1, 1)
       "(x, y, z) : (x >= 0, y >= 0, z >= 0, 1 - x >= 0, 1 - y >= 0, 1 - z >= "
       "0, x + y - 1 >= 0, y + z - 1 >= 0, z + x - 1 >= 0)",
       // x => (not y) or (not z)
       // lexmin = (x, 0, 0)
       // lexmax = (x, 1, 1 - x)
       "(x, y, z) : (x >= 0, y >= 0, z >= 0, 1 - x >= 0, 1 - y >= 0, 1 - z >= "
  ...
  PresburgerRelation rel3 = parsePresburgerRelationFromPresburgerSet(
      ...
      // lexmin = (x, 0, 0, 1 - x)
      // lexmax = (x, 1, 1 - x, x)
      {"(x, u, v, w) : (x >= 0, u >= 0, v >= 0, w >= 0, 1 - x >= 0, 1 - u >= "
       "0, 1 - v >= 0, 1 - w >= 0, -x + u + v + w >= 0, x + v - 1>= 0, x - w "
       ">= 0)",
       // x => (u => (v => w)) and (x or (not v)) and (x or (not w))
       // lexmin = (x, 0, 0, x)
       // lexmax = (x, 1, x, x)
       "(x, u, v, w) : (x >= 0, u >= 0, v >= 0, w >= 0, 1 - x >= 0, 1 - u >= "
       "0, 1 - v >= 0, 1 - w >= 0, -x - u - v + w + 2 >= 0, x - v >= 0, x - w "
       ">= 0)",
       ...
       // lexmax = (x, 1, x, x)
       "(x, u, v, w) : (x >= 0, u >= 0, v >= 0, w >= 0, 1 - x >= 0, 1 - u >= "
       "0, 1 - v >= 0, 1 - w >= 0, x + u - v >= 0, x - u + w >= 0, x - v >= 0, "
       "x - w >= 0)"},
      1);
