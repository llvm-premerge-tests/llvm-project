    ...
    for (auto lc_note : lc_notes) {
      ...
      if (m_data.GetU32(&payload_offset, &version, 1) != nullptr) {
        if (version == 1) {
          ...
          if (buf) {
            m_data.CopyData(payload_offset, strsize, buf);
            buf[strsize] = '\0';
            result = buf;
            if (buf)
              free(buf);
...
bool ObjectFileMachO::GetCorefileThreadExtraInfos(std::vector<tid_t> &tids) {
  ...
  if (module_sp) {
    ...
    for (auto lc_note : lc_notes) {
      ...
      if (!buf) {
        LLDB_LOGF(log,
                  "Unable to allocate %" PRIu64 " bytes to read 'process metadata' LC_NOTE JSON contents",
                  strsize);
        return false;
      }
      if (m_data.CopyData(payload_offset, strsize, buf) != strsize) {
        LLDB_LOGF(log,
                  "Unable to read %" PRIu64 " bytes of 'process metadata' LC_NOTE JSON contents",
                  strsize);
        free (buf);
        return false;
      }
  ...
  if (target_triple.getVendor() == llvm::Triple::Apple &&
    ...
    if (make_core) {
      ...
      if (range_error.Success()) {
        ...
        thread_extrainfo_lcnote_up->payload.PutRawBytes(strm.GetData(), strm.GetSize());
