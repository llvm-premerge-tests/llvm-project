...
bool ObjectFileMachO::GetCorefileThreadExtraInfos(std::vector<tid_t> &tids) {
  ...
  if (module_sp) {
    ...
    for (auto lc_note : lc_notes) {
      offset_t payload_offset = std::get<0>(lc_note);
      offset_t payload_size = std::get<1>(lc_note);
      const char *payload_cstr = m_data.GetCStr(&payload_offset, payload_size);
      if (!payload_cstr) {
        LLDB_LOGF(log,
                  "Unable to read 'process metadata' LC_NOTE JSON contents "
                  "size %" PRIu64,
                  payload_size);
        return false;
      }
      StructuredData::ObjectSP object_sp =
          StructuredData::ParseJSON(payload_cstr);
