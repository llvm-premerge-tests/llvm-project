...
// default for riscv32. These get changed in CreateInstance for riscv64
...
ABISysV_riscv::CreateInstance(ProcessSP process_sp, const ArchSpec &arch) {
  ...
  if (llvm::Triple::riscv64 == machine) {
    word_size = 8U;
    reg_size = word_size;
  }
  return (llvm::Triple::riscv32 == machine ||
          llvm::Triple::riscv64 == machine) ?
      ABISP(new ABISysV_riscv(std::move(process_sp), MakeMCRegisterInfo(arch))) :
      ABISP();
}
...
bool ABISysV_riscv::PrepareTrivialCall(Thread &thread, addr_t sp, addr_t pc,
...
static ValueObjectSP GetValObjFromIntRegs(Thread &thread,
  ...
  if (sizeof(uint32_t) >= byte_size) {
  ...
  } else if (sizeof(uint64_t) == byte_size) {
    ...
    if (llvm::Triple::riscv32 == machine) {
      ...
      if (reg_ctx->ReadRegister(reg_info_a0, reg_value_a0) &&
        ...
        Status error;
        if (reg_value_a0.GetAsMemoryData(
                *reg_info_a0, heap_data_up->GetBytes() + 0, 8,
                byte_order, error) &&
            reg_value_a1.GetAsMemoryData(
                *reg_info_a1, heap_data_up->GetBytes() + 8, 8,
                byte_order, error)) {
  ...
  // Floating point return type.
    ...
    if (compiler_type.IsFloatingPointType(float_count, is_complex) &&
      ...
      const uint32_t arch_fp_flags = arch.GetFlags() &
          ArchSpec::eRISCV_float_abi_mask;
      return_valobj_sp = GetValObjFromFPRegs(thread, reg_ctx, machine,
                                             arch_fp_flags, type_flags,
                                             byte_size);
      return return_valobj_sp;
    }
  }
...
bool ABISysV_riscv::RegisterIsCalleeSaved(const RegisterInfo *reg_info) {
  ...
  bool is_callee_saved = llvm::StringSwitch<bool>(name)
      ...
      .Cases("x1", "x2", "x8", "x9", "x18", "x19", "x20", "x21", "x22", true)
...
void ABISysV_riscv::Initialize() {
  PluginManager::RegisterPlugin(GetPluginNameStatic(),
                                "System V ABI for RISCV targets", CreateInstance);
}
...
static uint32_t GetGenericNum(llvm::StringRef name) {
  ...
  for (auto it : llvm::enumerate(regs)) {
    ...
    if (info.name == "zero")
      info.alt_name.SetCString("x0");
    else if (info.name == "ra")
      info.alt_name.SetCString("x1");
    else if (info.name == "sp")
      info.alt_name.SetCString("x2");
    else if (info.name == "gp")
      info.alt_name.SetCString("x3");
    else if (info.name == "fp")
      info.alt_name.SetCString("s0");
    else if (info.name == "s0")
      info.alt_name.SetCString("x8");
