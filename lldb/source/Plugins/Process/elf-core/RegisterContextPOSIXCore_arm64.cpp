...
bool RegisterContextCorePOSIX_arm64::ReadRegister(const RegisterInfo *reg_info,
  ...
  if (IsFPR(reg)) {
    ...
    // be non-register padding data after the header. Use the embedded header's
    // size field instead.
    lldb::offset_t size_offset = 0;
    uint32_t size = m_za_data.GetU32(&size_offset);
    ...
    if (!za_enabled) {
      uint64_t svl = m_sme_regs.svg_reg * 8;
      za_note_size = sizeof(sve::user_za_header) + (svl * svl);
    }

    offset = reg_info->byte_offset - m_register_info_up->GetZAOffset();
    assert(offset < za_note_size);
    ...
    value.SetFromMemoryData(*reg_info,
                            src + offset + sizeof(sve::user_za_header),
                            reg_info->byte_size, lldb::eByteOrderLittle, error);
  } else
    return false;
